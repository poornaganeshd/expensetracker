...
export default function Nomad() {
  ...
  const addE = data => {
    ...
    sEx(p => [rec, ...p]);
    sbUpsert("expenses", [toSB(rec, ["id", "amount", "categoryId", "walletId", "note", "date", "eventId", "groupId", "receipt_url", "paidBy", "balBefore"])]);
    dance();
  ...
  const addI = data => { const amt = roundMoney(data.amount); if (data.walletId === "upi_lite") { showT("UPI Lite is for spending only", "error"); return false } if (amt <= 0) { showT("Enter a valid amount", "error"); return false } if (amt > 10000000) { showT("Amount too large (max ₹1 crore)", "error"); return false } if (typeof data.note === "string" && data.note.length > 500) data = { ...data, note: data.note.slice(0, 500) }; const balBefore = roundMoney(wBal[data.walletId] || 0); const rec = { id: uid(), type: "income", ...data, amount: amt, balBefore, created_at: new Date().toISOString() }; sInc(p => [rec, ...p]); sbUpsert("incomes", [toSB(rec, ["id", "amount", "sourceId", "walletId", "note", "date", "receipt_url", "balBefore"])]); dance(); showT(online ? "Income added" : "Income saved offline", "success"); return true };
  const addT = data => { const b = roundMoney(wBal[data.fromWallet] || 0), amt = roundMoney(data.amount); if (amt <= 0) { showT("Enter an amount above zero", "error"); return false } if (amt > 10000000) { showT("Amount too large (max ₹1 crore)", "error"); return false } if (b < amt) { showT(`Insufficient balance`, "error"); return false } if (typeof data.note === "string" && data.note.length > 500) data = { ...data, note: data.note.slice(0, 500) }; const fromBalBefore = b; const toBalBefore = roundMoney(wBal[data.toWallet] || 0); const rec = { id: uid(), type: "transfer", ...data, amount: amt, fromBalBefore, toBalBefore, created_at: new Date().toISOString() }; sTr(p => [rec, ...p]); sbUpsert("transfers", [toSB(rec, ["id", "amount", "fromWallet", "toWallet", "note", "date", "fromBalBefore", "toBalBefore"])]); dance(); showT(online ? "Transfer done" : "Transfer queued offline", "success"); return true };
