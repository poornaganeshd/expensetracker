...
export default function Nomad() {
  ...
  const historyItems = useMemo(() => {
    ...
    if (searching) { const q = hSearch.toLowerCase().trim(); items = items.filter(it => (it.note || "").toLowerCase().includes(q) || (cats.find(c => c.id === it.categoryId)?.name || "").toLowerCase().includes(q) || (isrc.find(s => s.id === it.sourceId)?.name || "").toLowerCase().includes(q) || (it.splitName || "").toLowerCase().includes(q) || (it.name || "").toLowerCase().includes(q) || (evs.find(e => e.id === it.eventId)?.name || "").toLowerCase().includes(q)); }
