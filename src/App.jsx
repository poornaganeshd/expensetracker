...
function RecountM({ wallet: w, currentBal: cb, onShort, onOver, onClose: cl }) {
  const [v, sV] = useState(String(roundMoney(cb)));
  const numV = Number(v) || 0;
  const isNeg = numV < 0;
  const gap = roundMoney(numV - roundMoney(cb));
  const isShort = gap < 0;
  return <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.5)", zIndex: 200, display: "flex", alignItems: "flex-end", justifyContent: "center" }} onClick={cl}><div onClick={e => e.stopPropagation()} style={{ background: "var(--card)", borderRadius: "20px 20px 0 0", padding: 28, width: "100%", maxWidth: 430 }}><div style={{ fontFamily: "var(--font-h)", fontSize: 16, fontWeight: 700, color: "var(--text)", marginBottom: 6 }}>🔢 Recount {w.name}</div><p style={{ fontSize: 13, color: "var(--muted)", fontFamily: "var(--font-b)", marginBottom: 16, lineHeight: 1.5 }}>App shows <strong>{fmt(roundMoney(cb))}</strong>. Count your {w.id === "cash" ? "notes and coins" : "balance"} and enter the actual total.</p><label style={ls}>Actual Count ({CUR})</label><input type="number" value={v} onChange={e => sV(e.target.value)} autoFocus style={{ ...is, fontSize: 28, fontWeight: 700, fontFamily: "var(--font-h)", textAlign: "center", padding: "16px", color: isNeg ? "#D4726A" : gap < 0 ? "#E07A5F" : gap > 0 ? "#6BAA75" : w.color, borderColor: isNeg ? "#D4726A" : gap < 0 ? "#E07A5F" : gap > 0 ? "#6BAA75" : w.color, marginBottom: 8 }} />{isNeg && <p style={{ fontSize: 11, color: "#D4726A", marginBottom: 6, fontFamily: "var(--font-h)", fontWeight: 600 }}>Cannot be negative</p>}{gap !== 0 && !isNeg && <div style={{ fontSize: 12, fontFamily: "var(--font-h)", fontWeight: 700, color: isShort ? "#E07A5F" : "#6BAA75", background: isShort ? "#E07A5F15" : "#6BAA7515", borderRadius: 8, padding: "7px 14px", marginBottom: 16, textAlign: "center" }}>{isShort ? `${fmt(Math.abs(gap))} short — will log as expense` : `${fmt(gap)} extra — will log as income`}</div>}<div style={{ display: "flex", gap: 10, marginTop: 6 }}><button onClick={cl} style={{ flex: 1, padding: 14, border: "1.5px solid var(--border)", borderRadius: 12, background: "transparent", color: "var(--muted)", fontFamily: "var(--font-h)", fontSize: 14, cursor: "pointer" }}>Cancel</button><button disabled={gap === 0 || isNeg} onClick={() => { if (isShort) onShort(Math.abs(gap)); else onOver(gap); cl(); }} style={{ flex: 2, padding: 14, border: "none", borderRadius: 12, background: gap === 0 || isNeg ? "#ccc" : isShort ? "#E07A5F" : "#6BAA75", color: "#fff", fontFamily: "var(--font-h)", fontSize: 14, cursor: gap === 0 || isNeg ? "not-allowed" : "pointer", fontWeight: 700 }}>Log {gap === 0 ? "—" : isShort ? "Expense" : "Income"}</button></div></div></div>;
}
...
{module === "finance" && <div style={{ position: "fixed", bottom: 0, left: 0, right: 0, background: "var(--nav-bg)", backdropFilter: "blur(24px)", WebkitBackdropFilter: "blur(24px)", borderTop: "1px solid var(--border)", display: "flex", justifyContent: "center", maxWidth: 430, margin: "0 auto", zIndex: 50, paddingBottom: "env(safe-area-inset-bottom)" }}>{[{ id: "dashboard", label: "Home" }, { id: "add", label: "Add" }, { id: "events", label: "Events" }, { id: "history", label: "History" }, { id: "settings", label: "Settings" }].map(n => <button key={n.id} onClick={() => sTab(n.id)} style={{ flex: 1, padding: "10px 0 8px", border: "none", background: "none", display: "flex", flexDirection: "column", alignItems: "center", gap: 3, cursor: "pointer", opacity: tab === n.id ? 1 : 0.45 }}><div style={{ position: "relative" }}><NI type={n.id} active={tab === n.id} />{n.id === "settings" && deadLetterCount > 0 && <div style={{ position: "absolute", top: -2, right: -2, width: 8, height: 8, borderRadius: "50%", background: "#D4726A", border: "2px solid var(--nav-bg)" }} />}</div><span style={{ fontFamily: "var(--font-h)", fontSize: 9, color: tab === n.id ? "#E07A5F" : "var(--muted)", fontWeight: tab === n.id ? 600 : 400 }}>{n.label}</span></button>)}</div>}
    ...
    {calW && <CalM wallet={calW} currentBal={wBal[calW.id] || 0} onSave={(v, note) => handleCal(calW.id, v, note)} onClose={() => sCalW(null)} />}{recountW && <RecountM wallet={recountW} currentBal={wBal[recountW.id] || 0} onShort={amt => addE({ amount: amt, walletId: recountW.id, categoryId: cats[0]?.id || "other", note: "Cash recount", date: localDateKey() })} onOver={amt => addI({ amount: amt, sourceId: isrc[0]?.id, walletId: recountW.id, note: "Recount surplus", date: localDateKey() })} onClose={() => sRecountW(null)} />}
    {recEditId && (() => {
