...
const localMode = !_creds.sbUrl && typeof localStorage !== "undefined" && localStorage.getItem("nomad-local-mode") === "true";
const needsSetup = !_creds.sbUrl && !localMode;
...
export default function Nomad() {
  ...
  const subscribeToPush = async () => {
    ...
    try {
      ...
      if (!publicKey) { showT("Push server returned empty VAPID key", "error"); return; }
      // VAPID public key is URL-safe base64; pushManager.subscribe requires Uint8Array
      ...
      else { const txt = await r.text().catch(() => ""); showT(`Push subscribe failed: ${r.status} ${txt.slice(0, 80)}`, "error"); }
    } catch (e) { showT(`Push subscribe error: ${(e && e.message) || e}`, "error"); }
  };
