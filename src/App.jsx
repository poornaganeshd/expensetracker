...
function Events({ events: evs, expenses: ex, splits: sp, settlements: stl, categories: cats, wallets: wl = WALLETS, staleByEvent = {}, onCreate: oC, onAddExp: oE, onAddSplit: oS, onSettleSplit: oSS, onSettleEventNet: oSEN, onDeleteSplit: oDS, onDeleteExp: oDE, onMarkDone: oMD, onReopen: oRO, onDelete: oD, onUpdate: oU, onToast: showT = () => {}, dm = false }) {
  ...
  const addSplit = () => { const a = parseAmount(sa); if (!sn.trim() || !Number.isFinite(a) || a <= 0 || !sel) { if (sn.trim() && sa) showT("Enter a valid amount", "error"); return false; } if (a > 10000000) { showT("Amount too large (max ₹1 crore)", "error"); return false; } oS({ id: uid(), name: sn.trim(), amount: roundMoney(a), direction: sd, settled: false, eventId: sel.id, note: spNote }); sSN(""); sSA(""); sSPNote(""); return true; };
  ...
  const goDetail = (id) => { sSel(id); sEPaidBy("me"); sEA(""); sEN(""); sEC(cats[0]?.id || ""); sEW(wl[0]?.id || "bank"); sSN(""); sSA(""); sSPNote(""); sSD("owed"); sSTgt(null); sBsS(1); sBsM("equal"); sBsT(""); sBsP([{ name: "", amount: "" }]); sBsN(""); sBsC(cats[0]?.id || ""); sBsW(wl[0]?.id || "bank"); sEvDelConfirm(null); sDoneConfirm(false); sSuTgt(null); sEditOpen(false); sShowSettled(false); sAddSheet(null); sV("detail"); };
  ...
  if (view === "detail" && sel) {
    ...
    const avColor = name => { const pal = ["#E07A5F", "#6BAA75", "#7B8CDE", "#F4A261", "#A78BFA", "#4ECDC4"]; let h = 0; for (const ch of name) h = (h * 31 + ch.charCodeAt(0)) & 0xffff; return pal[h % pal.length]; };
    ...
    return <div style={{ textAlign: "left", padding: "10px 16px calc(env(safe-area-inset-bottom, 0px) + 120px)", maxWidth: 520, margin: "0 auto" }}>
      ...
      {addSheet === "bill" && !isGroup && <div style={shO} onClick={() => sAddSheet(null)}><div onClick={e => e.stopPropagation()} style={shI}>{(() => {
      ...
      {suTgt && <div style={shO} onClick={() => sSuTgt(null)}><div onClick={e => e.stopPropagation()} style={shI}><div style={{ fontFamily: "var(--font-h)", fontSize: 16, fontWeight: 700, color: "var(--text)", marginBottom: 8 }}>Settle up with {suTgt.from === "You" ? suTgt.to : suTgt.from}</div><div style={{ fontSize: 13, color: "var(--muted)", fontFamily: "var(--font-b)", marginBottom: 16, lineHeight: 1.6 }}>{suTgt.from === "You" ? <>You pay <strong style={{ color: "var(--text)" }}>{suTgt.to}</strong></> : <><strong style={{ color: "var(--text)" }}>{suTgt.from}</strong> pays you</>} <strong style={{ color: "var(--text)", fontFamily: "var(--font-h)" }}>{fmt(suTgt.amt)}</strong> — this clears every pending IOU between you two in this event in one move.</div><label style={ls}>{suTgt.from === "You" ? "Pay from" : "Receive into"}</label><div style={{ display: "flex", gap: 6, marginBottom: 18 }}>{wl.map(w => <button key={w.id} className="evPress" onClick={() => sSuW(w.id)} style={{ flex: 1, minWidth: 0, padding: "10px 4px", borderRadius: 10, border: `1.5px solid ${suW === w.id ? w.color : "var(--border)"}`, background: suW === w.id ? w.color + "15" : "var(--bg)", fontSize: 12, fontWeight: suW === w.id ? 600 : 500, fontFamily: "var(--font-h)", color: suW === w.id ? w.color : "var(--muted)", cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center", gap: 4 }}><DI2 id={w.id} accent={w.neon || w.color} size={12} />{w.name}</button>)}</div><div style={{ display: "flex", gap: 10 }}><button className="evPress" onClick={() => sSuTgt(null)} style={{ flex: 1, padding: 13, border: "1.5px solid var(--border)", borderRadius: 12, background: "transparent", color: "var(--muted)", fontFamily: "var(--font-h)", fontSize: 13, fontWeight: 600, cursor: "pointer" }}>Cancel</button><button className="evPress" onClick={() => { const ok = oSEN(sel.id, suTgt.from === "You" ? suTgt.to : suTgt.from, suW); if (ok !== false) sSuTgt(null); }} style={{ flex: 2, padding: 13, border: "none", borderRadius: 12, background: grn, color: "#fff", fontFamily: "var(--font-h)", fontSize: 13, fontWeight: 700, cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center", gap: 5 }}><IconCheck size={14} />Settle {fmt(suTgt.amt)}</button></div></div></div>}
