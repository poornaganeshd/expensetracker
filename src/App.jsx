...
export default function Nomad() {
  ...
  const expCsv = () => { const q = v => `"${String(v ?? "").replace(/"/g, '""')}"`; const catName = id => cats.find(c => c.id === id)?.name || id || "Uncategorized"; const srcName = id => isrc.find(s => s.id === id)?.name || id || "Other"; const wallet = id => WALLETS.find(w => w.id === id)?.name || id || ""; let csv = "Type,Date,Amount,Category/Source/From,To/Wallet,Note\n"; inc.forEach(i => { csv += `Income,${i.date || ""},${i.amount},${q(srcName(i.sourceId))},${q(wallet(i.walletId))},${q(i.note)}\n`; }); ex.forEach(e => { csv += `Expense,${e.date || ""},${e.amount},${q(catName(e.categoryId))},${q(wallet(e.walletId))},${q(e.note)}\n`; }); tr.forEach(t => { csv += `Transfer,${t.date || ""},${t.amount},${q(wallet(t.fromWallet))},${q(wallet(t.toWallet))},${q(t.note)}\n`; }); const a = document.createElement("a"); a.href = URL.createObjectURL(new Blob([csv], { type: "text/csv" })); a.download = `nomad_export_${localDateKey()}.csv`; a.click(); showT(`CSV exported (${ex.length + inc.length + tr.length} rows)`, "success") };
