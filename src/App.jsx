...
export default function Nomad() {
  ...
  const handleCal = (wId, desired, note = "") => {
    ...
    // Verify-only: the entered balance already matches NOMAD's, so there is
    // nothing to reconcile. Confirm "in sync" and skip the calibration log +
    // snapshot — logging a zero-gap entry would stamp a misleading ⚖ adjustment
    // badge on the wallet card for a change that never happened.
    if (gap === 0) { showT(`✓ ${wName} verified — balance in sync`, "success"); return; }
    ...
    showT(`${gap > 0 ? "Added" : "Removed"} ${fmt(Math.abs(gap))} — reconciliation logged`, "success");
  };
