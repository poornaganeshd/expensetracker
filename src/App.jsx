...
function Chart({ expenses: ex, incomes: inc, settlements: stl, months: ms }) {
  ...
  const gd = m => {
    const exp = ex.filter(x => mk(x.date) === m).reduce((s, x) => s + x.amount, 0);
    const settleOut = stl.filter(x => mk(x.date) === m && x.direction === "owe").reduce((s, x) => s + x.amount, 0);
    const settleIn = stl.filter(x => mk(x.date) === m && x.direction === "owed").reduce((s, x) => s + x.amount, 0);
    return { i: inc.filter(x => mk(x.date) === m).reduce((s, x) => s + x.amount, 0), e: Math.max(0, exp + settleOut - settleIn) };
  };
...
export default function Nomad() {
  ...
  useEffect(() => {
    ...
    backupDebounceRef.current = setTimeout(() => {
      try { localStorage.setItem("nomad-v5", JSON.stringify({ expenses: ex, incomes: inc, transfers: tr, settlements: stl, categories: cats, incomeSources: isrc, splits: sp, events: evs, recurring: rec, darkMode: dm, walletStartBal: wsb, _modified: Date.now() })) } catch { }
    }, 800);
...
`}</style>
    ...
    {module === "finance" && <div style={{ padding: "0 16px" }}>
      ...
      {tab === "dashboard" && <div className="pe">
        ...
        <div style={{ ...cc, padding: "18px 14px", marginBottom: 16, position: "relative", overflow: "hidden" }}><div style={{ position: "absolute", bottom: 0, right: 0, width: 60, height: 3, borderRadius: "3px 0 0 0", background: "#7B8CDE" }} /><div style={{ fontFamily: "var(--font-h)", fontSize: 12, color: "#7B8CDE", marginBottom: 8, letterSpacing: "0.5px", fontWeight: 700 }}>Trend</div><div style={{ overflowX: "auto", width: "100%" }}><Chart expenses={ex} incomes={inc} settlements={stl} months={allM} /></div></div>
      ...
      {tab === "settings" && <div className="pe" style={{ paddingTop: 8 }}>
        ...
        {(() => { const list = mt === "expense" ? cats : isrc; const shown = manageXp ? list : list.slice(0, 4); return <div style={{ ...cc, padding: 20, marginBottom: 14 }}><div style={{ fontFamily: "var(--font-h)", fontSize: 12, color: "var(--muted)", marginBottom: 14, letterSpacing: "0.5px", fontWeight: 600 }}>Manage</div><div style={{ display: "flex", gap: 6, marginBottom: 16 }}>{["expense", "income"].map(t => <button key={t} onClick={() => { sMt(t); sManageXp(false) }} style={{ flex: 1, padding: "9px", borderRadius: 10, fontSize: 13, fontFamily: "var(--font-h)", border: `1.5px solid ${mt === t ? "#E07A5F" : "var(--border)"}`, background: mt === t ? "#E07A5F" : "var(--card)", color: mt === t ? "#fff" : "var(--muted)", cursor: "pointer", fontWeight: 500 }}>{t === "expense" ? "Categories" : "Income Sources"}</button>)}</div>{shown.map(c => <div key={c.id} style={{ display: "flex", alignItems: "center", gap: 10, padding: "10px 12px", background: "var(--bg)", borderRadius: 10, marginBottom: 6, border: "1px solid var(--border)" }}><DI2 id={c.id} accent={c.neon || c.color} size={18} /><span style={{ flex: 1, fontSize: 13, color: "var(--text)", fontWeight: 500, fontFamily: "var(--font-h)" }}>{c.name}</span><span style={{ width: 14, height: 14, borderRadius: "50%", background: c.color }} /><button onClick={() => { const defs = mt === "expense" ? DC : DI; if (defs.find(d => d.id === c.id)) return; if (mt === "expense") sCats(p => p.filter(x => x.id !== c.id)); else sIsrc(p => p.filter(x => x.id !== c.id)) }} style={{ background: "none", border: "none", color: "var(--muted)", cursor: "pointer", fontSize: 14, padding: "2px 6px", opacity: (mt === "expense" ? DC : DI).find(d => d.id === c.id) ? 0.15 : 0.5 }}>✕</button></div>)}{list.length > 4 && <button onClick={() => sManageXp(!manageXp)} style={{ width: "100%", padding: "8px", border: "1px dashed var(--border)", borderRadius: 8, background: "none", color: "var(--muted)", fontFamily: "var(--font-h)", fontSize: 12, cursor: "pointer", marginBottom: 6 }}>{manageXp ? `Show less ▲` : `Show all ${list.length} ▼`}</button>}<div style={{ borderTop: "1px solid var(--border)", marginTop: 14, paddingTop: 14 }}><label style={ls}>Add New</label><div style={{ display: "flex", gap: 6, marginBottom: 10 }}><input value={ne2} onChange={e => sNE2(e.target.value)} maxLength={2} style={{ ...is, width: 48, textAlign: "center", flexShrink: 0 }} /><input value={nn} onChange={e => sNN(e.target.value)} placeholder="Name…" style={is} /><input type="color" value={nc} onChange={e => sNC(e.target.value)} style={{ width: 42, height: 42, border: "none", cursor: "pointer", borderRadius: 8, flexShrink: 0 }} /></div><button onClick={addCust} style={{ width: "100%", padding: "11px", border: "none", borderRadius: 10, background: "#7B8CDE", color: "#fff", fontFamily: "var(--font-h)", fontSize: 13, cursor: "pointer", fontWeight: 600 }}>+ Add {mt === "expense" ? "Category" : "Source"}</button></div></div> })()}
        <div style={{ ...cc, padding: 20, marginBottom: 14 }}><div style={{ fontFamily: "var(--font-h)", fontSize: 12, color: "var(--muted)", marginBottom: 14, letterSpacing: "0.5px", fontWeight: 600 }}>Backend</div><div style={{ fontSize: 12, color: "var(--muted)", marginBottom: 12, lineHeight: 1.6 }}>{_creds.sbUrl ? <span style={{ color: "#6BAA75", fontWeight: 600 }}>✓ Using your own Supabase</span> : <span>Using shared Supabase (env vars)</span>}<br />{_creds.cloudName ? <span style={{ color: "#6BAA75", fontWeight: 600 }}>✓ Using your own Cloudinary</span> : <span>Using shared Cloudinary</span>}</div><button onClick={() => setShowSetup(true)} style={{ width: "100%", padding: "12px", border: "1.5px solid #7B8CDE", borderRadius: 10, background: "#7B8CDE12", color: "#7B8CDE", fontFamily: "var(--font-h)", fontSize: 13, cursor: "pointer", fontWeight: 600 }}>Change Backend Credentials</button></div>
