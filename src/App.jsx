...
function PeopleLedger({ splits: sp, settlements: stl, events: evs = [], wallets: wl = WALLETS, onSettleAll = () => {} }) {
  ...
  const [settleTgt, sSettleTgt] = useState(null), [settleW, sSettleW] = useState(null);
  ...
  const openSettle = p => { sSettleTgt(p); sSettleW((p.net > 0 ? nonUpi[0] : wl[0] || nonUpi[0])?.id); };
  return <div style={{ ...cc, borderRadius: 16, padding: 18, marginBottom: 14, position: "relative", overflow: "hidden" }}>
    ...
    {settleTgt && (() => { const p = settleTgt; const youReceive = p.net > 0; const opts = youReceive ? nonUpi : wl; return <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.55)", zIndex: 200, display: "flex", alignItems: "flex-end", justifyContent: "center" }} onClick={() => sSettleTgt(null)}><div onClick={e => e.stopPropagation()} style={{ background: "var(--card)", borderRadius: "20px 20px 0 0", padding: 22, width: "100%", maxWidth: 430 }}><div style={{ fontFamily: "var(--font-h)", fontSize: 15, fontWeight: 700, color: "var(--text)", marginBottom: 6 }}>Settle up with {p.name}</div><div style={{ fontSize: 12.5, color: "var(--muted)", fontFamily: "var(--font-b)", marginBottom: 14, lineHeight: 1.5 }}>Nets {Object.keys(p.ctx).length} place{Object.keys(p.ctx).length === 1 ? "" : "s"} into one move — {youReceive ? <>you receive <strong style={{ color: "#6BAA75" }}>{fmt(Math.abs(p.net))}</strong></> : <>you pay <strong style={{ color: "#E07A5F" }}>{fmt(Math.abs(p.net))}</strong></>}.</div><div style={{ fontSize: 9, fontFamily: "var(--font-h)", color: "var(--muted)", fontWeight: 600, marginBottom: 6, letterSpacing: "0.5px" }}>{youReceive ? "RECEIVE INTO" : "PAY FROM"}</div><div style={{ display: "flex", gap: 6, marginBottom: 16, flexWrap: "wrap" }}>{opts.map(w => <button key={w.id} onClick={() => sSettleW(w.id)} style={{ flex: "1 0 26%", padding: "9px 4px", borderRadius: 10, display: "flex", flexDirection: "column", alignItems: "center", gap: 3, border: `2px solid ${settleW === w.id ? w.color : "var(--border)"}`, background: settleW === w.id ? w.color + "15" : "var(--bg)", cursor: "pointer" }}><DI2 id={w.id} accent={w.neon || w.color} size={16} /><span style={{ fontSize: 9, fontFamily: "var(--font-h)", fontWeight: settleW === w.id ? 700 : 500, color: settleW === w.id ? w.color : "var(--muted)" }}>{w.name}</span></button>)}</div><div style={{ display: "flex", gap: 10 }}><button onClick={() => sSettleTgt(null)} style={{ flex: 1, padding: 13, border: "1.5px solid var(--border)", borderRadius: 12, background: "transparent", color: "var(--muted)", fontFamily: "var(--font-h)", fontSize: 13, cursor: "pointer" }}>Cancel</button><button onClick={ev => { if (ev.currentTarget.disabled) return; ev.currentTarget.disabled = true; const ok = onSettleAll(p.name, settleW); if (ok !== false) { sSettleTgt(null); sExp(null); } else { ev.currentTarget.disabled = false; } }} style={{ flex: 2, padding: 13, border: "none", borderRadius: 12, background: "#7B8CDE", color: "#fff", fontFamily: "var(--font-h)", fontSize: 13, fontWeight: 700, cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center", gap: 5 }}><IconCheck size={14} />{youReceive ? `Collect ${fmt(Math.abs(p.net))}` : `Pay ${fmt(Math.abs(p.net))}`}</button></div></div></div>; })()}
  </div>;
}
...
export default function Nomad() {
  ...
  const settleAllWith = (name, wid) => {
    const remOf = s => roundMoney(s.amount - stl.filter(x => x.splitId === s.id).reduce((t, x) => t + x.amount, 0));
    const nameLc = String(name || "").trim().toLowerCase();
    const items = sp.filter(s => (s.name || "").trim().toLowerCase() === nameLc && !s.deleted_at && !s.settled && !s.skipped).map(s => ({ s, rem: remOf(s) })).filter(x => x.rem > 0.005);
    if (!items.length) { showT("Nothing pending to settle", "info"); return false; }
    const net = roundMoney(items.reduce((t, x) => t + (x.s.direction === "owed" ? x.rem : -x.rem), 0));
    const youPay = net < -0.005;
    const hasIncoming = net > 0.005 || items.some(x => x.s.direction === "owed");
    ...
    const recs = items.map(x => ({ id: uid(), type: "settlement", splitName: x.s.name, splitId: x.s.id, amount: x.rem, direction: x.s.direction, walletId: wid, date: today, createdAt: new Date().toISOString(), ...(x.s.categoryId && { categoryId: x.s.categoryId }), ...(x.s.note && { note: x.s.note }), ...(x.s.groupId && { groupId: x.s.groupId }), ...(x.s.eventId && { eventId: x.s.eventId }) }));
    const ids = items.map(x => x.s.id);
    sStl(p => [...p, ...recs]);
    sbUpsert("settlements", recs.map(r => toSB(r, COLS.settlements)));
    sSp(p => p.map(x => ids.includes(x.id) ? { ...x, settled: true } : x));
