  ...
  return <div style={{ padding: "0 0 20px" }}>
    ...
    {type !== "recurring" && <><div style={{ marginBottom: 16 }}><label style={ls}>Amount</label>
      ...
      <div style={{ position: "relative", display: "flex", alignItems: "center", background: "var(--card)", border: `1.5px solid ${tc}`, borderRadius: 10, overflow: "hidden" }}>
        ...
        {type !== "transfer" && <span style={{ marginLeft: 16, fontSize: 22, lineHeight: 1, flexShrink: 0, userSelect: "none", color: tc, fontFamily: "var(--font-h)", fontWeight: 700 }}>₹</span>}
      ...
      {type === "expense" && <div style={{ background: "var(--card)", border: "1px solid var(--border)", borderRadius: 16, padding: "16px 16px 6px", marginBottom: 14, boxShadow: "0 1px 3px rgba(0,0,0,0.03)" }}><div style={{ display: "flex", alignItems: "center", gap: 6, marginBottom: 12 }}><div style={{ width: 3, height: 14, background: tc, borderRadius: 2 }} /><div style={{ fontFamily: "var(--font-h)", fontSize: 11, color: tc, letterSpacing: "1.5px", fontWeight: 700 }}>PAYMENT</div></div><label style={ls}>Pay From</label><WB wallets={aw} sel={wid} onSel={sW} /><label style={ls}>Category</label><div style={{ display: "flex", flexWrap: "wrap", gap: 6, marginBottom: 14 }}>{[...cats, ...(cats.find(c => c.id === "other") ? [] : [DC.find(c => c.id === "other")])].map(c => <button key={c.id} onClick={() => sCat(c.id)} style={{ padding: "8px 14px", borderRadius: 10, fontSize: 13, fontFamily: "var(--font-b)", border: `1.5px solid ${catId === c.id ? c.color : "var(--border)"}`, background: catId === c.id ? c.color + "18" : "var(--bg)", color: catId === c.id ? c.color : "var(--ts)", cursor: "pointer", fontWeight: catId === c.id ? 600 : 400, display: "flex", alignItems: "center", gap: 5 }}><DI2 id={c.id} accent={c.neon || c.color} size={14} />{c.name}</button>)}</div></div>}
      ...
      <div style={type === "expense" ? { background: "var(--card)", border: "1px solid var(--border)", borderRadius: 16, padding: "16px 16px 8px", marginBottom: 14, boxShadow: "0 1px 3px rgba(0,0,0,0.03)" } : {}}>{type === "expense" && <div style={{ display: "flex", alignItems: "center", gap: 6, marginBottom: 12 }}><div style={{ width: 3, height: 14, background: tc, borderRadius: 2 }} /><div style={{ fontFamily: "var(--font-h)", fontSize: 11, color: tc, letterSpacing: "1.5px", fontWeight: 700 }}>DETAILS</div></div>}
...
{module === "finance" && <div style={{ position: "fixed", bottom: 0, left: 0, right: 0, maxWidth: 430, margin: "0 auto", zIndex: 50, paddingBottom: "env(safe-area-inset-bottom)" }}><div style={{ position: "relative", height: 76 }}><svg width="100%" height="76" viewBox="0 0 430 76" preserveAspectRatio="none" style={{ position: "absolute", inset: 0, display: "block", filter: "drop-shadow(0 -3px 14px rgba(0,0,0,0.08))" }}><path d="M0,16 L168,16 C188,16 192,48 215,48 C238,48 242,16 262,16 L430,16 L430,76 L0,76 Z" fill="var(--nav-bg)" stroke="var(--border)" strokeWidth="1" /></svg><div style={{ position: "absolute", inset: "16px 0 0 0", display: "flex" }}>{[{ id: "dashboard", label: "Home" }, { id: "events", label: "Events" }, { id: "__fab", label: "" }, { id: "history", label: "History" }, { id: "settings", label: "Settings" }].map(n => n.id === "__fab" ? <div key="__fab" style={{ flex: 1 }} /> : <button key={n.id} onClick={() => sTab(n.id)} style={{ flex: 1, padding: "8px 0 0", border: "none", background: "none", display: "flex", flexDirection: "column", alignItems: "center", gap: 3, cursor: "pointer", opacity: tab === n.id ? 1 : 0.45 }}><div style={{ position: "relative" }}><NI type={n.id} active={tab === n.id} />{n.id === "settings" && deadLetterCount > 0 && <div style={{ position: "absolute", top: -2, right: -2, width: 8, height: 8, borderRadius: "50%", background: "#D4726A", border: "2px solid var(--nav-bg)" }} />}</div><span style={{ fontFamily: "var(--font-h)", fontSize: 9, color: tab === n.id ? "#E07A5F" : "var(--muted)", fontWeight: tab === n.id ? 600 : 400 }}>{n.label}</span></button>)}</div><button onClick={() => sTab("add")} aria-label="Add" style={{ position: "absolute", top: -16, left: "50%", width: 56, height: 56, borderRadius: "50%", border: "none", background: tab === "add" ? "#D4704A" : "#E07A5F", color: "#fff", cursor: "pointer", boxShadow: "0 6px 16px rgba(224,122,95,0.45), 0 2px 4px rgba(224,122,95,0.3)", display: "flex", alignItems: "center", justifyContent: "center", transform: `translateX(-50%) scale(${tab === "add" ? 0.94 : 1})`, transition: "transform 0.15s ease, background 0.15s ease" }}><svg width="26" height="26" viewBox="0 0 24 24" fill="none" stroke="#fff" strokeWidth="2.6" strokeLinecap="round"><line x1="12" y1="5" x2="12" y2="19" /><line x1="5" y1="12" x2="19" y2="12" /></svg></button></div></div>}
