...
export default function Nomad() {
  ...
  useEffect(() => { if (!loaded || isDemoMode) return; sLionMsgLoading(true); const ninety = new Date(); ninety.setDate(ninety.getDate() - 90); const d90 = ninety.toISOString().slice(0, 10); const rEx = ex.filter(e => String(e.date || "") >= d90); const rInc = inc.filter(i => String(i.date || "") >= d90); const cm = localDateKey().slice(0, 7); const totalInc = rInc.reduce((s, i) => s + i.amount, 0); const totalExp = rEx.reduce((s, e) => s + e.amount, 0); const catTotals = {}; rEx.forEach(e => { catTotals[e.categoryId] = (catTotals[e.categoryId] || 0) + e.amount; }); const topCats = Object.entries(catTotals).sort((a, b) => b[1] - a[1]).slice(0, 5).map(([id, amt]) => ({ name: cats.find(c => c.id === id)?.name || id, amount: amt, pct: totalExp > 0 ? Math.round(amt / totalExp * 100) : 0 })); const wBals = wallets.map(w => ({ name: w.name, balance: roundMoney(wBal[w.id] || 0) })); fetch("/api/ai-chat", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ question: "Give me a single witty one-liner (max 15 words) as my cheeky lion finance mascot. Roast or celebrate me based on the data. No advice, just personality!", context: { month: cm, totalIncome: totalInc, totalExpense: totalExp, topCategories: topCats, walletBalances: wBals, recurringCount: rec.filter(r => r.active !== false).length, streak: finStreak } }) }).then(r => r.json()).then(d => { if (d.answer) sLionMsg(d.answer.replace(/^["']|["']$/g, "").slice(0, 120)); sLionMsgLoading(false); }).catch(() => { sLionMsg(TIPS[Math.floor(Math.random() * TIPS.length)]); sLionMsgLoading(false); }); }, [loaded, ex.length]);
