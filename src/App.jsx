...
import {
  ...
  recurringDaysOverdue, distributeAmount, expenseShareMap, historySortCompare, itemTimestamp,
...
function SettleM({ split: sp, remaining: rm, onConfirm: oc, onClose: cl, wallets: wl = WALLETS }) {
  const isO = sp.direction === "owed"; const walletOptions = isO ? wl.filter(w => !isUpiLite(w)) : wl; const [wid, sW] = useState(() => defaultSettleWalletId(sp.direction, wl, isUpiLite) || "bank"); const maxAmt = rm ?? sp.amount; const [amt, sAmt] = useState(String(maxAmt)); const parsedAmt = parseAmount(amt); const validAmt = Math.min(Math.max(Number.isFinite(parsedAmt) ? parsedAmt : 0, 0.01), maxAmt); const isPartial = validAmt < maxAmt - 0.005;
...
function Splits({ splits: sp, settlements: stl, categories: cats = [], onAdd, onSettle: os, onSettleNet: osn = () => {}, onDelete: od, onSkip: ok, onWriteOff: owf = () => {}, expanded: exp, onToggle: ot, wallets: wl = WALLETS, walletBalances: wBalances = {}, onError = () => {} }) {
  ...
  const goDrill = n => { sSNP(null); sSNW(null); sSNA(""); sSAW(null); sDelConfirm(null); sDrill(n); };
  // Open the net-settle panel for one person, pre-filling the wallet and the
  // amount input with their full net (the common case = clear it all). Used by
  // every "Settle net" entry point so partial-amount state can't leak between people.
  const openNet = name => { sSNP(name); sSNW(defaultNetWid); sSNA(String(roundMoney(Math.abs(personMap[name]?.net || 0)))); sSAW(null); };
  ...
  // Canonicalize people by trimmed/lower-cased name so "Rahul", "rahul" and
  // "Rahul " collapse into ONE person — keyed by the first-seen display spelling.
  // This matches how the event settles already compare names case-insensitively.
  const _canonName = {}; const dispOf = raw => { const k = String(raw || "").trim().toLowerCase(); if (!k) return ""; if (!_canonName[k]) _canonName[k] = String(raw).trim(); return _canonName[k]; };
  const personMap = {}; sp.filter(s=>!s.eventId&&!s.deleted_at).forEach(s=>{ const n=dispOf(s.name); if(!personMap[n]) personMap[n]={splits:[],net:0}; personMap[n].splits.push(s); if(!s.settled){const rem=remainForSplit(s); personMap[n].net+=s.direction==="owed"?rem:-rem;} });
  const stlByPerson = {}; (stl||[]).filter(s=>!s.eventId).forEach(s=>{ const n=dispOf(s.splitName); if(!stlByPerson[n])stlByPerson[n]=[]; stlByPerson[n].push(s);});
  ...
  const netBtn = name => (mixedFor(name) && settleNetPerson!==name) ? <button onClick={e=>{e.stopPropagation();openNet(name);}} style={{padding:"6px 10px",border:"1.5px solid #7B8CDE",borderRadius:8,background:"#7B8CDE12",color:"#7B8CDE",fontFamily:"var(--font-h)",fontSize:11,fontWeight:700,cursor:"pointer",flexShrink:0,display:"flex",alignItems:"center",gap:4}}><ArrowsLeftRight size={12} weight="bold"/>Settle net</button> : null;
  const netPanel = name => { if(settleNetPerson!==name) return null; const pmx=personMap[name]||{net:0,splits:[]}; const netx=pmx.net; const absNet=roundMoney(Math.abs(netx)); const pendN=pmx.splits.filter(s=>!s.settled); const entered=parseAmount(settleNetAmt); const validEntered=Number.isFinite(entered)&&entered>0; const payNow=validEntered?roundMoney(Math.min(entered,absNet)):absNet; const isPartial=validEntered&&payNow<absNet-0.005; return <div onClick={e=>e.stopPropagation()} style={{background:"var(--bg)",borderRadius:12,padding:14,marginBottom:12,border:"1.5px solid #7B8CDE"}}><div style={{fontFamily:"var(--font-h)",fontSize:12,fontWeight:700,color:"var(--text)",marginBottom:4}}>{absNet<0.005?`Settle up with ${name}`:netx<0?`Pay net ${fmt(absNet)} to ${name}`:`Collect net ${fmt(absNet)} from ${name}`}</div><div style={{fontSize:10.5,color:"var(--muted)",fontFamily:"var(--font-b)",marginBottom:10,lineHeight:1.4}}>Nets all {pendN.length} IOUs — owe/owed cancel out. Pay the full net to clear them, or enter less to settle part now.</div><label style={{fontSize:9,fontFamily:"var(--font-h)",color:"var(--muted)",fontWeight:600,marginBottom:4,letterSpacing:"0.5px",display:"block"}}>AMOUNT</label><input type="number" inputMode="decimal" value={settleNetAmt} onChange={e=>sSNA(e.target.value)} placeholder="₹" style={{...is,marginBottom:4}}/><div style={{fontSize:10,color:"var(--muted)",fontFamily:"var(--font-b)",marginBottom:10}}>Full net {fmt(absNet)}{isPartial?` · ${fmt(roundMoney(absNet-payNow))} stays pending`:""}</div><div style={{fontSize:9,fontFamily:"var(--font-h)",color:"var(--muted)",fontWeight:600,marginBottom:6,letterSpacing:"0.5px"}}>{netx<0?"PAY FROM":"RECEIVE INTO"}</div><div style={{display:"flex",gap:6,marginBottom:10}}>{nonUpiWallets.map(w=><button key={w.id} onClick={()=>sSNW(w.id)} style={{flex:1,padding:"8px 4px",borderRadius:8,display:"flex",flexDirection:"column",alignItems:"center",gap:3,border:`2px solid ${settleNetWid===w.id?w.color:"var(--border)"}`,background:settleNetWid===w.id?w.color+"15":"var(--card)",cursor:"pointer"}}><DI2 id={w.id} accent={w.neon||w.color} size={16}/><span style={{fontSize:8,fontFamily:"var(--font-h)",fontWeight:settleNetWid===w.id?700:500,color:settleNetWid===w.id?w.color:"var(--muted)"}}>{w.name}</span></button>)}</div><div style={{display:"flex",gap:8}}><button onClick={()=>{sSNP(null);sSNW(null);sSNA("");}} style={{flex:1,padding:"9px",border:"1.5px solid var(--border)",borderRadius:9,background:"transparent",color:"var(--muted)",fontFamily:"var(--font-h)",fontSize:12,cursor:"pointer"}}>Cancel</button><button onClick={ev=>{if(ev.currentTarget.disabled)return;ev.currentTarget.disabled=true;const ok=osn(name,settleNetWid,isPartial?settleNetAmt:"");if(ok!==false){sSNP(null);sSNW(null);sSNA("");}else{ev.currentTarget.disabled=false;}}} style={{flex:2,padding:"9px",border:"none",borderRadius:9,background:"#7B8CDE",color:"#fff",fontFamily:"var(--font-h)",fontSize:12,fontWeight:700,cursor:"pointer",display:"flex",alignItems:"center",justifyContent:"center",gap:5}}><IconCheck size={14}/>{absNet<0.005?"Confirm settled":netx<0?`Pay ${fmt(payNow)}`:`Collect ${fmt(payNow)}`}</button></div></div>; };
...
// Global per-person ledger: nets every pending IOU with each person across
// BOTH personal splits and every event, so you see "Rahul owes you ₹420 net
// across everything" and can clear it in one settle. People are canonicalized
// case-insensitively. Only shown when at least one person has IOUs spanning more
// than one context (otherwise the personal Splits card already covers it).
function PeopleLedger({ splits: sp, settlements: stl, events: evs = [], wallets: wl = WALLETS, onSettleAll = () => {} }) {
  const [open, sOpen] = useState(true);
  const [expanded, sExp] = useState(null);
  const [settleTgt, sSettleTgt] = useState(null), [settleW, sSettleW] = useState(null), [settleAmt, sSettleAmt] = useState("");
  const people = useMemo(() => {
    const remOf = s => roundMoney((s.amount || 0) - (stl || []).filter(x => x.splitId === s.id).reduce((t, x) => t + x.amount, 0));
    const canon = {}; const disp = raw => { const k = String(raw || "").trim().toLowerCase(); if (!k) return ""; if (!canon[k]) canon[k] = String(raw).trim(); return canon[k]; };
    // Only ACTIVE events feed the cross-everything net. A completed/deleted
    // event's IOUs drop out of the rolling balance (you settle those inside the
    // event when you wrap it up); status is absent on legacy events = active.
    const activeEvIds = new Set((evs || []).filter(e => e && e.status !== "completed" && !e.deleted_at).map(e => e.id));
    const map = {};
    (sp || []).forEach(s => {
      if (!s || s.settled || s.skipped || s.deleted_at) return;
      if (s.eventId && !activeEvIds.has(s.eventId)) return;
      const rem = remOf(s); if (rem <= 0.005) return;
      const name = disp(s.name); if (!name) return;
      const signed = s.direction === "owed" ? rem : -rem;
      if (!map[name]) map[name] = { name, net: 0, owe: 0, owed: 0, ctx: {} };
      map[name].net = roundMoney(map[name].net + signed);
      if (s.direction === "owed") map[name].owed = roundMoney(map[name].owed + rem); else map[name].owe = roundMoney(map[name].owe + rem);
      const ckey = s.eventId || "__personal__";
      if (!map[name].ctx[ckey]) map[name].ctx[ckey] = { name: s.eventId ? ((evs.find(e => e.id === s.eventId) || {}).name || "Event") : "Personal", net: 0, count: 0 };
      map[name].ctx[ckey].net = roundMoney(map[name].ctx[ckey].net + signed); map[name].ctx[ckey].count++;
    });
    return Object.values(map).filter(p => Math.abs(p.net) > 0.005 || p.owe > 0.005 || p.owed > 0.005).sort((a, b) => Math.abs(b.net) - Math.abs(a.net));
  }, [sp, stl, evs]);
  // Surface only when there's real cross-context netting to do.
  if (!people.some(p => Object.keys(p.ctx).length > 1)) return null;
  const nonUpi = wl.filter(w => !isUpiLite(w));
  const openSettle = p => { sSettleTgt(p); sSettleW((p.net > 0 ? nonUpi[0] : wl[0] || nonUpi[0])?.id); sSettleAmt(String(roundMoney(Math.abs(p.net)))); };
  return <div style={{ ...cc, borderRadius: 16, padding: 18, marginBottom: 14, position: "relative", overflow: "hidden" }}>
    <div style={{ position: "absolute", bottom: 0, right: 0, width: 60, height: 3, borderRadius: "3px 0 0 0", background: "#7B8CDE" }} />
    <div onClick={() => sOpen(o => !o)} style={{ display: "flex", justifyContent: "space-between", alignItems: "center", cursor: "pointer", marginBottom: open ? 12 : 0 }}><div style={{ display: "flex", alignItems: "center", gap: 6 }}><ArrowsLeftRight size={14} weight="bold" color="#7B8CDE" /><div style={{ fontFamily: "var(--font-h)", fontSize: 12, color: "#7B8CDE", letterSpacing: "0.5px", fontWeight: 700 }}>Net across everything</div></div><span style={{ fontSize: 10, color: "var(--muted)", transition: "transform 0.2s", display: "inline-block", transform: open ? "rotate(0deg)" : "rotate(-90deg)" }}>▾</span></div>
    {open && people.map(p => { const isE = expanded === p.name; const aC = avatarColor(p.name); const ctxs = Object.values(p.ctx); const settled = Math.abs(p.net) < 0.005; return <div key={p.name} style={{ marginBottom: 8, borderRadius: 14, border: `1px solid ${p.net > 0 ? "#6BAA7530" : p.net < 0 ? "#E07A5F30" : "var(--border)"}`, overflow: "hidden" }}>
      <div onClick={() => sExp(isE ? null : p.name)} style={{ display: "flex", alignItems: "center", gap: 12, padding: "12px 14px", background: "var(--bg)", cursor: "pointer" }}><div style={{ width: 38, height: 38, borderRadius: "50%", background: aC, display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0 }}><span style={{ fontFamily: "var(--font-h)", fontSize: 13, fontWeight: 700, color: "#fff" }}>{initials(p.name)}</span></div><div style={{ flex: 1, minWidth: 0 }}><div style={{ fontSize: 14, fontWeight: 700, fontFamily: "var(--font-h)", color: "var(--text)" }}>{p.name}</div><div style={{ fontSize: 10.5, color: "var(--muted)", fontFamily: "var(--font-b)" }}>{ctxs.length} place{ctxs.length === 1 ? "" : "s"}{p.owe > 0.005 && p.owed > 0.005 ? ` · owe ${fmt(p.owe)} / owed ${fmt(p.owed)}` : ""}</div></div><div style={{ textAlign: "right", flexShrink: 0 }}>{settled ? <span style={{ fontSize: 11, color: "var(--muted)", display: "inline-flex", alignItems: "center", gap: 3 }}><IconCheck size={11} />even</span> : <><div style={{ fontFamily: "var(--font-h)", fontWeight: 800, fontSize: 16, color: p.net > 0 ? "#6BAA75" : "#E07A5F" }}>{fmt(Math.abs(p.net))}</div><div style={{ fontSize: 8.5, letterSpacing: "0.5px", color: p.net > 0 ? "#6BAA75" : "#E07A5F", fontFamily: "var(--font-h)", fontWeight: 600 }}>{p.net > 0 ? "NET OWED YOU" : "NET YOU OWE"}</div></>}</div></div>
      {isE && <div style={{ borderTop: "1px solid var(--border)", background: "var(--card)", padding: "8px 14px" }}>{ctxs.sort((a, b) => Math.abs(b.net) - Math.abs(a.net)).map((c, i) => <div key={i} style={{ display: "flex", justifyContent: "space-between", alignItems: "center", padding: "6px 0", borderBottom: i < ctxs.length - 1 ? "1px solid var(--border)" : "none" }}><span style={{ fontSize: 12, fontFamily: "var(--font-h)", color: "var(--ts)" }}>{c.name}<span style={{ color: "var(--muted)", fontWeight: 500 }}> · {c.count} IOU{c.count === 1 ? "" : "s"}</span></span><span style={{ fontSize: 12.5, fontFamily: "var(--font-h)", fontWeight: 700, color: Math.abs(c.net) < 0.005 ? "var(--muted)" : c.net > 0 ? "#6BAA75" : "#E07A5F" }}>{Math.abs(c.net) < 0.005 ? "even" : `${c.net > 0 ? "+" : "−"}${fmt(Math.abs(c.net))}`}</span></div>)}{!settled && <button onClick={() => openSettle(p)} style={{ width: "100%", marginTop: 10, padding: "9px", border: "1.5px solid #7B8CDE", borderRadius: 9, background: "#7B8CDE18", color: "#7B8CDE", fontFamily: "var(--font-h)", fontSize: 12, fontWeight: 700, cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center", gap: 6 }}><ArrowsLeftRight size={14} weight="bold" />Settle everything with {p.name}</button>}</div>}
    </div>; })}
    {settleTgt && (() => { const p = settleTgt; const youReceive = p.net > 0; const opts = youReceive ? nonUpi : wl; const absNet = roundMoney(Math.abs(p.net)); const entered = parseAmount(settleAmt); const validEntered = Number.isFinite(entered) && entered > 0; const payNow = validEntered ? roundMoney(Math.min(entered, absNet)) : absNet; const isPartial = validEntered && payNow < absNet - 0.005; return <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.55)", zIndex: 200, display: "flex", alignItems: "flex-end", justifyContent: "center" }} onClick={() => sSettleTgt(null)}><div onClick={e => e.stopPropagation()} style={{ background: "var(--card)", borderRadius: "20px 20px 0 0", padding: 22, width: "100%", maxWidth: 430 }}><div style={{ fontFamily: "var(--font-h)", fontSize: 15, fontWeight: 700, color: "var(--text)", marginBottom: 6 }}>Settle up with {p.name}</div><div style={{ fontSize: 12.5, color: "var(--muted)", fontFamily: "var(--font-b)", marginBottom: 14, lineHeight: 1.5 }}>Nets {Object.keys(p.ctx).length} place{Object.keys(p.ctx).length === 1 ? "" : "s"} into one move — {youReceive ? <>you receive <strong style={{ color: "#6BAA75" }}>{fmt(absNet)}</strong></> : <>you pay <strong style={{ color: "#E07A5F" }}>{fmt(absNet)}</strong></>}. Pay the full net or enter less to settle part now.</div><label style={{ fontSize: 9, fontFamily: "var(--font-h)", color: "var(--muted)", fontWeight: 600, marginBottom: 6, letterSpacing: "0.5px", display: "block" }}>AMOUNT</label><input type="number" inputMode="decimal" value={settleAmt} onChange={e => sSettleAmt(e.target.value)} placeholder="₹" style={{ ...is, marginBottom: 4 }} /><div style={{ fontSize: 10.5, color: "var(--muted)", fontFamily: "var(--font-b)", marginBottom: 14 }}>Full net {fmt(absNet)}{isPartial ? ` · ${fmt(roundMoney(absNet - payNow))} stays pending` : ""}</div><div style={{ fontSize: 9, fontFamily: "var(--font-h)", color: "var(--muted)", fontWeight: 600, marginBottom: 6, letterSpacing: "0.5px" }}>{youReceive ? "RECEIVE INTO" : "PAY FROM"}</div><div style={{ display: "flex", gap: 6, marginBottom: 16, flexWrap: "wrap" }}>{opts.map(w => <button key={w.id} onClick={() => sSettleW(w.id)} style={{ flex: "1 0 26%", padding: "9px 4px", borderRadius: 10, display: "flex", flexDirection: "column", alignItems: "center", gap: 3, border: `2px solid ${settleW === w.id ? w.color : "var(--border)"}`, background: settleW === w.id ? w.color + "15" : "var(--bg)", cursor: "pointer" }}><DI2 id={w.id} accent={w.neon || w.color} size={16} /><span style={{ fontSize: 9, fontFamily: "var(--font-h)", fontWeight: settleW === w.id ? 700 : 500, color: settleW === w.id ? w.color : "var(--muted)" }}>{w.name}</span></button>)}</div><div style={{ display: "flex", gap: 10 }}><button onClick={() => sSettleTgt(null)} style={{ flex: 1, padding: 13, border: "1.5px solid var(--border)", borderRadius: 12, background: "transparent", color: "var(--muted)", fontFamily: "var(--font-h)", fontSize: 13, cursor: "pointer" }}>Cancel</button><button onClick={ev => { if (ev.currentTarget.disabled) return; ev.currentTarget.disabled = true; const ok = onSettleAll(p.name, settleW, isPartial ? settleAmt : ""); if (ok !== false) { sSettleTgt(null); sExp(null); } else { ev.currentTarget.disabled = false; } }} style={{ flex: 2, padding: 13, border: "none", borderRadius: 12, background: "#7B8CDE", color: "#fff", fontFamily: "var(--font-h)", fontSize: 13, fontWeight: 700, cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center", gap: 5 }}><IconCheck size={14} />{youReceive ? `Collect ${fmt(payNow)}` : `Pay ${fmt(payNow)}`}</button></div></div></div>; })()}
  </div>;
}
...
function Events({ events: evs, expenses: ex, splits: sp, settlements: stl, categories: cats, wallets: wl = WALLETS, staleByEvent = {}, onCreate: oC, onAddExp: oE, onAddSplit: oS, onSettleSplit: oSS, onSettleEventNet: oSEN, onDeleteSplit: oDS, onSkipSplit: oSK = () => {}, onDeleteExp: oDE, onEditExp: oEditExp = () => {}, onEditSplit: oEditSplit = () => {}, onMarkDone: oMD, onReopen: oRO, onDelete: oD, onUpdate: oU, onToast: showT = () => {}, dm = false }) {
  ...
  if (view === "detail" && sel) {
    ...
    return <div style={{ textAlign: "left", padding: "10px 16px calc(env(safe-area-inset-bottom, 0px) + 120px)", maxWidth: 520, margin: "0 auto" }}>
      ...
      {eExps.length > 0 && <div style={{ ...ccL, padding: "14px 16px 6px", marginBottom: 14, animation: "evSlideUp 0.4s 0.13s ease both" }}><div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 4 }}><span style={secHd}>EXPENSES</span><span style={{ fontSize: 10.5, color: "var(--muted)", fontFamily: "var(--font-b)" }}>{eExps.length}</span></div>{[...eExps].reverse().map((e, xi, arr) => { const cat = cats.find(c => c.id === e.categoryId) || { id: "other", name: "Other", color: "#999", neon: "#999" }; return <div key={e.id} style={{ display: "flex", alignItems: "center", gap: 11, padding: "10px 0", borderBottom: xi < arr.length - 1 ? "1px solid var(--border)" : "none", animation: "evRowIn 0.35s ease both", animationDelay: `${Math.min(xi, 8) * 0.04}s` }}><div style={{ width: 34, height: 34, borderRadius: 11, background: (cat.color || "#999") + "16", display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0 }}><DI2 id={cat.id} accent={cat.neon || cat.color} size={17} /></div><div style={{ flex: 1, minWidth: 0 }}><div style={{ fontSize: 13.5, fontWeight: 600, color: "var(--text)", fontFamily: "var(--font-h)" }}>{cat.name}</div>{e.note && <div style={{ fontSize: 11, color: "var(--muted)", marginTop: 1, whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis", fontFamily: "var(--font-b)" }}>{e.note}</div>}{isGroup && e.paidBy && e.paidBy !== "me" && <div style={{ fontSize: 10, color: ind, marginTop: 1, fontFamily: "var(--font-h)", fontWeight: 600 }}>paid by {e.paidBy}</div>}</div><span style={{ fontFamily: "var(--font-h)", fontWeight: 700, fontSize: 14, color: acc, flexShrink: 0 }}>−{fmt(e.amount)}</span>{sel.status === "active" && <button className="evPress" onClick={() => openEditExp(e)} title="Edit expense" style={{ background: "none", border: "none", color: "var(--muted)", cursor: "pointer", opacity: 0.5, flexShrink: 0, padding: 2, display: "flex" }}><IconPencil size={14} /></button>}{sel.status === "active" && <button className="evPress" onClick={() => oDE(e.id)} title="Delete expense" style={{ background: "none", border: "none", color: "var(--muted)", cursor: "pointer", opacity: 0.5, flexShrink: 0, padding: 2, display: "flex" }}><IconX size={14} /></button>}</div>; })}</div>}
      {eSp.length > 0 && <div style={{ ...ccL, padding: "14px 16px 6px", marginBottom: 14, animation: "evSlideUp 0.4s 0.16s ease both" }}><div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 4 }}><span style={secHd}>SPLITS</span><span style={{ fontSize: 10.5, color: "var(--muted)", fontFamily: "var(--font-b)" }}>{eSp.length}</span></div>{(() => { const pend = eSp.filter(x => !x.settled), doneSp = eSp.filter(x => x.settled); const rows = showSettled ? [...pend, ...doneSp] : pend; return <>{rows.map((s, si) => <div key={s.id} style={{ display: "flex", alignItems: "center", gap: 11, padding: "10px 0", borderBottom: (si < rows.length - 1 || doneSp.length > 0) ? "1px solid var(--border)" : "none", opacity: s.settled ? 0.45 : 1, animation: "evRowIn 0.35s ease both", animationDelay: `${Math.min(si, 8) * 0.04}s` }}><div style={{ width: 34, height: 34, borderRadius: "50%", background: (s.skipped ? "#F4A261" : s.settled || s.direction !== "owe" ? grn : acc) + "16", display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0, color: s.skipped ? "#F4A261" : s.settled || s.direction !== "owe" ? grn : acc }}>{s.skipped ? <IconPlayerSkipForward size={15} /> : s.settled ? <CheckCircle size={16} weight="fill" /> : s.direction === "owe" ? <ArrowUpRight size={15} /> : <ArrowDownLeft size={15} />}</div><div style={{ flex: 1, minWidth: 0 }}><div style={{ fontSize: 13.5, fontWeight: 600, color: "var(--text)", fontFamily: "var(--font-h)" }}>{s.name}</div><div style={{ fontSize: 10.5, color: "var(--muted)", marginTop: 1, fontFamily: "var(--font-b)" }}>{s.skipped ? `Skipped${(paidBySplit[s.id] || 0) > 0.005 ? ` · ${fmt(remainForSplit(s))} written off` : ""}` : s.settled ? "Settled" : s.direction === "owe" ? "You owe" : "Owes you"}{!s.settled && (paidBySplit[s.id] || 0) > 0.005 && <span style={{ color: ind, fontWeight: 600 }}> · {fmt(paidBySplit[s.id])} paid, {fmt(remainForSplit(s))} left</span>}{!s.settled && staleIds.has(s.id) && <span style={{ color: acc, fontWeight: 600 }}> · pending 2+ days</span>}</div>{s.note && <div style={{ fontSize: 10, color: "var(--muted)", marginTop: 1, fontStyle: "italic", whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis", fontFamily: "var(--font-b)" }}>{s.note}</div>}</div><span style={{ fontFamily: "var(--font-h)", fontWeight: 700, fontSize: 14, color: s.skipped ? "#F4A261" : s.direction === "owe" ? acc : grn, textDecoration: s.skipped ? "line-through" : "none", flexShrink: 0 }}>{fmt(s.amount)}</span>{!s.settled && !s.groupId && <button className="evPress" onClick={() => openEditSplit(s)} title="Edit IOU" style={{ border: "1px solid var(--border)", background: "var(--bg)", borderRadius: 14, padding: "5px 9px", color: "var(--muted)", cursor: "pointer", flexShrink: 0, display: "flex", alignItems: "center" }}><IconPencil size={13} /></button>}{!s.settled && <button className="evPress" onClick={() => sSTgt(s)} style={{ border: "1px solid var(--border)", background: "var(--bg)", borderRadius: 14, padding: "5px 11px", fontSize: 10.5, fontWeight: 600, fontFamily: "var(--font-h)", color: "var(--ts)", cursor: "pointer", flexShrink: 0 }}>Settle</button>}{!s.settled && <button className="evPress" onClick={() => oSK(s.id)} title="Skip — write off without payment" style={{ border: "1px solid #F4A26140", background: "#F4A26112", borderRadius: 14, padding: "5px 9px", color: "#F4A261", cursor: "pointer", flexShrink: 0, display: "flex", alignItems: "center" }}><IconPlayerSkipForward size={13} /></button>}<button className="evPress" onClick={() => oDS(s.id)} style={{ background: "none", border: "none", color: "var(--muted)", cursor: "pointer", opacity: 0.5, flexShrink: 0, padding: 2, display: "flex" }}><IconX size={14} /></button></div>)}{doneSp.length > 0 && <button className="evPress" onClick={() => sShowSettled(v => !v)} style={{ width: "100%", background: "none", border: "none", padding: "10px 0", display: "flex", alignItems: "center", justifyContent: "center", gap: 5, color: "var(--muted)", fontSize: 10.5, fontFamily: "var(--font-h)", fontWeight: 600, cursor: "pointer" }}>{showSettled ? "Hide" : "Show"} {doneSp.length} settled<IconChevronRight size={12} style={{ transform: showSettled ? "rotate(90deg)" : "none", transition: "transform 0.2s" }} /></button>}</>; })()}</div>}
...
export default function Nomad() {
  ...
  // GLOBAL sibling of settleNet: nets a person's pending IOUs across EVERYTHING —
  // personal splits AND every event — in one movement. Same net-funds validation,
  // but settlements keep their eventId/groupId so each event ledger still
  // reconciles. Name match is case-insensitive (canonical person identity).
  const settleAllWith = (name, wid, payAmt) => {
    const remOf = s => roundMoney(s.amount - stl.filter(x => x.splitId === s.id).reduce((t, x) => t + x.amount, 0));
    const nameLc = String(name || "").trim().toLowerCase();
    // Mirror PeopleLedger: net only personal + ACTIVE-event IOUs. Completed/
    // deleted events are settled within the event, never in the rolling net.
    const activeEvIds = new Set(evs.filter(e => e && e.status !== "completed" && !e.deleted_at).map(e => e.id));
    const items = sp.filter(s => (s.name || "").trim().toLowerCase() === nameLc && !s.deleted_at && !s.settled && !s.skipped && (!s.eventId || activeEvIds.has(s.eventId))).map(s => ({ s, rem: remOf(s) })).filter(x => x.rem > 0.005);
    if (!items.length) { showT("Nothing pending to settle", "info"); return false; }
    const net = roundMoney(items.reduce((t, x) => t + (x.s.direction === "owed" ? x.rem : -x.rem), 0));
    const hasPayAmt = payAmt != null && payAmt !== "";
    const payNum = hasPayAmt ? Number(payAmt) : null;
    ...
    // Partial: pay down the net direction only, up to the capped amount. Spans
    // personal + every event; remainder and opposite-direction IOUs stay pending.
    if (hasPayAmt && roundMoney(payNum) < Math.abs(net) - 0.005) {
      const dir = net > 0 ? "owed" : "owe";
      if (dir === "owed" && isUpiLite(wid, wallets)) { showT("UPI Lite cannot receive money — pick another wallet", "error"); return false; }
      let cap = roundMoney(Math.min(payNum, Math.abs(net)));
    ...
    sSp(p => p.map(x => ids.includes(x.id) ? { ...x, settled: true } : x));
    ids.forEach(id => sbUpsert("splits", [{ id, settled: true }], `splits:${id}`));
    showT(Math.abs(net) < 0.005 ? `Settled up with ${name} everywhere ✓` : youPay ? `Paid net ${fmt(-net)} to ${name} ✓` : `Received net ${fmt(net)} from ${name} ✓`, online ? "success" : "info");
    return true;
  };
...
`}</style>
    ...
    {module === "finance" && <div style={{ padding: "0 16px" }}>
      ...
      {tab === "dashboard" && <div className="pe">
        ...
        <PeopleLedger splits={sp} settlements={stl} events={evs} wallets={wallets} onSettleAll={settleAllWith} />
