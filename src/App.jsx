...
export default function Nomad() {
  ...
  // Expenses the user actually paid from their OWN wallets. "__tracked__" rows are
  // group-event bills someone ELSE paid (logged only for event accounting; the
  // user's real cost is the linked "owe" split/settlement). They never touched a
  // real wallet, so they must NOT inflate personal-spending totals. They stay in
  // `ex` so they still render in history and feed event netSpent.
  const mySpendEx = useMemo(() => ex.filter(e => e.walletId !== "__tracked__"), [ex]);
  const finScore = useMemo(() => computeFinanceScore({ expenses: mySpendEx, incomes: inc, recurring: rec }), [mySpendEx, inc, rec]);
  ...
  const tI = flt.incomes.reduce((s, i) => s + i.amount, 0), tE = Math.max(0, flt.expenses.filter(e => e.walletId !== "__tracked__").reduce((s, e) => s + e.amount, 0) + flt.settlements.filter(s => s.direction === "owe").reduce((s, x) => s + x.amount, 0) - flt.settlements.filter(s => s.direction === "owed").reduce((s, x) => s + x.amount, 0));
  ...
  const exAll = useMemo(() => [...mySpendEx, ...settlementsAsExpenses], [mySpendEx, settlementsAsExpenses]);
  ...
  useEffect(() => {
    ...
    const totalExp = mySpendEx.reduce((s, e) => s + e.amount, 0);
    const catTotals = {};
    mySpendEx.forEach(e => { catTotals[e.categoryId] = (catTotals[e.categoryId] || 0) + e.amount; });
