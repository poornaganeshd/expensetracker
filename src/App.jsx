...
import { IconCheck, IconTrash, IconHistory, IconChevronRight, IconChevronLeft, IconSend, IconAlertTriangle, IconX, IconClock, IconArrowDown, IconArrowUp, IconPlus } from "@tabler/icons-react";
...
function Splits({ splits: sp, settlements: stl, onAdd, onSettle: os, onDelete: od, expanded: exp, onToggle: ot }) {
  ...
  const tO = sp.filter(s => s.direction === "owe" && !s.settled && !s.eventId).reduce((t, s) => t + s.amount, 0), tI = sp.filter(s => s.direction === "owed" && !s.settled && !s.eventId).reduce((t, s) => t + s.amount, 0);
  const fmtDate = ts => { try { return new Date(ts).toLocaleDateString("en-IN", { month: "short", day: "numeric" }); } catch { return ""; } };
  if (!exp) return <div onClick={ot} style={{ ...cc, borderRadius: 16, padding: "16px 18px", marginBottom: 14, cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "space-between", position: "relative", overflow: "hidden" }}><div style={{ position: "absolute", bottom: 0, right: 0, width: 60, height: 3, borderRadius: "3px 0 0 0", background: "#D4726A" }} /><div><div style={{ fontFamily: "var(--font-h)", fontSize: 12, color: "#D4726A", letterSpacing: "0.5px", fontWeight: 700 }}>Split Expenses</div><div style={{ fontSize: 11, color: "var(--muted)", fontFamily: "var(--font-b)", marginBottom: 4 }}>Personal IOUs · group splits live in Events</div><div style={{ fontSize: 13, fontFamily: "var(--font-b)", color: "var(--ts)", marginTop: 2 }}>{activePeople.length === 0 ? "No pending splits" : <><span style={{ color: "#E07A5F" }}>You owe {fmt(tO)}</span> · <span style={{ color: "#6BAA75" }}>Owed {fmt(tI)}</span></>}</div></div><span style={{ fontSize: 18, color: "var(--muted)" }}>→</span></div>;
  if (exp && drill) {
    const pm = personMap[drill] || { splits: [], net: 0 }; const net = pm.net; const pStls = stlByPerson[drill] || [];
    const events = [...pm.splits.map(s => ({ ...s, _k: "split" })), ...pStls.map(s => ({ ...s, _k: "stl" }))].sort((a, b) => { const ta = a._k === "split" ? (a.createdAt || a.id) : (a.createdAt || a.date); const tb = b._k === "split" ? (b.createdAt || b.id) : (b.createdAt || b.date); return new Date(tb) - new Date(ta); });
    return <div style={{ ...cc, borderRadius: 16, padding: 18, marginBottom: 14, position: "relative", overflow: "hidden" }}><div style={{ position: "absolute", bottom: 0, right: 0, width: 60, height: 3, borderRadius: "3px 0 0 0", background: "#D4726A" }} /><div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 12 }}><button onClick={() => sDrill(null)} style={{ background: "none", border: "none", fontSize: 12, color: "var(--muted)", cursor: "pointer", fontFamily: "var(--font-h)" }}>← All</button><div style={{ fontFamily: "var(--font-h)", fontSize: 14, fontWeight: 700, color: "var(--text)" }}>{drill}</div><div style={{ fontSize: 12, fontFamily: "var(--font-h)", fontWeight: 700, color: Math.abs(net) < 0.01 ? "var(--muted)" : net > 0 ? "#6BAA75" : "#E07A5F" }}>{Math.abs(net) < 0.01 ? "Settled up ✅" : net > 0 ? `Owes you ${fmt(net)}` : `You owe ${fmt(-net)}`}</div></div>
    ...
    {events.map(ev => ev._k === "split" ? <div key={ev.id} style={{ display: "flex", alignItems: "center", gap: 10, padding: "10px 12px", background: "var(--bg)", borderRadius: 10, marginBottom: 6, border: `1px solid ${ev.settled ? "var(--border)" : ev.direction === "owe" ? "#E07A5F40" : "#6BAA7540"}`, opacity: ev.settled ? 0.55 : 1 }}><span style={{ fontSize: 15 }}>{ev.settled ? "✅" : ev.direction === "owe" ? "🔴" : "🟢"}</span><div style={{ flex: 1 }}><div style={{ fontSize: 12, fontFamily: "var(--font-h)", fontWeight: 600, color: "var(--text)" }}>{ev.direction === "owe" ? "You owe" : "Owes you"}{ev.note && <span style={{ opacity: 0.7, fontWeight: 400 }}> · {ev.note}</span>}</div><div style={{ fontSize: 10, color: "var(--muted)", fontFamily: "var(--font-b)" }}>{ev.settled ? "Settled" : "Pending"}{ev.createdAt && ` · ${fmtDate(ev.createdAt)}`}</div></div><span style={{ fontFamily: "var(--font-h)", fontWeight: 700, fontSize: 14, color: ev.direction === "owe" ? "#E07A5F" : "#6BAA75" }}>{fmt(ev.amount)}</span>{!ev.settled && <button onClick={() => sT(ev)} style={{ border: "1px solid var(--border)", background: "none", borderRadius: 6, padding: "4px 8px", fontSize: 10, color: "var(--muted)", cursor: "pointer" }}>Settle</button>}<button onClick={() => od(ev.id)} style={{ background: "none", border: "none", color: "var(--muted)", cursor: "pointer", fontSize: 14, opacity: 0.35 }}>✕</button></div> : <div key={ev.id} style={{ display: "flex", alignItems: "center", gap: 10, padding: "10px 12px", background: "var(--bg)", borderRadius: 10, marginBottom: 6, border: "1px solid var(--border)", opacity: 0.65 }}><span style={{ fontSize: 15 }}>💸</span><div style={{ flex: 1 }}><div style={{ fontSize: 12, fontFamily: "var(--font-h)", fontWeight: 600, color: "var(--text)" }}>{ev.direction === "owed" ? "Received back" : "Paid back"}</div><div style={{ fontSize: 10, color: "var(--muted)", fontFamily: "var(--font-b)" }}>{ev.createdAt ? fmtDate(ev.createdAt) : ev.date || ""}</div></div><span style={{ fontFamily: "var(--font-h)", fontWeight: 700, fontSize: 14, color: ev.direction === "owed" ? "#6BAA75" : "#E07A5F" }}>{ev.direction === "owed" ? "+" : "−"}{fmt(ev.amount)}</span></div>)}
    <div style={{ borderTop: "1px solid var(--border)", marginTop: 14, paddingTop: 14 }}><div style={{ display: "flex", gap: 6, marginBottom: 8 }}>{["owe", "owed"].map(d => <button key={d} onClick={() => sD(d)} style={{ flex: 1, padding: "8px", borderRadius: 8, fontSize: 12, fontFamily: "var(--font-h)", border: `1.5px solid ${dir === d ? (d === "owe" ? "#E07A5F" : "#6BAA75") : "var(--border)"}`, background: dir === d ? (d === "owe" ? "#E07A5F18" : "#6BAA7518") : "var(--card)", color: dir === d ? (d === "owe" ? "#E07A5F" : "#6BAA75") : "var(--muted)", cursor: "pointer", fontWeight: 500 }}>{d === "owe" ? "I owe them" : "They owe me"}</button>)}</div><div style={{ display: "flex", gap: 6 }}><input type="number" value={am} onChange={e => sA(e.target.value)} placeholder="₹ amount" style={{ ...is, flex: 1, marginBottom: 0 }} /><input value={snote} onChange={e => sSnote(e.target.value)} placeholder="Note" style={{ ...is, flex: 1, marginBottom: 0 }} /><button onClick={add} style={{ padding: "10px 14px", border: "none", borderRadius: 10, background: "#E07A5F", color: "#fff", fontFamily: "var(--font-h)", fontSize: 13, fontWeight: 600, cursor: "pointer" }}>+</button></div></div>
    {st && <SettleM split={st} onConfirm={wid => { os(st.id, wid); sT(null) }} onClose={() => sT(null)} />}</div>;
  }
  if (exp && showFeed) {
    const feed = [...sp.filter(s => !s.eventId).map(s => ({ ...s, _k: "split" })), ...(stl || []).filter(s => !s.eventId).map(s => ({ ...s, _k: "stl" }))].sort((a, b) => { const ta = a._k === "split" ? (a.createdAt || a.id) : (a.createdAt || a.date); const tb = b._k === "split" ? (b.createdAt || b.id) : (b.createdAt || b.date); return new Date(tb) - new Date(ta); });
    return <div style={{ ...cc, borderRadius: 16, padding: 18, marginBottom: 14, position: "relative", overflow: "hidden" }}><div style={{ position: "absolute", bottom: 0, right: 0, width: 60, height: 3, borderRadius: "3px 0 0 0", background: "#D4726A" }} /><div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 12 }}><button onClick={() => sSF(false)} style={{ background: "none", border: "none", fontSize: 12, color: "var(--muted)", cursor: "pointer", fontFamily: "var(--font-h)" }}>← Back</button><div style={{ fontFamily: "var(--font-h)", fontSize: 13, fontWeight: 700, color: "#D4726A" }}>Split History</div><div style={{ width: 40 }} /></div>
    ...
    {feed.map(ev => ev._k === "split" ? <div key={ev.id} style={{ display: "flex", alignItems: "center", gap: 10, padding: "10px 12px", background: "var(--bg)", borderRadius: 10, marginBottom: 6, border: `1px solid ${ev.settled ? "var(--border)" : ev.direction === "owe" ? "#E07A5F40" : "#6BAA7540"}`, opacity: ev.settled ? 0.55 : 1 }}><span style={{ fontSize: 15 }}>{ev.settled ? "✅" : ev.direction === "owe" ? "🔴" : "🟢"}</span><div style={{ flex: 1 }}><div style={{ fontSize: 13, fontWeight: 600, fontFamily: "var(--font-h)", color: "var(--text)" }}>{ev.name}</div><div style={{ fontSize: 11, color: "var(--muted)", fontFamily: "var(--font-b)" }}>{ev.direction === "owe" ? "You owe" : "Owes you"}{ev.note && ` · ${ev.note}`}{ev.createdAt && ` · ${fmtDate(ev.createdAt)}`}</div></div><span style={{ fontFamily: "var(--font-h)", fontWeight: 700, fontSize: 14, color: ev.direction === "owe" ? "#E07A5F" : "#6BAA75" }}>{fmt(ev.amount)}</span></div> : <div key={ev.id} style={{ display: "flex", alignItems: "center", gap: 10, padding: "10px 12px", background: "var(--bg)", borderRadius: 10, marginBottom: 6, border: "1px solid var(--border)", opacity: 0.65 }}><span style={{ fontSize: 15 }}>💸</span><div style={{ flex: 1 }}><div style={{ fontSize: 13, fontWeight: 600, fontFamily: "var(--font-h)", color: "var(--text)" }}>{ev.splitName}</div><div style={{ fontSize: 11, color: "var(--muted)", fontFamily: "var(--font-b)" }}>{ev.direction === "owed" ? "Received" : "Paid"}{ev.createdAt ? ` · ${fmtDate(ev.createdAt)}` : ev.date ? ` · ${ev.date}` : ""}</div></div><span style={{ fontFamily: "var(--font-h)", fontWeight: 700, fontSize: 14, color: ev.direction === "owed" ? "#6BAA75" : "#E07A5F" }}>{ev.direction === "owed" ? "+" : "−"}{fmt(ev.amount)}</span></div>)}
    </div>;
  }
  return <div style={{ ...cc, borderRadius: 16, padding: 18, marginBottom: 14, position: "relative", overflow: "hidden" }}><div style={{ position: "absolute", bottom: 0, right: 0, width: 60, height: 3, borderRadius: "3px 0 0 0", background: "#D4726A" }} /><div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 4 }}><div style={{ fontFamily: "var(--font-h)", fontSize: 12, color: "#D4726A", letterSpacing: "0.5px", fontWeight: 700 }}>Split Expenses</div><div style={{ display: "flex", gap: 6 }}><button onClick={() => sSF(true)} style={{ background: "none", border: "1px solid var(--border)", borderRadius: 6, padding: "4px 10px", fontSize: 10, color: "var(--muted)", cursor: "pointer", fontFamily: "var(--font-h)" }}>History</button><button onClick={ot} style={{ background: "none", border: "none", fontSize: 12, color: "var(--muted)", cursor: "pointer", fontFamily: "var(--font-h)" }}>← Back</button></div></div>
  <div style={{ fontSize: 10, color: "var(--muted)", fontFamily: "var(--font-b)", marginBottom: 12 }}>Track who owes who for informal splits. Tap a person to see full history.</div>
  ...
  {activePeople.map(name => { const pm = personMap[name]; const net = pm.net; const pending = pm.splits.filter(s => !s.settled); const settleable = pending.length === 1 ? pending[0] : null; return <div key={name} onClick={() => sDrill(name)} style={{ display: "flex", alignItems: "center", gap: 12, padding: "12px 14px", background: "var(--bg)", borderRadius: 12, marginBottom: 8, border: `1px solid ${net > 0 ? "#6BAA7540" : "#E07A5F40"}`, cursor: "pointer" }}><div style={{ width: 36, height: 36, borderRadius: 10, background: net > 0 ? "#6BAA7518" : "#E07A5F18", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 15, flexShrink: 0 }}>{net > 0 ? "🟢" : "🔴"}</div><div style={{ flex: 1, minWidth: 0 }}><div style={{ fontSize: 14, fontWeight: 700, fontFamily: "var(--font-h)", color: "var(--text)" }}>{name}</div><div style={{ fontSize: 11, color: "var(--muted)", fontFamily: "var(--font-b)" }}>{net > 0 ? "Owes you" : "You owe"}{pending.length > 1 && ` · ${pending.length} IOUs`}</div></div><div style={{ textAlign: "right", flexShrink: 0 }}><div style={{ fontFamily: "var(--font-h)", fontWeight: 700, fontSize: 15, color: net > 0 ? "#6BAA75" : "#E07A5F" }}>{fmt(Math.abs(net))}</div>{settleable && <button onClick={e => { e.stopPropagation(); sT(settleable); }} style={{ border: "1px solid var(--border)", background: "none", borderRadius: 6, padding: "3px 8px", fontSize: 10, color: "var(--muted)", cursor: "pointer", marginTop: 3 }}>Settle</button>}{pending.length > 1 && <div style={{ fontSize: 10, color: "var(--muted)", fontFamily: "var(--font-h)", marginTop: 3 }}>tap → details</div>}</div></div>; })}
  {settledPeople.length > 0 && <details style={{ marginTop: 4 }}><summary style={{ fontSize: 11, color: "var(--muted)", cursor: "pointer", fontFamily: "var(--font-h)", fontWeight: 500, marginBottom: 6 }}>Settled up ({settledPeople.length})</summary>{settledPeople.map(name => <div key={name} onClick={() => sDrill(name)} style={{ display: "flex", alignItems: "center", gap: 10, padding: "10px 12px", background: "var(--bg)", borderRadius: 10, marginBottom: 4, opacity: 0.5, cursor: "pointer", border: "1px solid var(--border)" }}><span style={{ fontSize: 14 }}>✅</span><span style={{ flex: 1, fontSize: 13, fontFamily: "var(--font-h)", color: "var(--ts)", fontWeight: 600 }}>{name}</span><span style={{ fontSize: 11, color: "var(--muted)", fontFamily: "var(--font-h)" }}>All settled</span></div>)}</details>}
...
function RecountM({ wallet: w, currentBal: cb, onClose: cl }) {
  ...
  const isShort = gap < 0;
  const adj = (d, n) => sCounts(p => ({ ...p, [d]: Math.max(0, (p[d] || 0) + n) }));
...
export default function Nomad() {
  ...
  const [notifPerm, sNotifPerm] = useState(() => ("Notification" in window ? Notification.permission : "unsupported"));
  const [pushSubscribed, sPushSubscribed] = useState(() => localStorage.getItem("nomad-push-subscribed") === "1");
  ...
  const subscribeToPush = async () => {
    ...
    try {
      ...
      if (!keyRes.ok) { showT("Push server not configured — add VAPID keys to Vercel", "info"); return; }
      ...
      const r = await fetch("/api/push-subscribe", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ subscription: sub.toJSON(), supabase_url: SB_URL, anon_key: _creds.sbKey }) });
      if (r.ok) { localStorage.setItem("nomad-push-subscribed", "1"); sPushSubscribed(true); showT("Push notifications active — you'll get reminders even when the app is closed", "success"); }
    } catch (_e) { void _e; }
  };
  ...
  useEffect(() => {
    ...
    if (Notification.permission === "default") {
      const t = setTimeout(() => Notification.requestPermission().then(p => { sNotifPerm(p); if (p === "granted" && !pushSubscribed) subscribeToPush(); }), 3000);
      return () => clearTimeout(t);
    }
    sNotifPerm(Notification.permission);
    if (Notification.permission === "granted" && !pushSubscribed) subscribeToPush();
  // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [loaded]);
    ...
    reminders.forEach((r, i) => {
      setTimeout(() => {
        ...
        if (Notification.permission === "granted" && navigator.serviceWorker?.controller) {
          navigator.serviceWorker.ready.then(reg => reg.showNotification("NOMAD — " + (r.type === "warn" ? "Bill Due" : "Upcoming Bill"), { body: r.msg, icon: "/icon-192.png", badge: "/icon-192.png", tag: r.id, requireInteraction: r.type === "warn" })).catch(() => {});
        }
      }, i * 700);
    });
  }, [loaded, rec, sp]);
  ...
  const addI = data => { const amt = roundMoney(data.amount); if (data.walletId === "upi_lite") { showT("UPI Lite is for spending only", "error"); return false } if (amt <= 0) { showT("Enter a valid amount", "error"); return false } if (typeof data.note === "string" && data.note.length > 500) data = { ...data, note: data.note.slice(0, 500) }; const rec = { id: uid(), type: "income", ...data, amount: amt, created_at: new Date().toISOString() }; sInc(p => [rec, ...p]); sbUpsert("incomes", [toSB(rec, ["id", "amount", "sourceId", "walletId", "note", "date", "receipt_url"])]); dance(); showT(online ? "Income added" : "Income saved offline", "success"); return true };
  const addT = data => { const b = roundMoney(wBal[data.fromWallet] || 0), amt = roundMoney(data.amount); if (amt <= 0) { showT("Enter an amount above zero", "error"); return false } if (b < amt) { showT(`Insufficient balance`, "error"); return false } if (typeof data.note === "string" && data.note.length > 500) data = { ...data, note: data.note.slice(0, 500) }; const rec = { id: uid(), type: "transfer", ...data, amount: amt, created_at: new Date().toISOString() }; sTr(p => [rec, ...p]); sbUpsert("transfers", [toSB(rec, ["id", "amount", "fromWallet", "toWallet", "note", "date"])]); dance(); showT(online ? "Transfer done" : "Transfer queued offline", "success"); return true };
  ...
  const settle = (sid, wid) => {
    ...
    const rec = { id: uid(), type: "settlement", splitName: s.name, splitId: s.id, amount: s.amount, direction: s.direction, walletId: wid, date: today, createdAt: new Date().toISOString(), ...(s.groupId && { groupId: s.groupId }), ...(s.eventId && { eventId: s.eventId }) };
    sStl(p => [...p, rec]);
  ...
  const impCsv = (file) => { const r = new FileReader(); r.onload = e => { const rows = parseBankCsv(e.target.result); if (rows.length === 0) { showT("No valid rows found — check CSV format", "error"); return; } sCsvPreview(rows); showT(`Parsed ${rows.length} rows — review and confirm import`, "info"); }; r.readAsText(file); };
...
`}</style>
    ...
    {module === "finance" && <div style={{ padding: "0 16px" }}>
      ...
      {tab === "dashboard" && <div className="pe">
        ...
        <Splits splits={sp} settlements={stl} expanded={spX} onToggle={() => sSpX(!spX)} onAdd={s => { const sr = { ...s, createdAt: new Date().toISOString() }; sSp(p => [...p, sr]); sbUpsert("splits", [toSB(sr, ["id", "name", "amount", "direction", "settled", "eventId", "groupId", "note"])]) }} onSettle={settle} onDelete={id => { sSp(p => p.filter(s => s.id !== id)); sbDeleteWhere("splits", `id=eq.${id}`) }} />
...
{historyItems.map(it => { const tlBal = hTimeline ? timelineData[it.id] : null; const tlWallets = tlBal ? (it.type === "transfer" ? [{ id: it.fromWallet }, { id: it.toWallet }] : [{ id: it.type === "expense" ? (it.walletId || "upi_lite") : it.type === "income" ? (it.walletId || "bank") : it.walletId }]) : []; return <div key={it.id} style={{ position: "relative", ...(hTimeline ? { paddingLeft: 28, marginBottom: 4 } : {}) }}>{hTimeline && <div style={{ position: "absolute", left: 10, top: 0, bottom: 0, width: 2, background: "var(--border)" }} />}{hTimeline && <div style={{ position: "absolute", left: 5, top: 22, width: 12, height: 12, borderRadius: "50%", background: it.type === "expense" ? "#E07A5F" : it.type === "income" ? "#6BAA75" : "#7B8CDE", border: "2px solid var(--bg)", zIndex: 1 }} />}<div style={{ display: "flex", alignItems: "stretch", gap: 6 }}>{bulkMode && <div onClick={() => sBulkSel(p => { const n = new Set(p); n.has(it.id) ? n.delete(it.id) : n.add(it.id); return n; })} style={{ display: "flex", alignItems: "center", padding: "0 4px", cursor: "pointer", flexShrink: 0 }}><div style={{ width: 20, height: 20, borderRadius: 5, border: `2px solid ${bulkSel.has(it.id) ? "#E07A5F" : "var(--border)"}`, background: bulkSel.has(it.id) ? "#E07A5F" : "var(--card)", display: "flex", alignItems: "center", justifyContent: "center" }}>{bulkSel.has(it.id) && <span style={{ color: "#fff", fontSize: 11, fontWeight: 700 }}>✓</span>}</div></div>}<div style={{ flex: 1, minWidth: 0 }}><TxCard item={it} categories={cats} incomeSources={isrc} events={evs} onDelete={delItem} recurringCats={recCats} wallets={wallets} onRefund={refundItem} />{tlBal && <div style={{ display: "flex", gap: 6, flexWrap: "wrap", padding: "0 4px 10px 4px", marginTop: -4 }}>{tlWallets.map(tw => { const wName = wallets.find(w => w.id === tw.id)?.name || tw.id; const bef = tlBal.before[tw.id] ?? 0; const aft = tlBal.after[tw.id] ?? 0; return <div key={tw.id} style={{ fontSize: 10, fontFamily: "var(--font-h)", color: "var(--muted)", background: "var(--bg)", borderRadius: 6, padding: "3px 8px", display: "inline-flex", alignItems: "center", gap: 5, border: "1px solid var(--border)" }}><span style={{ fontWeight: 600, color: "var(--ts)", fontSize: 9 }}>{wName}</span><span>{fmt(bef)}</span><span style={{ opacity: 0.4, fontSize: 9 }}>→</span><span style={{ fontWeight: 700, color: bef > aft ? "#E07A5F" : bef < aft ? "#6BAA75" : "var(--muted)" }}>{fmt(aft)}</span></div>; })}</div>}</div></div></div>; })}{historyItems.length === 0 && <div style={{ textAlign: "center", padding: "60px 20px", color: "var(--muted)", fontSize: 14, lineHeight: 1.8 }}>{flt.expenses.length === 0 && flt.incomes.length === 0 ? <><div style={{ fontSize: 32, marginBottom: 12 }}>📋</div><div style={{ fontFamily: "var(--font-h)", fontSize: 15, fontWeight: 600, color: "var(--ts)", marginBottom: 6 }}>No transactions yet</div><div style={{ fontSize: 12, marginBottom: 20 }}>Log expenses, income, and transfers<br />to see your spending history here.</div><button onClick={() => sTab("add")} style={{ padding: "12px 28px", border: "none", borderRadius: 12, background: "#E07A5F", color: "#fff", fontFamily: "var(--font-h)", fontSize: 13, fontWeight: 700, cursor: "pointer" }}>+ Add First Transaction</button></> : "No results match your filters."}</div>}</div>}
      ...
      {tab === "settings" && <div className="pe" style={{ paddingTop: 8 }}>
        ...
        {"Notification" in window && <div style={{ ...cc, padding: "14px 20px", marginBottom: 14 }}><div style={{ fontFamily: "var(--font-h)", fontSize: 12, color: "var(--muted)", marginBottom: 12, letterSpacing: "0.5px", fontWeight: 600 }}>Notifications</div><div style={{ display: "flex", alignItems: "center", gap: 10, padding: "10px 12px", background: "var(--bg)", borderRadius: 10, border: "1px solid var(--border)", marginBottom: 10 }}><div style={{ width: 8, height: 8, borderRadius: "50%", background: notifPerm === "granted" ? "#6BAA75" : notifPerm === "denied" ? "#D4726A" : "#FBBF24", flexShrink: 0 }} /><div style={{ flex: 1 }}><div style={{ fontSize: 12, fontFamily: "var(--font-h)", fontWeight: 600, color: "var(--text)" }}>{notifPerm === "granted" ? "Notifications enabled" : notifPerm === "denied" ? "Blocked by browser" : "Not yet enabled"}</div><div style={{ fontSize: 10, color: "var(--muted)", marginTop: 2, fontFamily: "var(--font-h)" }}>{notifPerm === "granted" ? (pushSubscribed ? "Always-on push active — reminders arrive even when app is closed" : "In-app only — tap below to enable always-on push") : notifPerm === "denied" ? "Allow in browser site settings to enable" : "Enable to get bill reminders even when app is in background"}</div></div></div>{notifPerm === "default" && <button onClick={() => Notification.requestPermission().then(p => { sNotifPerm(p); if (p === "granted") subscribeToPush(); })} style={{ width: "100%", padding: "10px", border: "none", borderRadius: 10, background: "#7B8CDE", color: "#fff", fontFamily: "var(--font-h)", fontSize: 12, fontWeight: 600, cursor: "pointer", marginBottom: 8 }}>Enable Notifications</button>}{notifPerm === "granted" && !pushSubscribed && SB_ENABLED && <button onClick={subscribeToPush} style={{ width: "100%", padding: "10px", border: "none", borderRadius: 10, background: "#6BAA75", color: "#fff", fontFamily: "var(--font-h)", fontSize: 12, fontWeight: 600, cursor: "pointer", marginBottom: 8 }}>Enable Always-On Push</button>}{notifPerm === "granted" && <button onClick={() => { navigator.serviceWorker?.ready.then(reg => { reg.showNotification("NOMAD", { body: "Notifications are working!", icon: "/icon-192.png" }); showT("Test notification sent", "success"); }).catch(() => showT("Could not send notification — make sure app is installed as PWA", "error")); }} style={{ width: "100%", padding: "10px", border: "1.5px solid var(--border)", borderRadius: 10, background: "var(--card)", color: "var(--muted)", fontFamily: "var(--font-h)", fontSize: 12, fontWeight: 600, cursor: "pointer" }}>Send test notification</button>}</div>}
