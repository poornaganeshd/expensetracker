...
export default function Nomad() {
  ...
  const [aiSubs, sAiSubs] = useState(() => { try { const s = localStorage.getItem("nomad-ai-subscriptions"); if (!s) return null; const p = JSON.parse(s); if (Date.now() - p.ts > 86400000) return null; return p; } catch { return null; } });
  const [aiSubsLoading, sAiSubsLoading] = useState(false);
  const [subsOpen, sSubsOpen] = useState(false);
...
`}</style>
    ...
    {module === "finance" && <div style={{ padding: "0 16px" }}>
      ...
      {tab === "dashboard" && <div className="pe">
        ...
        {(() => { const accent = "#2DD4BF"; const detect = async () => { sAiSubsLoading(true); try { const cutoff = new Date(); cutoff.setDate(cutoff.getDate() - 180); const c = localDateKey(cutoff); const recent = ex.filter(e => String(e.date || "") >= c); if (recent.length < 3) { showT("Need at least 3 expenses in the last 180 days to detect subscriptions", "info"); sAiSubsLoading(false); return; } const body = { mode: "subscriptions", transactions: redactTransactions(recent) }; const r = await fetch("/api/ai-analyze", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify(body) }); const data = await r.json(); if (!r.ok) throw new Error(data.error || "Subscription scan failed"); const result = { subscriptions: data.subscriptions || [], ts: Date.now() }; sAiSubs(result); try { localStorage.setItem("nomad-ai-subscriptions", JSON.stringify(result)); } catch { /* quota */ } } catch (e) { showT(e.message || "Subscription scan unavailable", "error"); } finally { sAiSubsLoading(false); } }; const list = (aiSubs && aiSubs.subscriptions) || []; const monthly = list.reduce((s, x) => s + (x.cadence === "weekly" ? (x.amount || 0) * 4.33 : x.cadence === "yearly" ? (x.amount || 0) / 12 : (x.amount || 0)), 0); return <div style={{ ...cc, padding: "12px 14px", marginBottom: 14, borderLeft: `3px solid ${accent}` }}><div onClick={() => sSubsOpen(o => !o)} style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: subsOpen ? 10 : 0, cursor: "pointer" }}><div style={{ display: "flex", alignItems: "center", gap: 8, flex: 1, minWidth: 0 }}><Receipt size={14} weight="fill" color={accent} /><div style={{ fontFamily: "var(--font-h)", fontSize: 12, color: accent, fontWeight: 700, letterSpacing: "0.5px" }}>Subscriptions</div>{list.length > 0 && <span style={{ fontSize: 9, color: "var(--muted)", fontFamily: "var(--font-h)", whiteSpace: "nowrap" }}>{list.length} found · ~₹{roundMoney(monthly)}/mo</span>}</div><span style={{ fontSize: 10, color: "var(--muted)", transition: "transform 0.2s", display: "inline-block", transform: subsOpen ? "rotate(0deg)" : "rotate(-90deg)" }}>▾</span></div>{subsOpen && <><button onClick={detect} disabled={aiSubsLoading} style={{ width: "100%", padding: "9px", borderRadius: 9, border: `1.5px solid ${accent}`, background: aiSubsLoading ? "var(--border)" : accent + "15", color: accent, fontFamily: "var(--font-h)", fontSize: 12, fontWeight: 700, cursor: aiSubsLoading ? "default" : "pointer", marginBottom: list.length ? 10 : 0 }}>{aiSubsLoading ? "Scanning…" : aiSubs ? "Rescan subscriptions" : "Detect subscriptions"}</button>{list.length > 0 ? <div style={{ display: "flex", flexDirection: "column", gap: 6 }}>{list.map((s, i) => <div key={i} style={{ padding: "8px 11px", background: "var(--bg)", borderRadius: 8, borderLeft: `2px solid ${accent}` }}><div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", gap: 8 }}><div style={{ fontFamily: "var(--font-h)", fontSize: 12.5, fontWeight: 700, color: "var(--text)", overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{s.merchant}</div><div style={{ fontSize: 12.5, fontWeight: 800, color: accent, whiteSpace: "nowrap" }}>₹{s.amount}<span style={{ fontSize: 9, color: "var(--muted)", fontWeight: 600 }}>/{s.cadence === "weekly" ? "wk" : s.cadence === "yearly" ? "yr" : s.cadence === "monthly" ? "mo" : "?"}</span></div></div>{s.note && <div style={{ fontSize: 10.5, color: "var(--ts)", marginTop: 3, lineHeight: 1.4 }}>{s.note}</div>}<div style={{ fontSize: 8.5, color: "var(--muted)", marginTop: 3, textTransform: "uppercase", letterSpacing: "0.5px" }}>{s.confidence} confidence{s.lastDate ? ` · last ${s.lastDate}` : ""}</div></div>)}</div> : aiSubs ? <div style={{ fontSize: 11, color: "var(--muted)", textAlign: "center", padding: "4px 0" }}>No recurring subscriptions detected in the last 180 days.</div> : null}</>}</div>; })()}
