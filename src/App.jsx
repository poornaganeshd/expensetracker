...
function Splits({ splits: sp, settlements: stl, onAdd, onSettle: os, onDelete: od, onSkip: ok, expanded: exp, onToggle: ot, wallets: wl = WALLETS, walletBalances: wBalances = {}, onError = () => {} }) {
...
export default function Nomad() {
  ...
  const addT = data => { const b = roundMoney(wBal[data.fromWallet] || 0), amt = roundMoney(data.amount); if (amt <= 0) { showT("Enter an amount above zero", "error"); return false } if (amt > 10000000) { showT("Amount too large (max ₹1 crore)", "error"); return false } if (!data.fromWallet || !data.toWallet) { showT("Pick a source and a destination wallet", "error"); return false } if (data.fromWallet === data.toWallet) { showT("Source and destination must be different wallets", "error"); return false } if (b < amt) { showT(`Insufficient balance`, "error"); return false } if (typeof data.note === "string" && data.note.length > 500) data = { ...data, note: data.note.slice(0, 500) }; const fromBalBefore = b; const toBalBefore = roundMoney(wBal[data.toWallet] || 0); const rec = { id: uid(), type: "transfer", ...data, amount: amt, fromBalBefore, toBalBefore, created_at: new Date().toISOString() }; sTr(p => [rec, ...p]); sbUpsert("transfers", [toSB(rec, COLS.transfers)]); dance(); showT(online ? "Transfer done" : "Transfer queued offline", "success"); return true };
  ...
  const settle = (sid, wid, payAmt) => {
    ...
    if (s.direction === "owed" && wid === "upi_lite") { showT("UPI Lite cannot receive money", "error"); return false }
