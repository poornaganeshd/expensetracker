...
const LOCAL_DEL_KEY = "nomad-local-deleted-v1";
const saveLocalDeleted = (table, item) => { try { const buf = JSON.parse(localStorage.getItem(LOCAL_DEL_KEY) || "{}"); buf[item.id] = { ...item, _tbl: table, _local: true, deleted_at: new Date().toISOString() }; localStorage.setItem(LOCAL_DEL_KEY, JSON.stringify(buf)); } catch { /* quota */ } };
const getLocalDeleted = () => { try { const buf = JSON.parse(localStorage.getItem(LOCAL_DEL_KEY) || "{}"); const cutoff = new Date(Date.now() - 30 * 864e5).toISOString(); return Object.values(buf).filter(i => i.deleted_at >= cutoff); } catch { return []; } };
const clearLocalDeleted = (id) => { try { const buf = JSON.parse(localStorage.getItem(LOCAL_DEL_KEY) || "{}"); delete buf[id]; localStorage.setItem(LOCAL_DEL_KEY, JSON.stringify(buf)); } catch { /* quota */ } };
...
function Splits({ splits: sp, settlements: stl, onAdd, onSettle: os, onDelete: od, onSkip: ok, expanded: exp, onToggle: ot, wallets: wl = WALLETS }) {
  const [nm, sN] = useState(""), [am, sA] = useState(""), [dir, sD] = useState("owe"), [snote, sSnote] = useState(""), [st, sT] = useState(null), [drill, sDrill] = useState(null), [showFeed, sSF] = useState(false), [delConfirm, sDelConfirm] = useState(null), [settleAllWid, sSAW] = useState(null), [openCards, sOC] = useState(new Set());
...
function AddPage({ categories: cats, incomeSources: isrc, recurringCats: rCats, onAddExpense: oE, onAddIncome: oI, onAddTransfer: oT, onAddRec: oR, onError: showT = () => {}, patterns = [], autoRules = [], onLearnRule = () => {}, wallets: aw = WALLETS }) {
  ...
  return <div style={{ padding: "0 0 20px" }}>
    ...
    {type === "recurring" && <><label style={ls}>Name</label><input value={rName} onChange={e => sRN(e.target.value)} placeholder="e.g. Netflix, Rent…" style={{ ...is, marginBottom: 12 }} /><label style={ls}>Amount ({CUR})</label><input type="number" value={rAmt} onChange={e => sRA(e.target.value)} placeholder="0" style={{ ...is, fontSize: 24, fontWeight: 700, fontFamily: "var(--font-h)", textAlign: "center", padding: "14px", color: "#A78BFA", borderColor: "#A78BFA", marginBottom: 12 }} /><label style={ls}>Category</label><div style={{ display: "flex", flexWrap: "wrap", gap: 6, marginBottom: 12 }}>{(rCats || RC).map(c => <button key={c.id} onClick={() => sRC(c.id)} style={{ padding: "6px 12px", borderRadius: 8, fontSize: 12, fontFamily: "var(--font-b)", border: `1.5px solid ${rCat === c.id ? c.color : "var(--border)"}`, background: rCat === c.id ? c.color + "18" : "var(--card)", color: rCat === c.id ? c.color : "var(--ts)", cursor: "pointer", display: "flex", alignItems: "center", gap: 4 }}><DI2 id={c.id} accent={c.neon || c.color} size={13} />{c.name}</button>)}</div>
      ...
      <label style={ls}>Wallet</label><WB wallets={aw.filter(w => w.id !== "upi_lite")} sel={rWal} onSel={sRW} /><label style={ls}>Frequency</label><div style={{ display: "flex", gap: 6, marginBottom: 12, flexWrap: "wrap" }}>{[{ id: "monthly", label: "Monthly" }, { id: "yearly", label: "Yearly" }, { id: "custom", label: "Every X Days" }].map(f => <button key={f.id} onClick={() => sRF(f.id)} style={{ flex: 1, padding: "9px", borderRadius: 9, fontSize: 12, fontFamily: "var(--font-h)", border: `1.5px solid ${rFreq === f.id ? "#A78BFA" : "var(--border)"}`, background: rFreq === f.id ? "#A78BFA18" : "var(--card)", color: rFreq === f.id ? "#A78BFA" : "var(--muted)", cursor: "pointer", fontWeight: 600, whiteSpace: "nowrap" }}>{f.label}</button>)}</div>
        ...
        <div style={{ overflowY: "auto", padding: "18px 22px", flex: 1, display: "flex", flexDirection: "column", gap: 14 }}>
          ...
          <div>
            ...
            <div style={{ display: "flex", gap: 8 }}>
              {wl.filter(w => w.id !== "upi_lite").map(w => (
...
export default function Nomad() {
  ...
  const addI = data => { const amt = roundMoney(data.amount); if (data.walletId === "upi_lite") { showT("UPI Lite is for spending only", "error"); return false } if (amt <= 0) { showT("Enter a valid amount", "error"); return false } if (amt > 10000000) { showT("Amount too large (max ₹1 crore)", "error"); return false } if (typeof data.note === "string" && data.note.length > 500) data = { ...data, note: data.note.slice(0, 500) }; const rec = { id: uid(), type: "income", ...data, amount: amt, created_at: new Date().toISOString() }; sInc(p => [rec, ...p]); sbUpsert("incomes", [toSB(rec, ["id", "amount", "sourceId", "walletId", "note", "date", "receipt_url"])]); dance(); showT(online ? "Income added" : "Income saved offline", "success"); return true };
  const addT = data => { const b = roundMoney(wBal[data.fromWallet] || 0), amt = roundMoney(data.amount); if (amt <= 0) { showT("Enter an amount above zero", "error"); return false } if (amt > 10000000) { showT("Amount too large (max ₹1 crore)", "error"); return false } if (b < amt) { showT(`Insufficient balance`, "error"); return false } if (typeof data.note === "string" && data.note.length > 500) data = { ...data, note: data.note.slice(0, 500) }; const rec = { id: uid(), type: "transfer", ...data, amount: amt, created_at: new Date().toISOString() }; sTr(p => [rec, ...p]); sbUpsert("transfers", [toSB(rec, ["id", "amount", "fromWallet", "toWallet", "note", "date"])]); dance(); showT(online ? "Transfer done" : "Transfer queued offline", "success"); return true };
  ...
  const loadRecentlyDeleted = async () => { sRecDelLoading(true); const [dEx, dInc, dTr, dRec, dEvs, dSp] = await Promise.all([sbGetDeleted("expenses"), sbGetDeleted("incomes"), sbGetDeleted("transfers"), sbGetDeleted("recurring"), sbGetDeleted("events"), sbGetDeleted("splits")]); const fromDB = [...(dEx || []).map(i => ({ ...i, _tbl: "expenses" })), ...(dInc || []).map(i => ({ ...i, _tbl: "incomes" })), ...(dTr || []).map(i => ({ ...i, _tbl: "transfers" })), ...(dRec || []).map(i => ({ ...i, _tbl: "recurring" })), ...(dEvs || []).map(i => ({ ...i, _tbl: "events" })), ...(dSp || []).map(i => ({ ...i, _tbl: "splits" }))]; const seen = new Set(fromDB.map(i => i.id)); const local = getLocalDeleted().filter(i => !seen.has(i.id)); const all = [...fromDB, ...local].sort((a, b) => (b.deleted_at || "").localeCompare(a.deleted_at || "")); sRecDelItems(all); sRecDelLoading(false); };
  const restoreDeleted = (item) => { const { _tbl, _local, ...row } = item; if (_local) { const splitCols = ["id", "name", "amount", "direction", "settled", "eventId", "groupId", "note"]; sbUpsert(_tbl, [toSB(row, splitCols)]); clearLocalDeleted(row.id); } else { sbWrite(`${SB_URL}/rest/v1/${_tbl}?id=eq.${row.id}`, { method: "PATCH", body: { deleted_at: null } }); } const clean = { ...row, deleted_at: null }; if (_tbl === "expenses") sEx(p => [...p, clean]); else if (_tbl === "incomes") sInc(p => [...p, clean]); else if (_tbl === "transfers") sTr(p => [...p, clean]); else if (_tbl === "recurring") sRec(p => [...p, clean]); else if (_tbl === "events") sEvs(p => [...p, clean]); else if (_tbl === "splits") sSp(p => [...p, clean]); sRecDelItems(p => p ? p.filter(i => i.id !== row.id) : p); showT("Restored", "success"); };
...
`}</style>
    ...
    {module === "finance" && <div style={{ padding: "0 16px" }}>
      ...
      {tab === "events" && <div className="pse" style={{ background: "transparent", padding: 0 }}><Events events={evs} expenses={ex} splits={sp} settlements={stl} categories={cats} onCreate={ev => { sEvs(p => [...p, ev]); sbUpsert("events", [toSB(ev, ["id", "name", "emoji", "date", "status", "type", "participants"])]) }} onAddExp={addE} onAddSplit={s => { sSp(p => [...p, s]); sbUpsert("splits", [toSB(s, ["id", "name", "amount", "direction", "settled", "eventId", "groupId", "note"])]) }} onSettleSplit={settle} onDeleteSplit={id => { const item = sp.find(s => s.id === id); sSp(p => p.filter(s => s.id !== id)); sbDelete("splits", id); if (item) saveLocalDeleted("splits", item); }} onMarkDone={id => { sEvs(p => p.map(e => e.id === id ? { ...e, status: "completed" } : e)); sbUpsert("events", [{ id, status: "completed" }]) }} onDelete={id => { const ev = evs.find(e => e.id === id); if (!ev) return; sEvs(p => p.filter(e => e.id !== id)); sbDelete("events", id); showUndoToast(ev.name + " deleted", { type: "event", exp: ev }); }} dm={dm} /></div>}
...
{module === "finance" && <div style={{ position: "fixed", bottom: 0, left: 0, right: 0, background: "var(--nav-bg)", backdropFilter: "blur(24px)", WebkitBackdropFilter: "blur(24px)", borderTop: "1px solid var(--border)", display: "flex", justifyContent: "center", maxWidth: 430, margin: "0 auto", zIndex: 50, paddingBottom: "env(safe-area-inset-bottom)" }}>{[{ id: "dashboard", label: "Home" }, { id: "add", label: "Add" }, { id: "events", label: "Events" }, { id: "history", label: "History" }, { id: "settings", label: "Settings" }].map(n => <button key={n.id} onClick={() => sTab(n.id)} style={{ flex: 1, padding: "10px 0 8px", border: "none", background: "none", display: "flex", flexDirection: "column", alignItems: "center", gap: 3, cursor: "pointer", opacity: tab === n.id ? 1 : 0.45 }}><div style={{ position: "relative" }}><NI type={n.id} active={tab === n.id} />{n.id === "settings" && deadLetterCount > 0 && <div style={{ position: "absolute", top: -2, right: -2, width: 8, height: 8, borderRadius: "50%", background: "#D4726A", border: "2px solid var(--nav-bg)" }} />}</div><span style={{ fontFamily: "var(--font-h)", fontSize: 9, color: tab === n.id ? "#E07A5F" : "var(--muted)", fontWeight: tab === n.id ? 600 : 400 }}>{n.label}</span></button>)}</div>}
    ...
    {recEditId && (() => {
      ...
      if (!r) return null;
      return <RecEditPanel r={r} recCats={recCats} wallets={wallets} onSave={patch => { const updated = rec.map(x => x.id === r.id ? { ...x, ...patch } : x); sRec(updated); sbUpsert("recurring", [toSB(updated.find(x => x.id === r.id), ["id", "name", "amount", "categoryId", "categoryName", "walletId", "frequency", "dayOfMonth", "intervalDays", "yearMonth", "yearDay", "startDate", "active", "lastPaidDate", "lastSkippedDate"])], null, getVersion("recurring", r.id) ? { "If-Unmodified-Since": getVersion("recurring", r.id) } : {}); sRecEditId(null); showT((patch.name || r.name) + " updated", "success"); }} onClose={() => sRecEditId(null)} />;
    })()}
