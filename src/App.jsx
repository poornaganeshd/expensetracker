...
function Splits({ splits: sp, settlements: stl, categories: cats = [], onAdd, onSettle: os, onSettleNet: osn = () => {}, onDelete: od, onSkip: ok, expanded: exp, onToggle: ot, wallets: wl = WALLETS, walletBalances: wBalances = {}, onError = () => {} }) {
  const [nm, sN] = useState(""), [am, sA] = useState(""), [dir, sD] = useState("owe"), [snote, sSnote] = useState(""), [scat, sScat] = useState(""), [st, sT] = useState(null), [drill, sDrill] = useState(null), [showFeed, sSF] = useState(false), [delConfirm, sDelConfirm] = useState(null), [settleAllWid, sSAW] = useState(null), [settleNetWid, sSNW] = useState(null), [settleNetPerson, sSNP] = useState(null), [openCards, sOC] = useState(new Set());
  const effScat = scat || cats[0]?.id || "";
  // Settle panels (net / all) are single component-level state shared by every
  // person. Reset them on every drill in/out so a half-opened panel can't leak
  // onto the next person with a carried-over wallet selection. Always navigate
  // drills through goDrill (never raw sDrill) so this stays guaranteed.
  ...
  const people = Object.keys(personMap); const isPersonResolved = n => personMap[n].splits.every(s => s.settled || s.skipped); const activePeople = people.filter(n => !isPersonResolved(n)).sort((a,b) => Math.abs(personMap[b].net) - Math.abs(personMap[a].net)); const settledPeople = people.filter(isPersonResolved);
  ...
  // "Settle net" appears only when a person sits on BOTH sides (you owe them AND
  // they owe you) — that's the case where one net payment clears everything and
  // is otherwise impossible to log. Same-direction stacks already use "Settle all".
  const nonUpiWallets = wl.filter(w=>!isUpiLite(w));
  const defaultNetWid = (nonUpiWallets[0]||wl[0])?.id;
  const mixedFor = name => { const pend=(personMap[name]?.splits||[]).filter(s=>!s.settled); return pend.some(s=>s.direction==="owe"&&remainForSplit(s)>0.005) && pend.some(s=>s.direction==="owed"&&remainForSplit(s)>0.005); };
  const netBtn = name => (mixedFor(name) && settleNetPerson!==name) ? <button onClick={e=>{e.stopPropagation();sSNP(name);sSNW(defaultNetWid);sSAW(null);}} style={{padding:"6px 10px",border:"1.5px solid #7B8CDE",borderRadius:8,background:"#7B8CDE12",color:"#7B8CDE",fontFamily:"var(--font-h)",fontSize:11,fontWeight:700,cursor:"pointer",flexShrink:0,display:"flex",alignItems:"center",gap:4}}><ArrowsLeftRight size={12} weight="bold"/>Settle net</button> : null;
  const netPanel = name => { if(settleNetPerson!==name) return null; const pmx=personMap[name]||{net:0,splits:[]}; const netx=pmx.net; const pendN=pmx.splits.filter(s=>!s.settled); return <div onClick={e=>e.stopPropagation()} style={{background:"var(--bg)",borderRadius:12,padding:14,marginBottom:12,border:"1.5px solid #7B8CDE"}}><div style={{fontFamily:"var(--font-h)",fontSize:12,fontWeight:700,color:"var(--text)",marginBottom:4}}>{Math.abs(netx)<0.005?`Settle up with ${name}`:netx<0?`Pay net ${fmt(-netx)} to ${name}`:`Collect net ${fmt(netx)} from ${name}`}</div><div style={{fontSize:10.5,color:"var(--muted)",fontFamily:"var(--font-b)",marginBottom:10,lineHeight:1.4}}>Clears all {pendN.length} IOUs at once — the owe/owed amounts cancel out and only the net {netx<0?"leaves":"enters"} this wallet.</div><div style={{fontSize:9,fontFamily:"var(--font-h)",color:"var(--muted)",fontWeight:600,marginBottom:6,letterSpacing:"0.5px"}}>{netx<0?"PAY FROM":"RECEIVE INTO"}</div><div style={{display:"flex",gap:6,marginBottom:10}}>{nonUpiWallets.map(w=><button key={w.id} onClick={()=>sSNW(w.id)} style={{flex:1,padding:"8px 4px",borderRadius:8,display:"flex",flexDirection:"column",alignItems:"center",gap:3,border:`2px solid ${settleNetWid===w.id?w.color:"var(--border)"}`,background:settleNetWid===w.id?w.color+"15":"var(--card)",cursor:"pointer"}}><DI2 id={w.id} accent={w.neon||w.color} size={16}/><span style={{fontSize:8,fontFamily:"var(--font-h)",fontWeight:settleNetWid===w.id?700:500,color:settleNetWid===w.id?w.color:"var(--muted)"}}>{w.name}</span></button>)}</div><div style={{display:"flex",gap:8}}><button onClick={()=>{sSNP(null);sSNW(null);}} style={{flex:1,padding:"9px",border:"1.5px solid var(--border)",borderRadius:9,background:"transparent",color:"var(--muted)",fontFamily:"var(--font-h)",fontSize:12,cursor:"pointer"}}>Cancel</button><button onClick={ev=>{if(ev.currentTarget.disabled)return;ev.currentTarget.disabled=true;const ok=osn(name,settleNetWid);if(ok!==false){sSNP(null);sSNW(null);}else{ev.currentTarget.disabled=false;}}} style={{flex:2,padding:"9px",border:"none",borderRadius:9,background:"#7B8CDE",color:"#fff",fontFamily:"var(--font-h)",fontSize:12,fontWeight:700,cursor:"pointer",display:"flex",alignItems:"center",justifyContent:"center",gap:5}}><IconCheck size={14}/>{Math.abs(netx)<0.005?"Confirm settled":netx<0?`Pay ${fmt(-netx)}`:`Collect ${fmt(netx)}`}</button></div></div>; };
...
function AddPage({ categories: cats, incomeSources: isrc, recurringCats: rCats, onAddExpense: oE, onAddIncome: oI, onAddTransfer: oT, onAddRec: oR, onError: showT = () => {}, patterns = [], autoRules = [], onLearnRule = () => {}, wallets: aw = WALLETS, cloudinaryEnabled = false }) {
  ...
  const confirmItemsImport = () => {
    ...
    itemsPreview.items.forEach(it => {
      ...
      const ok = oE({ id: uid(), amount, categoryId: cid, walletId: wid, date, note: itemNote }) !== false;
      if (ok) added++;
    });
    ...
    {type !== "recurring" && (() => {
      ...
      return <>
        {isExp && patterns.length > 0 && <div style={{ marginBottom: 14 }}>
          ...
          <div className="nm-hscroll" style={{ display: "flex", gap: 8, overflowX: "auto", paddingBottom: 2 }}>{patterns.slice(0, 8).map((p, i) => { const cat = cats.find(c => c.id === p.categoryId) || DC.find(c => c.id === p.categoryId) || { color: "#E07A5F", neon: "#FF9F1C" }; return <button key={i} onClick={() => { sAmt(String(p.amount)); sCat(p.categoryId); sW(p.walletId); if (p.note) sNote(p.note); }} style={{ flexShrink: 0, display: "flex", alignItems: "center", gap: 7, padding: "9px 13px 9px 10px", borderRadius: 14, border: `1.5px solid ${alpha(cat.color, 0.45)}`, background: alpha(cat.color, 0.08), cursor: "pointer", fontFamily: "var(--font-h)" }}><DI2 id={p.categoryId} accent={cat.neon || cat.color} size={16} /><span style={{ display: "flex", flexDirection: "column", alignItems: "flex-start", lineHeight: 1.15 }}><span style={{ fontSize: 13, fontWeight: 800, color: cat.color }}>₹{p.amount}</span>{p.note && <span style={{ fontSize: 10, color: "var(--muted)", fontWeight: 600 }}>{p.note}</span>}</span></button>; })}</div>
        </div>}
        ...
        <div style={{ borderRadius: 24, padding: "16px 18px 18px", marginBottom: 14, background: `linear-gradient(165deg, ${alpha(tc, 0.13)}, ${alpha(tc, 0.04)})`, border: `1.5px solid ${alpha(tc, 0.28)}` }}>
          ...
          {isFXcur && <div style={{ marginTop: 8, display: "flex", alignItems: "center", justifyContent: "space-between" }}><span style={{ fontFamily: "var(--font-h)", fontSize: 10.5, color: "var(--muted)", fontWeight: 600 }}>1 {fxCur} = ₹{fxRate.toFixed(2)}</span>{parseFloat(amt) > 0 && <span style={{ fontFamily: "var(--font-h)", fontSize: 14, fontWeight: 800, color: tc }}>≈ {fmt(inrAmt)}</span>}</div>}
...
function LedgerM({ wallet: w, rows, curBal, onReconcile, lastVerifyLabel, onClose: cl }) {
  ...
  return <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.5)", zIndex: 200, display: "flex", alignItems: "flex-end", justifyContent: "center" }} onClick={cl}>
    <div onClick={e => e.stopPropagation()} style={{ background: "var(--card)", borderRadius: "20px 20px 0 0", padding: "20px 20px 24px", width: "100%", maxWidth: 430, maxHeight: "88vh", display: "flex", flexDirection: "column" }}>
      <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 4 }}><DI2 id={w.id} accent={w.neon || w.color} size={20} /><div style={{ fontFamily: "var(--font-h)", fontSize: 16, fontWeight: 700, color: "var(--text)" }}>{w.name} · Running balance</div></div>
...
function Events({ events: evs, expenses: ex, splits: sp, settlements: stl, categories: cats, wallets: wl = WALLETS, staleByEvent = {}, onCreate: oC, onAddExp: oE, onAddSplit: oS, onSettleSplit: oSS, onSettleEventNet: oSEN, onDeleteSplit: oDS, onDeleteExp: oDE, onMarkDone: oMD, onReopen: oRO, onDelete: oD, onUpdate: oU, onToast: showT = () => {}, dm = false }) {
  ...
  const [bsOpen, sBsO] = useState(false), [bsMode, sBsM] = useState("equal"), [bsTotal, sBsT] = useState(""), [bsPpl, sBsP] = useState([{ name: "", amount: "" }]), [bsCat, sBsC] = useState(cats[0]?.id || ""), [bsW, sBsW] = useState(wl[0]?.id || "bank"), [bsNote, sBsN] = useState(""), [bsStep, sBsS] = useState(1);
  const [evDelConfirm, sEvDelConfirm] = useState(null);
  const [doneConfirm, sDoneConfirm] = useState(false), [suTgt, sSuTgt] = useState(null), [suW, sSuW] = useState(null);
  ...
  const create = () => { if (!nn.trim()) return; const reserved = new Set(["you", "me"]); const parts = evType === "group" ? evParts.map(p => p.trim()).filter((p, i, arr) => p && !reserved.has(p.toLowerCase()) && arr.findIndex(x => x.toLowerCase() === p.toLowerCase()) === i) : []; oC({ id: uid(), name: nn.trim(), emoji: ne, date: localDateKey(), status: "active", type: evType, participants: parts }); sNN(""); sNE("film"); sEvType("solo"); sEvParts([""]); sV("list") };
  const addExp = () => {
    const a = parseAmount(ea); if (!Number.isFinite(a) || a <= 0 || !sel) return;
    ...
    const pb = isGrp ? (ePaidBy === "me" || partsList.some(p => p.toLowerCase() === String(ePaidBy).toLowerCase()) ? ePaidBy : "me") : undefined;
    const expId = uid();
    const ok = oE({ id: expId, amount: a, categoryId: ec, walletId: ew, note: en, date: localDateKey(), eventId: sel.id, ...(pb ? { paidBy: pb } : {}) });
    if (ok !== false) {
      if (isGrp && partsList.length > 0) {
        const allP = ["You", ...partsList];
        const shares = distributeAmount(a, allP.length);
        const payerIdx = pb === "me" ? 0 : allP.findIndex(p => p.toLowerCase() === String(pb).toLowerCase());
        const noteBase = (en || cats.find(c => c.id === ec)?.name || "Group expense").slice(0, 400);
        if (payerIdx === 0) {
          allP.forEach((person, idx) => {
            if (idx === 0) return;
            const share = shares[idx];
            if (share <= 0) return;
            oS({ id: uid(), name: person, amount: share, direction: "owed", settled: false, eventId: sel.id, groupId: expId, note: `Auto: ${noteBase}` });
          });
        } else if (payerIdx > 0) {
          const yourShare = shares[0];
          if (yourShare > 0) oS({ id: uid(), name: allP[payerIdx], amount: yourShare, direction: "owe", settled: false, eventId: sel.id, groupId: expId, note: `Auto: ${noteBase}` });
        }
      }
      sEA(""); sEN("");
    }
  };
  const addSplit = () => { const a = parseAmount(sa); if (!sn.trim() || !Number.isFinite(a) || a <= 0 || !sel) { if (sn.trim() && sa) showT("Enter a valid amount", "error"); return; } if (a > 10000000) { showT("Amount too large (max ₹1 crore)", "error"); return; } oS({ id: uid(), name: sn.trim(), amount: roundMoney(a), direction: sd, settled: false, eventId: sel.id, note: spNote }); sSN(""); sSA(""); sSPNote("") };
  ...
  const goDetail = (id) => { sSel(id); sEPaidBy("me"); sEA(""); sEN(""); sEC(cats[0]?.id || ""); sEW(wl[0]?.id || "bank"); sSN(""); sSA(""); sSPNote(""); sSD("owed"); sSTgt(null); sBsO(false); sBsS(1); sBsM("equal"); sBsT(""); sBsP([{ name: "", amount: "" }]); sBsN(""); sBsC(cats[0]?.id || ""); sBsW(wl[0]?.id || "bank"); sEvDelConfirm(null); sDoneConfirm(false); sSuTgt(null); sEditOpen(false); sV("detail"); };
  ...
  if (view === "detail" && sel) {
    ...
    const openEdit = () => { sEnName(sel.name); sEnIcon(sel.emoji || "film"); sEnParts([...(sel.participants || [])]); sEnNew(""); sEditOpen(true); };
    const saveEdit = () => { const nm2 = enName.trim(); if (!nm2) { showT("Event name can't be empty", "error"); return; } oU({ ...sel, name: nm2, emoji: enIcon, ...(isGroup ? { participants: enParts } : {}) }); sEditOpen(false); };
    ...
    const avColor = name => { const pal = ["#C4603A", "#5A7A5A", "#B8963E", "#7A6A50", "#8C5E7A", "#4E7A8C"]; let h = 0; for (const ch of name) h = (h * 31 + ch.charCodeAt(0)) & 0xffff; return pal[h % pal.length]; };
    const initialsOf = name => name.trim().split(/\s+/).slice(0, 2).map(w => w[0]?.toUpperCase() || "").join("");
    const hl = <div style={{ flex: 1, height: 1, background: "linear-gradient(to right, rgba(156,143,122,0.3), transparent)" }} />;
    return <div style={{ position: "relative", background: pC, height: "calc(100vh - 90px)", display: "flex", flexDirection: "column", overflow: "hidden" }}><TopoBg />
      <div style={{ position: "relative", zIndex: 10, padding: "max(18px, calc(env(safe-area-inset-top, 0px) + 12px)) 20px 14px", display: "flex", alignItems: "center", justifyContent: "space-between" }}><button onClick={() => sV("list")} style={{ display: "flex", alignItems: "center", gap: 4, background: "none", border: "none", color: mutC, cursor: "pointer", fontSize: 13, letterSpacing: "0.5px", padding: 0 }}><CaretLeft size={15} />Events</button><div style={{ display: "flex", gap: 7, alignItems: "center" }}><button className="evPress" onClick={shareSummary} title="Share summary" style={{ width: 30, height: 30, borderRadius: "50%", border: `1px solid ${brdC}`, background: cardC, color: mutC, display: "flex", alignItems: "center", justifyContent: "center", cursor: "pointer", padding: 0 }}><ShareNetwork size={14} /></button><button className="evPress" onClick={openEdit} title="Edit event" style={{ width: 30, height: 30, borderRadius: "50%", border: `1px solid ${brdC}`, background: cardC, color: mutC, display: "flex", alignItems: "center", justifyContent: "center", cursor: "pointer", padding: 0 }}><PencilSimple size={14} /></button>{sel.status === "active" ? <button className="evPress" onClick={() => pendingCnt > 0 ? sDoneConfirm(true) : oMD(sel.id)} style={{ display: "flex", alignItems: "center", gap: 5, padding: "7px 14px", border: "1px solid rgba(90,122,90,0.4)", borderRadius: 20, background: "rgba(90,122,90,0.1)", color: greenC, fontSize: 11, fontWeight: 600, letterSpacing: "0.5px", cursor: "pointer" }}><IconCheck size={13} />Mark Done</button> : <button className="evPress" onClick={() => oRO(sel.id)} style={{ display: "flex", alignItems: "center", gap: 5, padding: "7px 14px", border: "1px solid rgba(184,150,62,0.4)", borderRadius: 20, background: "rgba(184,150,62,0.08)", color: goldC, fontSize: 11, fontWeight: 600, letterSpacing: "0.5px", cursor: "pointer" }}><IconClock size={13} />Reopen</button>}<button className="evPress" onClick={() => sEvDelConfirm(sel.id)} title="Delete event" style={{ width: 30, height: 30, borderRadius: "50%", border: "1px solid rgba(192,82,74,0.35)", background: "rgba(192,82,74,0.08)", color: redC, display: "flex", alignItems: "center", justifyContent: "center", cursor: "pointer", padding: 0 }}><Trash size={13} /></button></div></div>
      <div style={{ position: "relative", zIndex: 10, flex: 1, overflowY: "auto", padding: "2px 20px 28px" }}>
      <div style={{ display: "flex", alignItems: "center", gap: 14, marginBottom: 16, animation: "evSlideUp 0.4s ease both" }}><div style={{ width: 54, height: 54, borderRadius: 16, background: dm ? "linear-gradient(135deg, #362510, #251a0e)" : "linear-gradient(135deg, #F5EFE4, #EDE5D4)", display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0, border: "1px solid rgba(184,150,62,0.2)", color: terraC }}><EvIcon id={sel.emoji} size={26} /></div><div style={{ flex: 1, minWidth: 0 }}><div style={{ fontFamily: "'Playfair Display', Georgia, serif", fontSize: 23, color: inkC, letterSpacing: "-0.3px", whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis" }}>{sel.name}</div><div style={{ display: "flex", alignItems: "center", gap: 6, marginTop: 4, flexWrap: "wrap" }}><span style={{ fontSize: 11, color: mutC, letterSpacing: "0.5px" }}>{fmtDate(sel.date)}</span><span style={{ display: "inline-flex", alignItems: "center", gap: 4, background: sel.status === "active" ? "rgba(184,150,62,0.1)" : "rgba(90,122,90,0.1)", color: sel.status === "active" ? goldC : greenC, fontSize: 10, fontWeight: 500, letterSpacing: "0.5px", padding: "2px 8px", borderRadius: 20, border: `1px solid ${sel.status === "active" ? "rgba(184,150,62,0.25)" : "rgba(90,122,90,0.25)"}` }}>{sel.status === "active" ? <IconClock size={10} /> : <IconCheck size={10} />}{sel.status === "active" ? "Active" : "Done"}</span>{isGroup && <span style={{ display: "inline-flex", alignItems: "center", gap: 4, background: "rgba(196,96,58,0.08)", color: terraC, fontSize: 10, fontWeight: 500, letterSpacing: "0.5px", padding: "2px 8px", borderRadius: 20, border: "1px solid rgba(196,96,58,0.25)" }}><Users size={10} />Group</span>}</div></div></div>
      <div style={{ background: stripC, borderRadius: 14, padding: "14px 6px", display: "flex", alignItems: "center", marginBottom: 20, animation: "evSlideUp 0.4s 0.05s ease both" }}>{(isGroup ? [[grpTotal, true, "Group Total"], [grpShare, true, "Your Share"], [allParts.length, false, "People"]] : [[ns, true, "Net Spent"], [tp, true, "Total Paid"], [eExps.length, false, "Entries"]]).map(([v, m, l], i) => <div key={l} style={{ flex: 1, textAlign: "center", borderLeft: i > 0 ? "1px solid rgba(255,255,255,0.1)" : "none" }}><div style={{ fontSize: 15, fontWeight: 500, color: "#C9A84C", fontFamily: "'Playfair Display', Georgia, serif" }}><AnimNum value={v} money={m} /></div><div style={{ fontSize: 9, color: "rgba(255,255,255,0.4)", letterSpacing: "0.8px", textTransform: "uppercase", marginTop: 2 }}>{l}</div></div>)}</div>
      {isGroup && allParts.length > 1 && <div style={secLbl}><span style={{ display: "inline-flex", alignItems: "center", gap: 6 }}><Users size={11} />Group Ledger</span>{hl}</div>}
      {isGroup && allParts.length > 1 && <div style={{ padding: "0 2px", marginBottom: 30, animation: "evSlideUp 0.4s 0.1s ease both" }}><div style={{ fontSize: 10.5, color: mutC, padding: "11px 0", borderBottom: `1px solid ${rowB}`, letterSpacing: "0.3px" }}>Equal share {fmt(grpShare)} per person · {allParts.length} people</div>{allParts.map(p => { const paid = grpPaid[p] || 0, bal = roundMoney(paid - (grpShareMap[p] ?? grpShare) - (grpSettled[p] || 0)); const settledP = Math.abs(bal) < 0.01; const av = avColor(p); const pi = allParts.indexOf(p); return <div key={p} style={{ display: "flex", alignItems: "center", gap: 12, padding: "11px 0", borderBottom: `1px solid ${rowB}`, animation: "evRowIn 0.35s ease both", animationDelay: `${0.12 + Math.min(pi, 8) * 0.05}s` }}><div style={{ width: 32, height: 32, borderRadius: "50%", background: av + "1F", border: `1px solid ${av}40`, color: av, display: "flex", alignItems: "center", justifyContent: "center", fontSize: 10.5, fontWeight: 700, flexShrink: 0, letterSpacing: "0.5px" }}>{initialsOf(p)}</div><div style={{ flex: 1, minWidth: 0 }}><div style={{ fontSize: 13.5, fontWeight: 500, color: inkC }}>{p}</div><div style={{ fontSize: 10.5, color: mutC, marginTop: 1 }}>paid {fmt(paid)} · share {fmt(grpShareMap[p] ?? grpShare)}</div></div><div style={{ textAlign: "right", flexShrink: 0 }}>{settledP ? <span style={{ fontSize: 11, color: mutC, display: "inline-flex", alignItems: "center", gap: 4 }}><IconCheck size={11} />settled</span> : <><div style={{ fontFamily: "'Playfair Display', Georgia, serif", fontSize: 15, color: bal > 0 ? greenC : terraC }}>{fmt(Math.abs(bal))}</div><div style={{ fontSize: 9, letterSpacing: "0.8px", textTransform: "uppercase", color: bal > 0 ? greenC : terraC, marginTop: 1 }}>{bal > 0 ? "gets back" : "owes"}</div></>}</div></div>; })}{suggested.length > 0 && <div style={{ padding: "10px 0 12px" }}><div style={{ fontSize: 9, letterSpacing: "1.5px", textTransform: "uppercase", color: goldC, fontWeight: 600, marginBottom: 8 }}>Settle Up</div>{suggested.map((g, i) => <div key={i} style={{ display: "flex", alignItems: "center", gap: 8, fontSize: 12.5, marginBottom: i < suggested.length - 1 ? 7 : 0, animation: "evRowIn 0.35s ease both", animationDelay: `${0.2 + Math.min(i, 8) * 0.06}s` }}><span style={{ fontWeight: 600, color: g.from === "You" ? terraC : inkC }}>{g.from}</span><span style={{ display: "inline-flex", animation: "evNudge 1.6s ease-in-out infinite" }}><ArrowRight size={12} color={mutC} /></span><span style={{ fontWeight: 600, color: g.to === "You" ? greenC : inkC }}>{g.to}</span><span style={{ marginLeft: "auto", fontFamily: "'Playfair Display', Georgia, serif", fontSize: 14, color: inkC }}>{fmt(g.amt)}</span>{(g.from === "You" || g.to === "You") && <button className="evPress" onClick={() => { sSuTgt(g); sSuW(wl[0]?.id || "bank"); }} style={{ border: "1px solid rgba(184,150,62,0.4)", background: "rgba(184,150,62,0.08)", borderRadius: 20, padding: "4px 11px", fontSize: 10, fontWeight: 600, letterSpacing: "0.5px", color: goldC, cursor: "pointer", flexShrink: 0 }}>Settle</button>}</div>)}</div>}</div>}
      {eSp.length > 0 && <div style={{ display: "flex", alignItems: "stretch", padding: "0 2px", marginBottom: 30, animation: "evSlideUp 0.4s 0.12s ease both" }}><div style={{ flex: 1 }}><div style={{ display: "flex", alignItems: "center", gap: 5, fontSize: 9, color: redC, letterSpacing: "1px", textTransform: "uppercase", fontWeight: 600 }}><IconArrowUp size={11} />You Owe</div><div style={{ fontFamily: "'Playfair Display', Georgia, serif", fontSize: 22, color: redC, marginTop: 4 }}><AnimNum value={tO} /></div></div><div style={{ width: 1, background: "linear-gradient(to bottom, rgba(156,143,122,0.35), transparent)", margin: "0 18px" }} /><div style={{ flex: 1 }}><div style={{ display: "flex", alignItems: "center", gap: 5, fontSize: 9, color: greenC, letterSpacing: "1px", textTransform: "uppercase", fontWeight: 600 }}><IconArrowDown size={11} />Owed To You</div><div style={{ fontFamily: "'Playfair Display', Georgia, serif", fontSize: 22, color: greenC, marginTop: 4 }}><AnimNum value={tI} /></div></div></div>}
      {eExps.length > 0 && <div style={secLbl}><span style={{ display: "inline-flex", alignItems: "center", gap: 6 }}><Receipt size={11} />Expenses · {eExps.length}</span>{hl}</div>}
      {eExps.length > 0 && <div style={{ padding: "0 2px", marginBottom: 30, animation: "evSlideUp 0.4s 0.15s ease both" }}>{[...eExps].reverse().map((e, xi, arr) => { const cat = cats.find(c => c.id === e.categoryId) || { id: "other", name: "Other", color: "#999", neon: "#999" }; return <div key={e.id} style={{ display: "flex", alignItems: "center", gap: 12, padding: "11px 0", borderBottom: xi < arr.length - 1 ? `1px solid ${rowB}` : "none", animation: "evRowIn 0.35s ease both", animationDelay: `${Math.min(xi, 8) * 0.04}s` }}><div style={{ width: 34, height: 34, borderRadius: 10, background: dm ? "rgba(54,37,16,0.6)" : "linear-gradient(135deg, #F5EFE4, #EDE5D4)", border: "1px solid rgba(184,150,62,0.15)", display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0 }}><DI2 id={cat.id} accent={cat.neon || cat.color} size={16} /></div><div style={{ flex: 1, minWidth: 0 }}><div style={{ fontSize: 13.5, fontWeight: 500, color: inkC }}>{cat.name}</div>{e.note && <div style={{ fontSize: 11, color: mutC, marginTop: 1, whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis" }}>{e.note}</div>}{isGroup && e.paidBy && e.paidBy !== "me" && <div style={{ fontSize: 10, color: goldC, marginTop: 2, fontWeight: 600, letterSpacing: "0.3px", display: "inline-flex", alignItems: "center", gap: 3 }}><Users size={10} />paid by {e.paidBy}</div>}</div><span style={{ fontFamily: "'Playfair Display', Georgia, serif", fontSize: 15, color: terraC, flexShrink: 0 }}>−{fmt(e.amount)}</span>{sel.status === "active" && <button className="evPress" onClick={() => oDE(e.id)} title="Delete expense" style={{ background: "none", border: "none", color: mutC, cursor: "pointer", opacity: 0.5, flexShrink: 0, padding: 0, display: "flex" }}><IconX size={14} /></button>}</div>; })}</div>}
      {eSp.length > 0 && <div style={secLbl}><span style={{ display: "inline-flex", alignItems: "center", gap: 6 }}><ArrowsLeftRight size={11} />Splits · {eSp.length}</span>{hl}</div>}
      {eSp.length > 0 && <div style={{ padding: "0 2px", marginBottom: 30, animation: "evSlideUp 0.4s 0.18s ease both" }}>{eSp.map((s, si) => <div key={s.id} style={{ display: "flex", alignItems: "center", gap: 12, padding: "11px 0", borderBottom: si < eSp.length - 1 ? `1px solid ${rowB}` : "none", opacity: s.settled ? 0.45 : 1, animation: "evRowIn 0.35s ease both", animationDelay: `${Math.min(si, 8) * 0.04}s` }}><div style={{ width: 34, height: 34, borderRadius: "50%", background: s.settled || s.direction !== "owe" ? "rgba(90,122,90,0.1)" : "rgba(192,82,74,0.1)", border: `1px solid ${s.settled || s.direction !== "owe" ? "rgba(90,122,90,0.25)" : "rgba(192,82,74,0.25)"}`, display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0, color: s.settled || s.direction !== "owe" ? greenC : redC }}>{s.settled ? <CheckCircle size={16} weight="fill" /> : s.direction === "owe" ? <ArrowUpRight size={15} /> : <ArrowDownLeft size={15} />}</div><div style={{ flex: 1, minWidth: 0 }}><div style={{ fontSize: 13.5, fontWeight: 500, color: inkC }}>{s.name}</div><div style={{ fontSize: 10.5, color: mutC, marginTop: 1 }}>{s.settled ? "Settled" : s.direction === "owe" ? "You owe" : "Owes you"}{!s.settled && (paidBySplit[s.id] || 0) > 0.005 && <span style={{ color: goldC, fontWeight: 600 }}> · {fmt(paidBySplit[s.id])} paid, {fmt(remainForSplit(s))} left</span>}{!s.settled && staleIds.has(s.id) && <span style={{ color: redC, fontWeight: 600 }}> · pending 2+ days</span>}</div>{s.note && <div style={{ fontSize: 10, color: mutC, marginTop: 1, fontStyle: "italic", whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis" }}>{s.note}</div>}</div><span style={{ fontFamily: "'Playfair Display', Georgia, serif", fontSize: 15, color: s.direction === "owe" ? redC : greenC, flexShrink: 0 }}>{fmt(s.amount)}</span>{!s.settled && <button className="evPress" onClick={() => sSTgt(s)} style={{ border: "1px solid rgba(184,150,62,0.4)", background: "rgba(184,150,62,0.08)", borderRadius: 20, padding: "4px 11px", fontSize: 10, fontWeight: 600, letterSpacing: "0.5px", color: goldC, cursor: "pointer", flexShrink: 0 }}>Settle</button>}<button onClick={() => oDS(s.id)} style={{ background: "none", border: "none", color: mutC, cursor: "pointer", opacity: 0.5, flexShrink: 0, padding: 0, display: "flex" }}><IconX size={14} /></button></div>)}</div>}
      {sel.status === "active" && <div style={secLbl}><span style={{ display: "inline-flex", alignItems: "center", gap: 6 }}><IconPlus size={11} />Add Expense</span>{hl}</div>}
      {sel.status === "active" && <div style={{ padding: "0 2px", marginBottom: 30 }}><div style={{ display: "flex", flexWrap: "wrap", gap: 6, marginBottom: 12 }}>{cats.map(c => <button key={c.id} onClick={() => sEC(c.id)} style={{ padding: "6px 12px", borderRadius: 20, fontSize: 12, fontFamily: "var(--font-b)", border: `1px solid ${ec === c.id ? c.color : brdC}`, background: ec === c.id ? c.color + "1A" : "transparent", color: ec === c.id ? c.color : mutC, cursor: "pointer", fontWeight: ec === c.id ? 600 : 400, display: "flex", alignItems: "center", gap: 4 }}><DI2 id={c.id} accent={c.neon || c.color} size={14} />{c.name}</button>)}</div>{isGroup && <div style={{ marginBottom: 12 }}><label style={wls}>Paid by</label><div style={{ display: "flex", flexWrap: "wrap", gap: 6 }}>{["me", ...(sel.participants || [])].map(p => { const label = p === "me" ? "You" : p; return <button key={p} onClick={() => sEPaidBy(p)} style={{ padding: "7px 14px", borderRadius: 20, fontSize: 12, border: `1px solid ${ePaidBy === p ? terraC : brdC}`, background: ePaidBy === p ? "rgba(196,96,58,0.1)" : "transparent", color: ePaidBy === p ? terraC : mutC, cursor: "pointer", fontWeight: ePaidBy === p ? 600 : 400 }}>{label}</button>; })}</div></div>}{(!isGroup || ePaidBy === "me") && <><label style={wls}>Paid From</label><div style={{ display: "flex", gap: 6, marginBottom: 12 }}>{wl.map(w => <button key={w.id} onClick={() => sEW(w.id)} style={{ flex: 1, padding: "8px", borderRadius: 10, border: `1px solid ${ew === w.id ? w.color : brdC}`, background: ew === w.id ? w.color + "15" : "transparent", fontSize: 12, fontWeight: ew === w.id ? 600 : 500, color: ew === w.id ? w.color : mutC, cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center", gap: 4 }}><DI2 id={w.id} accent={w.neon || w.color} size={12} />{w.name}</button>)}</div></>}<div style={{ display: "flex", gap: 8 }}><input type="number" value={ea} onChange={e => sEA(e.target.value)} placeholder="₹" style={{ ...wis, width: 80 }} /><input value={en} onChange={e => sEN(e.target.value)} placeholder="Note" style={{ ...wis, flex: 1, width: "auto" }} /><button className="evPress" onClick={addExp} style={{ padding: "10px 15px", border: "none", borderRadius: 10, background: terraC, color: "#fff", cursor: "pointer", display: "flex", alignItems: "center", boxShadow: "0 2px 8px rgba(196,96,58,0.3)" }}><IconPlus size={16} /></button></div></div>}
      {/* Bill Splitter is solo-event only: in group events the per-expense
          "Paid by" flow already creates the IOUs, and the GROUP LEDGER
      ...
      {sel.status === "active" && !isGroup && (() => {
        ...
        if (!bsOpen) return <button className="evPress" onClick={() => sBsO(true)} style={{ width: "100%", padding: 14, border: "1.5px dashed rgba(184,150,62,0.55)", borderRadius: 14, background: "rgba(184,150,62,0.06)", color: goldC, fontSize: 13, fontWeight: 600, letterSpacing: "0.5px", cursor: "pointer", marginBottom: 20, display: "flex", alignItems: "center", justifyContent: "center", gap: 7 }}><Receipt size={15} />Bill Splitter</button>;
        if (bsStep === 3) return <div style={{ padding: "20px 2px", marginBottom: 30, textAlign: "center" }}><div style={{ display: "flex", justifyContent: "center", marginBottom: 8, color: greenC }}><CheckCircle size={34} weight="fill" /></div><div style={{ fontSize: 14, color: greenC, fontWeight: 600 }}>Split recorded</div><div style={{ fontSize: 12, color: mutC, marginTop: 4 }}>Full bill paid. Your final share is {fmt(myShare)}.</div></div>;
        if (bsStep === 2) { const cat = cats.find(c => c.id === bsCat) || cats[0]; return <div style={{ padding: "0 2px", marginBottom: 30 }}><div style={{ ...secLbl, padding: "0 0 12px" }}><span style={{ display: "inline-flex", alignItems: "center", gap: 6 }}><Receipt size={11} />Confirm Split</span></div><div style={{ background: "rgba(196,96,58,0.07)", borderRadius: 10, padding: "12px 14px", marginBottom: 10 }}><div style={{ fontSize: 10, color: mutC, fontWeight: 600, letterSpacing: "1px", textTransform: "uppercase", marginBottom: 6 }}>Paid Now</div><div style={{ display: "flex", alignItems: "center", gap: 10 }}><DI2 id={cat?.id} accent={cat?.neon || cat?.color} size={20} /><div style={{ flex: 1, fontSize: 13, fontWeight: 600, color: inkC }}>Full bill from {wl.find(w => w.id === bsW)?.name || "wallet"}</div><div style={{ fontSize: 18, fontFamily: "'Playfair Display', Georgia, serif", color: terraC }}>−{fmt(totalNum)}</div></div></div><div style={{ background: "rgba(90,122,90,0.07)", borderRadius: 10, padding: "10px 14px", marginBottom: 14 }}><div style={{ display: "flex", justifyContent: "space-between", fontSize: 12, color: inkC }}><span>Your final share</span><span style={{ fontWeight: 700, color: terraC }}>{fmt(myShare)}</span></div></div><div style={{ marginBottom: 14 }}><div style={{ fontSize: 10, color: mutC, fontWeight: 600, letterSpacing: "1px", textTransform: "uppercase", marginBottom: 8 }}>They Owe You</div>{validPpl.map((p, i) => { const amt = bsMode === "equal" ? eqOthers[i] : roundMoney((Number.isFinite(parseAmount(p.amount)) ? parseAmount(p.amount) : 0)); return <div key={i} style={{ display: "flex", justifyContent: "space-between", padding: "7px 0", borderBottom: `1px solid ${rowB}` }}><span style={{ fontSize: 13, color: inkC }}>{p.name}</span><span style={{ fontSize: 13, fontWeight: 600, color: greenC }}>{fmt(amt)}</span></div> })}</div><div style={{ display: "flex", gap: 8 }}><button onClick={() => sBsS(1)} style={{ flex: 1, padding: 12, border: `1px solid ${brdC}`, borderRadius: 10, background: "transparent", color: mutC, fontSize: 13, cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center", gap: 4 }}><CaretLeft size={13} />Edit</button><button className="evPress" onClick={bsSubmit} style={{ flex: 2, padding: 12, border: "none", borderRadius: 10, background: greenC, color: "#fff", fontSize: 13, fontWeight: 700, cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center", gap: 5, boxShadow: "0 2px 8px rgba(90,122,90,0.3)" }}><IconCheck size={14} />Confirm</button></div></div> }
        return <div style={{ padding: "0 2px", marginBottom: 30 }}><div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 12 }}><div style={{ ...secLbl, padding: 0 }}><span style={{ display: "inline-flex", alignItems: "center", gap: 6 }}><Receipt size={11} />Bill Splitter</span></div><button onClick={bsReset} style={{ background: "none", border: "none", color: mutC, cursor: "pointer", display: "flex", padding: 0 }}><IconX size={14} /></button></div>
          <div style={{ display: "flex", gap: 6, marginBottom: 14 }}>{[{ id: "equal", label: "Equal Split" }, { id: "custom", label: "Custom Split" }].map(m => <button key={m.id} onClick={() => sBsM(m.id)} style={{ flex: 1, padding: "9px", borderRadius: 10, fontSize: 12, border: `1px solid ${bsMode === m.id ? goldC : brdC}`, background: bsMode === m.id ? "rgba(184,150,62,0.1)" : "transparent", color: bsMode === m.id ? goldC : mutC, cursor: "pointer", fontWeight: 600 }}>{m.label}</button>)}</div>
          <label style={wls}>Note (optional)</label><input value={bsNote} onChange={e => sBsN(e.target.value)} placeholder="What was this bill for?" style={{ ...wis, marginBottom: 14 }} /><label style={wls}>Total Bill (₹)</label><input type="number" value={bsTotal} onChange={e => sBsT(e.target.value)} placeholder="0" style={{ ...wis, marginBottom: 14, fontSize: 22, fontFamily: "'Playfair Display', Georgia, serif", textAlign: "center" }} />
          <label style={wls}>People (excluding you)</label>{bsPpl.map((p, i) => <div key={i} style={{ display: "flex", gap: 6, marginBottom: 8, alignItems: "center" }}><input value={p.name} onChange={e => sBsP(pp => pp.map((x, idx) => idx === i ? { ...x, name: e.target.value } : x))} placeholder="Name" style={{ ...wis, flex: 1, width: "auto" }} />{bsMode === "custom" && <input type="number" value={p.amount} onChange={e => sBsP(pp => pp.map((x, idx) => idx === i ? { ...x, amount: e.target.value } : x))} placeholder="₹" style={{ ...wis, width: 78 }} />}{bsMode === "custom" && p.name.trim() && !(Number.isFinite(parseAmount(p.amount)) && parseAmount(p.amount) > 0) && <span style={{ fontSize: 10, color: redC, flexShrink: 0, fontWeight: 600 }}>₹0!</span>}{bsPpl.length > 1 && <button onClick={() => sBsP(pp => pp.filter((_, idx) => idx !== i))} style={{ background: "none", border: "none", color: mutC, cursor: "pointer", opacity: 0.5, display: "flex", padding: 0 }}><IconX size={14} /></button>}</div>)}
          <button onClick={() => sBsP(p => [...p, { name: "", amount: "" }])} style={{ background: "none", border: `1px dashed ${brdC}`, borderRadius: 10, padding: "8px 14px", fontSize: 12, color: mutC, cursor: "pointer", marginBottom: 14, width: "100%", display: "flex", alignItems: "center", justifyContent: "center", gap: 5 }}><IconPlus size={13} />Add person</button>
          {totalNum > 0 && validPpl.length > 0 && <div style={{ background: stoneC, borderRadius: 10, padding: "10px 14px", marginBottom: 14 }}>{bsMode === "equal" ? <div style={{ display: "flex", justifyContent: "space-between", fontSize: 12, color: inkC }}><span>Per person ({hc})</span><span style={{ fontWeight: 600 }}>{fmt(equalShares[0] || 0)}</span></div> : <div style={{ display: "flex", justifyContent: "space-between", fontSize: 12, color: custOT > totalNum ? redC : inkC }}><span>Others total</span><span style={{ fontWeight: 600 }}>{fmt(custOT)} / {fmt(totalNum)}{custOT > totalNum ? " (over!)" : ""}</span></div>}<div style={{ display: "flex", justifyContent: "space-between", fontSize: 13, color: terraC, fontWeight: 700, marginTop: 6 }}><span>Your share</span><span>{fmt(myShare)}</span></div></div>}
          <label style={wls}>Category</label><div style={{ display: "flex", flexWrap: "wrap", gap: 6, marginBottom: 12 }}>{cats.map(c => <button key={c.id} onClick={() => sBsC(c.id)} style={{ padding: "6px 12px", borderRadius: 20, fontSize: 12, fontFamily: "var(--font-b)", border: `1px solid ${bsCat === c.id ? c.color : brdC}`, background: bsCat === c.id ? c.color + "1A" : "transparent", color: bsCat === c.id ? c.color : mutC, cursor: "pointer", fontWeight: bsCat === c.id ? 600 : 400, display: "flex", alignItems: "center", gap: 4 }}><DI2 id={c.id} accent={c.neon || c.color} size={14} />{c.name}</button>)}</div>
          <label style={wls}>Paid From</label><div style={{ display: "flex", gap: 6, marginBottom: 14 }}>{wl.map(w => <button key={w.id} onClick={() => sBsW(w.id)} style={{ flex: 1, padding: "8px", borderRadius: 10, border: `1px solid ${bsW === w.id ? w.color : brdC}`, background: bsW === w.id ? w.color + "15" : "transparent", fontSize: 12, fontWeight: bsW === w.id ? 600 : 500, color: bsW === w.id ? w.color : mutC, cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center", gap: 4 }}><DI2 id={w.id} accent={w.neon || w.color} size={12} />{w.name}</button>)}</div>
          <button className="evPress" onClick={() => { if (canSub) sBsS(2) }} disabled={!canSub} style={{ width: "100%", padding: 13, border: "none", borderRadius: 10, background: canSub ? greenC : stoneC, color: canSub ? "#fff" : mutC, fontSize: 13, fontWeight: 700, cursor: canSub ? "pointer" : "default", opacity: canSub ? 1 : 0.7, display: "flex", alignItems: "center", justifyContent: "center", gap: 5, boxShadow: canSub ? "0 2px 8px rgba(90,122,90,0.3)" : "none" }}>Review Split<ArrowRight size={13} /></button></div>
      })()}
      {sel.status === "active" && <div style={secLbl}><span style={{ display: "inline-flex", alignItems: "center", gap: 6 }}><ArrowsLeftRight size={11} />Add Split</span>{hl}</div>}
      {sel.status === "active" && <div style={{ padding: "0 2px", marginBottom: 30 }}><div style={{ display: "flex", gap: 6, marginBottom: 12 }}>{["owed", "owe"].map(d => <button key={d} onClick={() => sSD(d)} style={{ flex: 1, padding: "9px", borderRadius: 10, fontSize: 12, border: `1px solid ${sd === d ? (d === "owe" ? redC : greenC) : brdC}`, background: sd === d ? (d === "owe" ? "rgba(192,82,74,0.08)" : "rgba(90,122,90,0.08)") : "transparent", color: sd === d ? (d === "owe" ? redC : greenC) : mutC, cursor: "pointer", fontWeight: sd === d ? 600 : 500, display: "flex", alignItems: "center", justifyContent: "center", gap: 5 }}>{d === "owe" ? <ArrowUpRight size={13} /> : <ArrowDownLeft size={13} />}{d === "owe" ? "I owe them" : "They owe me"}</button>)}</div><div style={{ display: "flex", gap: 8 }}><input value={sn} onChange={e => sSN(e.target.value)} placeholder="Friend name" style={{ ...wis, flex: 1, width: "auto" }} /><input type="number" value={sa} onChange={e => sSA(e.target.value)} placeholder="₹" style={{ ...wis, width: 74 }} /><input value={spNote} onChange={e => sSPNote(e.target.value)} placeholder="Note" style={{ ...wis, flex: 1, width: "auto" }} /><button className="evPress" onClick={addSplit} style={{ padding: "10px 15px", border: "none", borderRadius: 10, background: greenC, color: "#fff", cursor: "pointer", display: "flex", alignItems: "center", boxShadow: "0 2px 8px rgba(90,122,90,0.3)" }}><IconPlus size={16} /></button></div></div>}
      </div>
      {doneConfirm && <div style={{ position: "fixed", inset: 0, background: dm ? "rgba(0,0,0,0.65)" : "rgba(20,10,0,0.45)", zIndex: 200, display: "flex", alignItems: "flex-end", justifyContent: "center" }} onClick={() => sDoneConfirm(false)}><div onClick={e => e.stopPropagation()} style={{ background: dm ? "#1a1108" : "#f5f0e6", backgroundImage: `radial-gradient(circle, ${dm ? "rgba(180,140,90,0.12)" : "rgba(140,100,50,0.1)"} 1.2px, transparent 1.2px)`, backgroundSize: "18px 18px", backgroundPosition: "9px 9px", borderRadius: "20px 20px 0 0", padding: "26px 22px 38px", width: "100%", maxWidth: 430, maxHeight: "85vh", overflowY: "auto", borderTop: `1px solid ${dm ? "rgba(180,140,90,0.25)" : "rgba(160,120,70,0.2)"}` }}><div style={{ fontFamily: "'Playfair Display', Georgia, serif", fontSize: 19, color: dm ? "#f0e6d3" : "#2a1f10", marginBottom: 8 }}>Mark event done?</div><div style={{ fontSize: 13, color: dm ? "#8a7560" : "#9a8060", fontFamily: "var(--font-b)", marginBottom: 18, lineHeight: 1.6 }}>{pendingCnt} unsettled IOU{pendingCnt === 1 ? "" : "s"} remain in this event. They stay visible and can still be settled later — and you can reopen the event any time.</div><div style={{ display: "flex", gap: 10 }}><button className="evPress" onClick={() => sDoneConfirm(false)} style={{ flex: 1, padding: 13, border: `1px solid ${dm ? "rgba(200,169,110,0.35)" : "rgba(138,96,48,0.3)"}`, borderRadius: 10, background: dm ? "rgba(30,22,14,0.7)" : "rgba(240,234,220,0.8)", color: dm ? "#c8a96e" : "#8a6030", fontFamily: "var(--font-h)", fontSize: 13, fontWeight: 600, cursor: "pointer" }}>Cancel</button><button className="evPress" onClick={() => { oMD(sel.id); sDoneConfirm(false); }} style={{ flex: 1, padding: 13, border: "none", borderRadius: 10, background: greenC, color: "#fff", fontFamily: "var(--font-h)", fontSize: 13, fontWeight: 700, cursor: "pointer" }}>Mark Done</button></div></div></div>}
      {suTgt && <div style={{ position: "fixed", inset: 0, background: dm ? "rgba(0,0,0,0.65)" : "rgba(20,10,0,0.45)", zIndex: 200, display: "flex", alignItems: "flex-end", justifyContent: "center" }} onClick={() => sSuTgt(null)}><div onClick={e => e.stopPropagation()} style={{ background: dm ? "#1a1108" : "#f5f0e6", backgroundImage: `radial-gradient(circle, ${dm ? "rgba(180,140,90,0.12)" : "rgba(140,100,50,0.1)"} 1.2px, transparent 1.2px)`, backgroundSize: "18px 18px", backgroundPosition: "9px 9px", borderRadius: "20px 20px 0 0", padding: "26px 22px 38px", width: "100%", maxWidth: 430, maxHeight: "85vh", overflowY: "auto", borderTop: `1px solid ${dm ? "rgba(180,140,90,0.25)" : "rgba(160,120,70,0.2)"}` }}><div style={{ fontFamily: "'Playfair Display', Georgia, serif", fontSize: 19, color: dm ? "#f0e6d3" : "#2a1f10", marginBottom: 8 }}>Settle up with {suTgt.from === "You" ? suTgt.to : suTgt.from}</div><div style={{ fontSize: 13, color: dm ? "#8a7560" : "#9a8060", fontFamily: "var(--font-b)", marginBottom: 18, lineHeight: 1.6 }}>{suTgt.from === "You" ? <>You pay <strong>{suTgt.to}</strong></> : <><strong>{suTgt.from}</strong> pays you</>} <strong style={{ fontFamily: "'Playfair Display', Georgia, serif", fontSize: 15 }}>{fmt(suTgt.amt)}</strong> — this clears every pending IOU between you two in this event in one move.</div><label style={wls}>{suTgt.from === "You" ? "Pay from" : "Receive into"}</label><div style={{ display: "flex", gap: 6, marginBottom: 18 }}>{wl.map(w => <button key={w.id} className="evPress" onClick={() => sSuW(w.id)} style={{ flex: 1, padding: "9px", borderRadius: 10, border: `1px solid ${suW === w.id ? w.color : brdC}`, background: suW === w.id ? w.color + "15" : "transparent", fontSize: 12, fontWeight: suW === w.id ? 600 : 500, color: suW === w.id ? w.color : mutC, cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center", gap: 4 }}><DI2 id={w.id} accent={w.neon || w.color} size={12} />{w.name}</button>)}</div><div style={{ display: "flex", gap: 10 }}><button className="evPress" onClick={() => sSuTgt(null)} style={{ flex: 1, padding: 13, border: `1px solid ${dm ? "rgba(200,169,110,0.35)" : "rgba(138,96,48,0.3)"}`, borderRadius: 10, background: dm ? "rgba(30,22,14,0.7)" : "rgba(240,234,220,0.8)", color: dm ? "#c8a96e" : "#8a6030", fontFamily: "var(--font-h)", fontSize: 13, fontWeight: 600, cursor: "pointer" }}>Cancel</button><button className="evPress" onClick={() => { const ok = oSEN(sel.id, suTgt.from === "You" ? suTgt.to : suTgt.from, suW); if (ok !== false) sSuTgt(null); }} style={{ flex: 2, padding: 13, border: "none", borderRadius: 10, background: greenC, color: "#fff", fontFamily: "var(--font-h)", fontSize: 13, fontWeight: 700, cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center", gap: 5 }}><IconCheck size={14} />Settle {fmt(suTgt.amt)}</button></div></div></div>}
      {editOpen && <div style={{ position: "fixed", inset: 0, background: dm ? "rgba(0,0,0,0.65)" : "rgba(20,10,0,0.45)", zIndex: 200, display: "flex", alignItems: "flex-end", justifyContent: "center" }} onClick={() => sEditOpen(false)}><div onClick={e => e.stopPropagation()} style={{ background: dm ? "#1a1108" : "#f5f0e6", backgroundImage: `radial-gradient(circle, ${dm ? "rgba(180,140,90,0.12)" : "rgba(140,100,50,0.1)"} 1.2px, transparent 1.2px)`, backgroundSize: "18px 18px", backgroundPosition: "9px 9px", borderRadius: "20px 20px 0 0", padding: "26px 22px 38px", width: "100%", maxWidth: 430, maxHeight: "85vh", overflowY: "auto", borderTop: `1px solid ${dm ? "rgba(180,140,90,0.25)" : "rgba(160,120,70,0.2)"}` }}><div style={{ fontFamily: "'Playfair Display', Georgia, serif", fontSize: 19, color: dm ? "#f0e6d3" : "#2a1f10", marginBottom: 8 }}>Edit Event</div><label style={wls}>Name</label><input value={enName} onChange={e => sEnName(e.target.value)} style={{ ...wis, marginBottom: 14 }} /><label style={wls}>Icon</label><div style={{ display: "flex", flexWrap: "wrap", gap: 6, marginBottom: 14 }}>{EI.map(id => <button key={id} className="evPress" onClick={() => sEnIcon(id)} style={{ width: 38, height: 38, borderRadius: 10, display: "flex", alignItems: "center", justifyContent: "center", border: `1px solid ${enIcon === id ? terraC : brdC}`, background: enIcon === id ? "rgba(196,96,58,0.1)" : "transparent", cursor: "pointer", color: enIcon === id ? terraC : mutC }}><EvIcon id={id} size={18} /></button>)}</div>{isGroup && <><label style={wls}>Participants</label><div style={{ display: "flex", flexWrap: "wrap", gap: 6, marginBottom: 8 }}>{enParts.map(p => { const locked = partActive(p); return <span key={p} title={locked ? "Has expenses or IOUs in this event — can't be removed" : undefined} style={{ display: "inline-flex", alignItems: "center", gap: 5, padding: "6px 11px", borderRadius: 20, fontSize: 12, border: `1px solid ${brdC}`, background: dm ? "rgba(30,22,14,0.7)" : "rgba(255,255,255,0.7)", color: dm ? "#f0e6d3" : "#2a1f10" }}>{p}{locked ? <span style={{ fontSize: 9, color: mutC, letterSpacing: "0.5px" }}>in use</span> : <button onClick={() => sEnParts(pp => pp.filter(x => x !== p))} style={{ background: "none", border: "none", color: mutC, cursor: "pointer", padding: 0, display: "flex" }}><IconX size={12} /></button>}</span>; })}</div><div style={{ display: "flex", gap: 6, marginBottom: 6 }}><input value={enNew} onChange={e => sEnNew(e.target.value)} onKeyDown={e => { if (e.key === "Enter") addEnPart(); }} placeholder="Add person" style={{ ...wis, flex: 1, width: "auto" }} /><button className="evPress" onClick={addEnPart} style={{ padding: "10px 15px", border: "none", borderRadius: 10, background: terraC, color: "#fff", cursor: "pointer", display: "flex", alignItems: "center" }}><IconPlus size={15} /></button></div><div style={{ fontSize: 10.5, color: mutC, marginBottom: 16, lineHeight: 1.5 }}>People with recorded expenses or IOUs can't be removed. Adding or removing people changes how future and existing totals are shared.</div></>}<div style={{ display: "flex", gap: 10 }}><button className="evPress" onClick={() => sEditOpen(false)} style={{ flex: 1, padding: 13, border: `1px solid ${dm ? "rgba(200,169,110,0.35)" : "rgba(138,96,48,0.3)"}`, borderRadius: 10, background: dm ? "rgba(30,22,14,0.7)" : "rgba(240,234,220,0.8)", color: dm ? "#c8a96e" : "#8a6030", fontFamily: "var(--font-h)", fontSize: 13, fontWeight: 600, cursor: "pointer" }}>Cancel</button><button className="evPress" onClick={saveEdit} style={{ flex: 2, padding: 13, border: "none", borderRadius: 10, background: terraC, color: "#fff", fontFamily: "var(--font-h)", fontSize: 13, fontWeight: 700, cursor: "pointer" }}>Save Changes</button></div></div></div>}
...
export default function Nomad() {
  ...
  const [hSearch, sHSearch] = useState(""), [hMinAmt, sHMinAmt] = useState(""), [hMaxAmt, sHMaxAmt] = useState(""), [hDateFrom, sHDateFrom] = useState(""), [hDateTo, sHDateTo] = useState(""), [hType, sHType] = useState("all"), [hShowFilters, sHShowFilters] = useState(false), [hTimeline, sHTimeline] = useState(false), [hCalDay, sHCalDay] = useState(null);
  ...
  useEffect(() => {
    ...
    const totalExp = ex.reduce((s, e) => s + e.amount, 0);
    const catTotals = {};
    ex.forEach(e => { catTotals[e.categoryId] = (catTotals[e.categoryId] || 0) + e.amount; });
  ...
  const settleNet = (name, wid) => {
    const remOf = s => roundMoney(s.amount - stl.filter(x => x.splitId === s.id).reduce((t, x) => t + x.amount, 0));
    const items = sp.filter(s => s.name === name && !s.eventId && !s.deleted_at && !s.settled && !s.skipped).map(s => ({ s, rem: remOf(s) })).filter(x => x.rem > 0.005);
    ...
    const recs = items.map(x => ({ id: uid(), type: "settlement", splitName: x.s.name, splitId: x.s.id, amount: x.rem, direction: x.s.direction, walletId: wid, date: today, createdAt: new Date().toISOString(), ...(x.s.categoryId && { categoryId: x.s.categoryId }), ...(x.s.note && { note: x.s.note }), ...(x.s.groupId && { groupId: x.s.groupId }) }));
  ...
  // Event-scoped sibling of settleNet: clears every pending IOU a participant
  // has inside ONE event with a single net movement (one-tap "Settle Up" from
  // the group ledger). Same net validation, but records keep their eventId so
  // the ledger's grpSettled math picks them up.
  const settleEventNet = (eventId, name, wid) => {
    const remOf = s => roundMoney(s.amount - stl.filter(x => x.splitId === s.id).reduce((t, x) => t + x.amount, 0));
    const nameLc = String(name || "").toLowerCase();
    const items = sp.filter(s => s.eventId === eventId && (s.name || "").toLowerCase() === nameLc && !s.deleted_at && !s.settled && !s.skipped).map(s => ({ s, rem: remOf(s) })).filter(x => x.rem > 0.005);
    ...
    const recs = items.map(x => ({ id: uid(), type: "settlement", splitName: x.s.name, splitId: x.s.id, amount: x.rem, direction: x.s.direction, walletId: wid, date: today, createdAt: new Date().toISOString(), eventId, ...(x.s.categoryId && { categoryId: x.s.categoryId }), ...(x.s.note && { note: x.s.note }), ...(x.s.groupId && { groupId: x.s.groupId }) }));
    const ids = items.map(x => x.s.id);
    sStl(p => [...p, ...recs]);
    sbUpsert("settlements", recs.map(r => toSB(r, COLS.settlements)));
    sSp(p => p.map(x => ids.includes(x.id) ? { ...x, settled: true } : x));
  ...
  return <div style={{ ...theme, fontFamily: "var(--font-b)", background: "var(--bg)", color: "var(--text)", minHeight: "100vh", width: "100%", maxWidth: 430, margin: "0 auto", padding: "0 0 110px", overflowX: "hidden", boxSizing: "border-box" }}><style>{`
...
{module === "finance" && <div style={{ position: "fixed", bottom: 0, left: 0, right: 0, maxWidth: 430, margin: "0 auto", zIndex: 50, paddingBottom: "env(safe-area-inset-bottom)" }}><div style={{ position: "relative", height: 76 }}><svg width="100%" height="76" viewBox="0 0 430 76" preserveAspectRatio="none" style={{ position: "absolute", inset: 0, display: "block", filter: "drop-shadow(0 -2px 12px rgba(0,0,0,0.07))" }}><path d="M0,18 L158,18 C184,18 185,52 215,52 C245,52 246,18 272,18 L430,18 L430,76 L0,76 Z" fill="var(--nav-bg)" stroke="var(--border)" strokeWidth="1" /></svg><div style={{ position: "absolute", inset: "18px 0 0 0", display: "flex" }}>{[{ id: "dashboard", label: "Home" }, { id: "events", label: "Events" }, { id: "__fab", label: "" }, { id: "history", label: "History" }, { id: "settings", label: "Settings" }].map(n => n.id === "__fab" ? <div key="__fab" style={{ flex: 1 }} /> : <button key={n.id} onClick={() => sTab(n.id)} style={{ flex: 1, padding: "8px 0 0", border: "none", background: "none", display: "flex", flexDirection: "column", alignItems: "center", gap: 3, cursor: "pointer", opacity: tab === n.id ? 1 : 0.45 }}><div style={{ position: "relative", display: "flex", alignItems: "center", justifyContent: "center" }}>{tab === n.id && <span key={"rip" + n.id} style={{ position: "absolute", top: "50%", left: "50%", width: 30, height: 30, borderRadius: "50%", background: "#E07A5F", pointerEvents: "none", animation: "ripple 0.6s ease-out forwards" }} />}<NI type={n.id} active={tab === n.id} />{n.id === "settings" && deadLetterCount > 0 && <div style={{ position: "absolute", top: -2, right: -2, width: 8, height: 8, borderRadius: "50%", background: "#D4726A", border: "2px solid var(--nav-bg)" }} />}</div><span style={{ fontFamily: "var(--font-h)", fontSize: 9, color: tab === n.id ? "#E07A5F" : "var(--muted)", fontWeight: tab === n.id ? 600 : 400 }}>{n.label}</span></button>)}</div><button onClick={() => sTab("add")} aria-label="Add" style={{ position: "absolute", top: -18, left: "50%", width: 58, height: 58, borderRadius: "50%", border: "none", background: tab === "add" ? "#D4704A" : "#E07A5F", color: "#fff", cursor: "pointer", boxShadow: "0 8px 20px rgba(224,122,95,0.42), 0 2px 6px rgba(224,122,95,0.28)", display: "flex", alignItems: "center", justifyContent: "center", overflow: "hidden", transform: `translateX(-50%) scale(${tab === "add" ? 0.94 : 1})`, transition: "transform 0.18s cubic-bezier(0.34,1.56,0.64,1), background 0.18s ease" }}>{tab === "add" && <span key="fabrip" style={{ position: "absolute", top: "50%", left: "50%", width: 58, height: 58, borderRadius: "50%", background: "rgba(255,255,255,0.45)", pointerEvents: "none", animation: "navsplash 0.6s ease-out forwards" }} />}<svg width="26" height="26" viewBox="0 0 24 24" fill="none" stroke="#fff" strokeWidth="2.6" strokeLinecap="round" style={{ position: "relative", zIndex: 1 }}><line x1="12" y1="5" x2="12" y2="19" /><line x1="5" y1="12" x2="19" y2="12" /></svg></button></div></div>}

    {calW && <CalM wallet={calW} currentBal={wBal[calW.id] || 0} onSave={(v, note) => handleCal(calW.id, v, note)} onViewLedger={() => { const wv = calW; sCalW(null); sLedgerW(wv); }} onClose={() => sCalW(null)} />}{recountW && <RecountM wallet={recountW} currentBal={wBal[recountW.id] || 0} onClose={() => sRecountW(null)} />}{ledgerW && (() => { const wid = ledgerW.id; const touches = (it) => it.type === "expense" ? (it.walletId || "upi_lite") === wid : it.type === "income" ? (it.walletId || "bank") === wid : it.type === "transfer" ? (it.fromWallet === wid || it.toWallet === wid) : it.type === "settlement" ? it.walletId === wid : false; const all = [...ex.map(e => ({ ...e, type: "expense" })), ...inc.map(i => ({ ...i, type: "income" })), ...tr.map(t => ({ ...t, type: "transfer" })), ...stl.map(s => ({ ...s, type: "settlement" }))].filter(touches).sort(historySortCompare); const labelFor = (it) => it.type === "expense" ? (cats.find(c => c.id === it.categoryId)?.name || recCats.find(c => c.id === it.categoryId)?.name || "Expense") : it.type === "income" ? (isrc.find(s => s.id === it.sourceId)?.name || "Income") : it.type === "transfer" ? (it.fromWallet === wid ? `Transfer → ${wallets.find(x => x.id === it.toWallet)?.name || "?"}` : `Transfer ← ${wallets.find(x => x.id === it.fromWallet)?.name || "?"}`) : (it.splitName ? `Settle · ${it.splitName}` : "Settlement"); const lastV = walletVerify[wid]?.last; const rowVerified = (it) => { if (!lastV) return false; const precise = it.created_at || it.createdAt || it.updated_at; if (precise) return new Date(precise).getTime() <= lastV.ts; if (!it.date) return false; return it.date <= lastV.date; }; const rows = all.map(it => { const tl = timelineData[it.id]; const after = roundMoney(tl?.after[wid] ?? 0); const before = roundMoney(tl?.before[wid] ?? 0); return { id: it.id, label: labelFor(it), date: it.date, note: dispNote(it.note), delta: roundMoney(after - before), after, verified: rowVerified(it) }; }); const lastVerifyLabel = lastV ? dl(lastV.date) : null; return <LedgerM wallet={ledgerW} rows={rows} curBal={roundMoney(wBal[wid] || 0)} lastVerifyLabel={lastVerifyLabel} onReconcile={() => { const wv = ledgerW; sLedgerW(null); sCalW(wv); }} onClose={() => sLedgerW(null)} />; })()}
    {recEditId && (() => {
