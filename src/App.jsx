...
function AddPage({ categories: cats, incomeSources: isrc, recurringCats: rCats, onAddExpense: oE, onAddIncome: oI, onAddTransfer: oT, onAddRec: oR, onError: showT = () => {}, patterns = [], autoRules = [], wallets: aw = WALLETS }) {
  ...
  const [tagInput, sTagInput] = useState(""), [tags, sTags] = useState([]);
  ...
  const submit = async () => {
    ...
    try {
      ...
      if (type === "expense") {
        ...
        txOk = oE({ id: txId, amount: inrAmt, categoryId: catId, date, note, walletId: wid, tags: tags.length ? tags : undefined, ...(rUrl ? { receipt_url: rUrl } : {}) }) !== false;
      } else if (type === "income") {
        const txId = uid();
        if (isFX) saveCurrencyMeta(txId, fxCur, a, fxRate);
        txOk = oI({ id: txId, amount: inrAmt, sourceId: srcId, date, note, walletId: iwid, tags: tags.length ? tags : undefined, ...(rUrl ? { receipt_url: rUrl } : {}) }) !== false;
  ...
  return <div style={{ padding: "0 0 20px" }}>
    ...
    {type !== "recurring" && <><div style={{ marginBottom: 16 }}><label style={ls}>Amount</label>
      ...
      <div style={{ display: "flex", gap: 10, marginBottom: 18 }}><div style={{ flex: 1 }}><label style={ls}>Date</label><input type="date" value={date} onChange={e => sDate(e.target.value)} style={is} /></div><div style={{ flex: 1 }}><label style={ls}>Note</label><input value={note} onChange={e => { const v = e.target.value; sNote(v); if (type === "expense" && autoRules.length) { const kw = v.toLowerCase().trim(); const m = autoRules.find(r => kw.includes(r.keyword.toLowerCase())); if (m) sCat(m.categoryId); } }} placeholder="Optional…" style={is} /></div></div>
      {type !== "transfer" && <div style={{ marginBottom: 18 }}><ReceiptPicker ref={receiptPickerRef} /></div>}
      {(type === "expense" || type === "income") && <div style={{ marginBottom: 18 }}>{tags.length > 0 && <div style={{ display: "flex", gap: 5, flexWrap: "wrap", marginBottom: 6 }}>{tags.map(t => <span key={t} style={{ display: "inline-flex", alignItems: "center", gap: 4, fontSize: 11, fontFamily: "var(--font-h)", fontWeight: 600, color: "#7B8CDE", background: "#7B8CDE15", padding: "3px 8px", borderRadius: 12 }}>#{t}<button onClick={() => sTags(p => p.filter(x => x !== t))} style={{ background: "none", border: "none", color: "#7B8CDE", cursor: "pointer", padding: 0, lineHeight: 1, fontSize: 10 }}>✕</button></span>)}</div>}<div style={{ display: "flex", gap: 6 }}><input value={tagInput} onChange={e => sTagInput(e.target.value.replace(/\s+/g, ""))} onKeyDown={e => { if ((e.key === "Enter" || e.key === " " || e.key === ",") && tagInput.trim()) { e.preventDefault(); const t = tagInput.trim().toLowerCase(); if (!tags.includes(t) && tags.length < 5) sTags(p => [...p, t]); sTagInput(""); } }} placeholder={tags.length < 5 ? "#tag (Enter to add)…" : ""} disabled={tags.length >= 5} style={{ ...is, flex: 1, padding: "8px 10px", marginBottom: 0, fontSize: 12 }} /></div></div>}
...
const TxCard = memo(function TxCard({ item: it, categories: cats, incomeSources: isrc, events: evs, onDelete: od, recurringCats: rCats, wallets: wl = WALLETS, onRefund: oRef }) {
  ...
  return <div style={{ ...cc, borderRadius: 14, padding: "14px 16px", display: "flex", alignItems: "center", gap: 12, marginBottom: 10 }}><div style={{ width: 44, height: 44, borderRadius: 12, background: (cat?.color || "#999") + "14", display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0 }}>{cat ? <DI2 id={cat.id} accent={cat.neon || cat.color} size={22} /> : <span style={{ fontSize: 22 }}>❓</span>}</div><div style={{ flex: 1, minWidth: 0, overflow: "hidden" }}><div style={{ display: "flex", alignItems: "center", gap: 6, flexWrap: "wrap" }}><span style={{ fontSize: 14, fontWeight: 600, color: "var(--text)", fontFamily: "var(--font-h)" }}>{cat?.name || "Unknown"}</span>{isE && <span style={{ fontSize: 7, fontFamily: "var(--font-h)", fontWeight: 600, color: isFix(it) ? "#A78BFA" : "#FBBF24", background: isFix(it) ? "#A78BFA15" : "#FBBF2415", padding: "1px 5px", borderRadius: 3 }}>{isFix(it) ? "FIXED" : "FLEX"}</span>}{w && <span style={{ fontSize: 9, fontFamily: "var(--font-h)", fontWeight: 600, color: w.color, background: w.color + "18", padding: "2px 6px", borderRadius: 4, display: "inline-flex", alignItems: "center", gap: 2 }}><DI2 id={w.id} accent={w.neon || w.color} size={10} /></span>}</div><div style={{ fontSize: 12, color: "var(--muted)", fontFamily: "var(--font-b)", marginTop: 2, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{evT && <span style={{ fontWeight: 600, color: "var(--ts)" }}>{evT} · </span>}{dl(it.date)}{it.note ? " · " + it.note : ""}</div>{fxMeta && <div style={{ fontSize: 10, color: "#7B8CDE", fontFamily: "var(--font-h)", fontWeight: 600, marginTop: 3, letterSpacing: "0.3px" }}>{fxMeta.currency} {fxMeta.originalAmount} @ {Number(fxMeta.rateUsed).toFixed(2)}</div>}
    {Array.isArray(it.tags) && it.tags.length > 0 && <div style={{ marginTop: 3, display: "flex", gap: 4, flexWrap: "wrap" }}>{it.tags.map(t => <span key={t} style={{ fontSize: 9, fontFamily: "var(--font-h)", fontWeight: 600, color: "#7B8CDE", background: "#7B8CDE15", padding: "2px 6px", borderRadius: 10 }}>#{t}</span>)}</div>}
...
export default function Nomad() {
  ...
  const [hTag, sHTag] = useState(""); // history tag filter
  ...
  const historyItems = useMemo(() => {
    ...
    if (hTag) items = items.filter(it => Array.isArray(it.tags) && it.tags.includes(hTag));
  ...
  }, [flt, ex, inc, tr, stl, fm, hSearch, hMinAmt, hMaxAmt, hDateFrom, hDateTo, hType, hTag, cats, isrc]);
  ...
  const addE = data => {
    ...
    sEx(p => [rec, ...p]);
    sbUpsert("expenses", [toSB(rec, ["id", "amount", "categoryId", "walletId", "note", "date", "eventId", "groupId", "receipt_url", "paidBy", "tags"])]);
    dance();
  ...
  const addI = data => { const amt = roundMoney(data.amount); if (data.walletId === "upi_lite") { showT("UPI Lite is for spending only", "error"); return } if (amt <= 0) { showT("Enter a valid amount", "error"); return } if (typeof data.note === "string" && data.note.length > 500) data = { ...data, note: data.note.slice(0, 500) }; const rec = { id: uid(), type: "income", ...data, amount: amt, created_at: new Date().toISOString() }; sInc(p => [rec, ...p]); sbUpsert("incomes", [toSB(rec, ["id", "amount", "sourceId", "walletId", "note", "date", "receipt_url", "tags"])]); dance(); showT(online ? "Income added" : "Income saved offline", "success") };
...
`}</style>
    ...
    {module === "finance" && <div style={{ padding: "0 16px" }}>
      ...
      {tab === "dashboard" && <div className="pe">
        ...
        {finStreak > 1 && <div style={{ ...cc, padding: "10px 16px", marginBottom: 12, display: "flex", alignItems: "center", gap: 10, borderLeft: "3px solid #FBBF24" }}><span style={{ fontSize: 22 }}>🔥</span><div><div style={{ fontFamily: "var(--font-h)", fontSize: 13, fontWeight: 700, color: "var(--text)" }}>{finStreak}-day logging streak!</div><div style={{ fontSize: 11, color: "var(--muted)", marginTop: 1 }}>You've logged transactions {finStreak} days in a row</div></div></div>}
      ...
      {tab === "add" && <div className="pse" style={{ paddingTop: 20 }}><AddPage categories={cats} incomeSources={isrc} recurringCats={recCats} onAddExpense={addE} onAddIncome={addI} onAddTransfer={addT} onAddRec={addRec} onError={showT} patterns={quickPatterns} autoRules={autoRules} wallets={wallets} /></div>}
