...
function AddPage({ categories: cats, incomeSources: isrc, recurringCats: rCats, onAddExpense: oE, onAddIncome: oI, onAddTransfer: oT, onAddRec: oR, onError: showT = () => {}, patterns = [], autoRules = [], onLearnRule = () => {}, wallets: aw = WALLETS, cloudinaryEnabled = false }) {
  ...
  useEffect(() => { const c = fxCur.trim().toUpperCase(); if (c.length !== 3 || c === "INR") { setFxRate(null); setFxDate(null); return; } setFxFetching(true); getExchangeRate(c).then(r => { setFxRate(r); setFxDate(getRateMeta(c)?.date || null); setFxFetching(false); }).catch(() => { setFxRate(null); setFxDate(null); setFxFetching(false); }); }, [fxCur]);
    ...
    {type !== "recurring" && (() => {
      ...
      return <>
        ...
        <div style={{ borderRadius: 24, padding: "16px 18px 18px", marginBottom: 14, background: `linear-gradient(165deg, ${alpha(tc, 0.13)}, ${alpha(tc, 0.04)})`, border: `1.5px solid ${alpha(tc, 0.28)}` }}>
          ...
          {isFXcur && <div style={{ marginTop: 8, display: "flex", alignItems: "center", justifyContent: "space-between" }}><span style={{ fontFamily: "var(--font-h)", fontSize: 10.5, color: "var(--muted)", fontWeight: 600 }}>1 {fxCur} = ₹{fxRate.toFixed(2)}{fxDate ? ` · daily rate ${fxDate}` : ""}</span>{parseFloat(amt) > 0 && <span style={{ fontFamily: "var(--font-h)", fontSize: 14, fontWeight: 800, color: tc }}>≈ {fmt(inrAmt)}</span>}</div>}
