...
export default function Nomad() {
  ...
  const expCSV = () => { const q = v => `"${String(v ?? "").replace(/"/g, '""')}"`; let csv = "Type,Date,Amount,Category/Source,Wallet,Note\n"; inc.forEach(i => { csv += `Income,${i.date},${i.amount},${q(isrc.find(s => s.id === i.sourceId)?.name)},${q(wallets.find(x => x.id === i.walletId)?.name)},${q(i.note)}\n` }); ex.forEach(e => { csv += `Expense,${e.date},${e.amount},${q(cats.find(c => c.id === e.categoryId)?.name)},${q(wallets.find(x => x.id === e.walletId)?.name)},${q(e.note)}\n` }); tr.forEach(t => { csv += `Transfer,${t.date},${t.amount},${q(`${t.fromWallet}→${t.toWallet}`)},${q("")},${q(t.note)}\n` }); stl.forEach(s => { csv += `Settlement,${s.date},${s.amount},${q(s.splitName)},${q(wallets.find(w => w.id === s.walletId)?.name)},${q(s.direction)}\n` }); const a = document.createElement("a"); a.href = URL.createObjectURL(new Blob([csv], { type: "text/csv" })); a.download = `nomad_${localDateKey()}.csv`; a.click() };
...
`}</style>
    ...
    {module === "finance" && <div style={{ padding: "0 16px" }}>
      ...
      {tab === "dashboard" && <div className="pe">
        ...
        <div style={{ display: "flex", gap: 8, marginBottom: 14 }}><button onClick={() => sTab("calendar")} style={{ flex: 1, padding: "10px 12px", borderRadius: 12, border: "1.5px solid var(--border)", background: "var(--card)", color: "var(--text)", fontFamily: "var(--font-h)", fontSize: 12, fontWeight: 700, cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center", gap: 6 }}><NI type="calendar" /><span>Calendar</span></button><button onClick={() => sTab("ai")} style={{ flex: 1, padding: "10px 12px", borderRadius: 12, border: "1.5px solid #7B8CDE", background: "#7B8CDE15", color: "#7B8CDE", fontFamily: "var(--font-h)", fontSize: 12, fontWeight: 700, cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center", gap: 6 }}><Sparkle size={16} weight="fill" /><span>AI Toolbox</span></button></div>
