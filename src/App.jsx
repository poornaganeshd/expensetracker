...
export default function Nomad() {
  ...
  useEffect(() => {
    ...
    if (Notification.permission === "default") {
      const t = setTimeout(() => Notification.requestPermission().then(p => sNotifPerm(p)), 3000);
    ...
    reminders.forEach((r, i) => {
      setTimeout(() => {
        ...
        if (Notification.permission === "granted") {
          try {
            const n = new Notification("NOMAD — " + (r.type === "warn" ? "Bill Due" : "Upcoming Bill"), { body: r.msg, icon: "/icon-192.png", tag: r.id, requireInteraction: r.type === "warn" });
            n.onclick = () => { window.focus(); n.close(); };
          } catch { /* sw context or permission revoked */ }
        }
      }, i * 700);
    });
  }, [loaded, rec, sp]);
...
{historyItems.map(it => <div key={it.id} style={{ display: "flex", alignItems: "stretch", gap: 6 }}>{bulkMode && <div onClick={() => sBulkSel(p => { const n = new Set(p); n.has(it.id) ? n.delete(it.id) : n.add(it.id); return n; })} style={{ display: "flex", alignItems: "center", padding: "0 4px", cursor: "pointer", flexShrink: 0 }}><div style={{ width: 20, height: 20, borderRadius: 5, border: `2px solid ${bulkSel.has(it.id) ? "#E07A5F" : "var(--border)"}`, background: bulkSel.has(it.id) ? "#E07A5F" : "var(--card)", display: "flex", alignItems: "center", justifyContent: "center" }}>{bulkSel.has(it.id) && <span style={{ color: "#fff", fontSize: 11, fontWeight: 700 }}>✓</span>}</div></div>}<div style={{ flex: 1, minWidth: 0 }}><TxCard item={it} categories={cats} incomeSources={isrc} events={evs} onDelete={delItem} recurringCats={recCats} wallets={wallets} onRefund={refundItem} /></div></div>)}{historyItems.length === 0 && <div style={{ textAlign: "center", padding: "60px 20px", color: "var(--muted)", fontSize: 14, lineHeight: 1.8 }}>{flt.expenses.length === 0 && flt.incomes.length === 0 ? <><div style={{ fontSize: 32, marginBottom: 12 }}>📋</div><div style={{ fontFamily: "var(--font-h)", fontSize: 15, fontWeight: 600, color: "var(--ts)", marginBottom: 6 }}>No transactions yet</div><div style={{ fontSize: 12, marginBottom: 20 }}>Log expenses, income, and transfers<br />to see your spending history here.</div><button onClick={() => sTab("add")} style={{ padding: "12px 28px", border: "none", borderRadius: 12, background: "#E07A5F", color: "#fff", fontFamily: "var(--font-h)", fontSize: 13, fontWeight: 700, cursor: "pointer" }}>+ Add First Transaction</button></> : "No results match your filters."}</div>}</div>}
      ...
      {tab === "settings" && <div className="pe" style={{ paddingTop: 8 }}>
        ...
        {"Notification" in window && <div style={{ ...cc, padding: "14px 20px", marginBottom: 14 }}><div style={{ fontFamily: "var(--font-h)", fontSize: 12, color: "var(--muted)", marginBottom: 12, letterSpacing: "0.5px", fontWeight: 600 }}>Notifications</div><div style={{ display: "flex", alignItems: "center", gap: 10, padding: "10px 12px", background: "var(--bg)", borderRadius: 10, border: "1px solid var(--border)", marginBottom: 10 }}><div style={{ width: 8, height: 8, borderRadius: "50%", background: notifPerm === "granted" ? "#6BAA75" : notifPerm === "denied" ? "#D4726A" : "#FBBF24", flexShrink: 0 }} /><div style={{ flex: 1 }}><div style={{ fontSize: 12, fontFamily: "var(--font-h)", fontWeight: 600, color: "var(--text)" }}>{notifPerm === "granted" ? "Notifications enabled" : notifPerm === "denied" ? "Blocked by browser" : "Not yet enabled"}</div><div style={{ fontSize: 10, color: "var(--muted)", marginTop: 2, fontFamily: "var(--font-h)" }}>{notifPerm === "granted" ? "Bill reminders will appear as system notifications" : notifPerm === "denied" ? "Allow in browser site settings to enable" : "Enable to get bill reminders even when app is in background"}</div></div></div>{notifPerm === "default" && <button onClick={() => Notification.requestPermission().then(p => sNotifPerm(p))} style={{ width: "100%", padding: "10px", border: "none", borderRadius: 10, background: "#7B8CDE", color: "#fff", fontFamily: "var(--font-h)", fontSize: 12, fontWeight: 600, cursor: "pointer" }}>Enable Notifications</button>}{notifPerm === "granted" && <button onClick={() => { try { new Notification("NOMAD", { body: "Notifications are working!", icon: "/icon-192.png" }); showT("Test notification sent", "success"); } catch { showT("Could not send notification", "error"); } }} style={{ width: "100%", padding: "10px", border: "1.5px solid var(--border)", borderRadius: 10, background: "var(--card)", color: "var(--muted)", fontFamily: "var(--font-h)", fontSize: 12, fontWeight: 600, cursor: "pointer" }}>Send test notification</button>}</div>}
