...
function Events({ events: evs, expenses: ex, splits: sp, settlements: stl, categories: cats, wallets: wl = WALLETS, staleByEvent = {}, onCreate: oC, onAddExp: oE, onAddSplit: oS, onSettleSplit: oSS, onDeleteSplit: oDS, onMarkDone: oMD, onDelete: oD, dm = false }) {
  ...
  const wcc = { background: cardC, borderRadius: 16, border: `1px solid ${brdC}`, boxShadow: "0 2px 12px rgba(44,36,22,0.07), 0 1px 3px rgba(44,36,22,0.05)" };
  const wis = { background: dm ? "rgba(15,10,5,0.5)" : "#FBF8F2", border: `1px solid ${brdC}`, borderRadius: 10, padding: "11px 14px", color: inkC, fontSize: 14, fontFamily: "var(--font-b)", outline: "none", width: "100%", boxSizing: "border-box" };
  ...
  if (view === "detail" && sel) {
    ...
    return <div style={{ position: "relative", background: pC, height: "calc(100vh - 90px)", display: "flex", flexDirection: "column", overflow: "hidden" }}><TopoBg />
      <div style={{ position: "relative", zIndex: 10, padding: "max(18px, calc(env(safe-area-inset-top, 0px) + 12px)) 20px 14px", display: "flex", alignItems: "center", justifyContent: "space-between" }}><button onClick={() => sV("list")} style={{ display: "flex", alignItems: "center", gap: 4, background: "none", border: "none", color: mutC, cursor: "pointer", fontSize: 13, letterSpacing: "0.5px", padding: 0 }}><CaretLeft size={15} />Events</button><div style={{ display: "flex", gap: 8, alignItems: "center" }}>{sel.status === "active" && <button className="evPress" onClick={() => oMD(sel.id)} style={{ display: "flex", alignItems: "center", gap: 5, padding: "7px 14px", border: "1px solid rgba(90,122,90,0.4)", borderRadius: 20, background: "rgba(90,122,90,0.1)", color: greenC, fontSize: 11, fontWeight: 600, letterSpacing: "0.5px", cursor: "pointer" }}><IconCheck size={13} />Mark Done</button>}<button className="evPress" onClick={() => sEvDelConfirm(sel.id)} style={{ display: "flex", alignItems: "center", gap: 5, padding: "7px 12px", border: "1px solid rgba(192,82,74,0.35)", borderRadius: 20, background: "rgba(192,82,74,0.08)", color: redC, fontSize: 11, fontWeight: 600, letterSpacing: "0.5px", cursor: "pointer" }}><Trash size={12} />Delete</button></div></div>
      ...
      {isGroup && allParts.length > 1 && <div style={{ ...wcc, padding: "2px 16px", marginBottom: 20, animation: "evSlideUp 0.4s 0.1s ease both" }}><div style={{ fontSize: 10.5, color: mutC, padding: "11px 0", borderBottom: `1px solid ${brdC}`, letterSpacing: "0.3px" }}>Equal share {fmt(grpShare)} per person · {allParts.length} people</div>{allParts.map(p => { const paid = grpPaid[p] || 0, bal = roundMoney(paid - (grpShareMap[p] ?? grpShare) - (grpSettled[p] || 0)); const settledP = Math.abs(bal) < 0.01; const av = avColor(p); const pi = allParts.indexOf(p); return <div key={p} style={{ display: "flex", alignItems: "center", gap: 12, padding: "11px 0", borderBottom: `1px solid ${brdC}`, animation: "evRowIn 0.35s ease both", animationDelay: `${0.12 + Math.min(pi, 8) * 0.05}s` }}><div style={{ width: 32, height: 32, borderRadius: "50%", background: av + "1F", border: `1px solid ${av}40`, color: av, display: "flex", alignItems: "center", justifyContent: "center", fontSize: 10.5, fontWeight: 700, flexShrink: 0, letterSpacing: "0.5px" }}>{initialsOf(p)}</div><div style={{ flex: 1, minWidth: 0 }}><div style={{ fontSize: 13.5, fontWeight: 500, color: inkC }}>{p}</div><div style={{ fontSize: 10.5, color: mutC, marginTop: 1 }}>paid {fmt(paid)} · share {fmt(grpShareMap[p] ?? grpShare)}</div></div><div style={{ textAlign: "right", flexShrink: 0 }}>{settledP ? <span style={{ fontSize: 11, color: mutC, display: "inline-flex", alignItems: "center", gap: 4 }}><IconCheck size={11} />settled</span> : <><div style={{ fontFamily: "'Playfair Display', Georgia, serif", fontSize: 15, color: bal > 0 ? greenC : terraC }}>{fmt(Math.abs(bal))}</div><div style={{ fontSize: 9, letterSpacing: "0.8px", textTransform: "uppercase", color: bal > 0 ? greenC : terraC, marginTop: 1 }}>{bal > 0 ? "gets back" : "owes"}</div></>}</div></div>; })}{(() => { const bals = allParts.map(p => ({ name: p, bal: roundMoney((grpPaid[p] || 0) - (grpShareMap[p] ?? grpShare) - (grpSettled[p] || 0)) })); const cr = bals.filter(b => b.bal > 0.01).map(b => ({ ...b })).sort((a, b) => b.bal - a.bal); const db = bals.filter(b => b.bal < -0.01).map(b => ({ ...b })).sort((a, b) => a.bal - b.bal); const settlements = []; let ci = 0, di = 0; while (ci < cr.length && di < db.length) { const amt = roundMoney(Math.min(cr[ci].bal, -db[di].bal)); settlements.push({ from: db[di].name, to: cr[ci].name, amt }); cr[ci].bal = roundMoney(cr[ci].bal - amt); db[di].bal = roundMoney(db[di].bal + amt); if (cr[ci].bal < 0.01) ci++; if (db[di].bal > -0.01) di++; } return settlements.length > 0 && <div style={{ padding: "10px 0 12px" }}><div style={{ fontSize: 9, letterSpacing: "1.5px", textTransform: "uppercase", color: goldC, fontWeight: 600, marginBottom: 8 }}>Settle Up</div>{settlements.map((s, i) => <div key={i} style={{ display: "flex", alignItems: "center", gap: 8, fontSize: 12.5, marginBottom: i < settlements.length - 1 ? 7 : 0, animation: "evRowIn 0.35s ease both", animationDelay: `${0.2 + Math.min(i, 8) * 0.06}s` }}><span style={{ fontWeight: 600, color: s.from === "You" ? terraC : inkC }}>{s.from}</span><span style={{ display: "inline-flex", animation: "evNudge 1.6s ease-in-out infinite" }}><ArrowRight size={12} color={mutC} /></span><span style={{ fontWeight: 600, color: s.to === "You" ? greenC : inkC }}>{s.to}</span><span style={{ marginLeft: "auto", fontFamily: "'Playfair Display', Georgia, serif", fontSize: 14, color: inkC }}>{fmt(s.amt)}</span></div>)}</div>; })()}</div>}
      {eSp.length > 0 && <div style={{ display: "flex", gap: 10, marginBottom: 20, animation: "evSlideUp 0.4s 0.12s ease both" }}><div style={{ flex: 1, background: "rgba(192,82,74,0.07)", border: "1px solid rgba(192,82,74,0.2)", borderRadius: 14, padding: "12px 14px" }}><div style={{ display: "flex", alignItems: "center", gap: 5, fontSize: 9, color: redC, letterSpacing: "1px", textTransform: "uppercase", fontWeight: 600 }}><IconArrowUp size={11} />You Owe</div><div style={{ fontFamily: "'Playfair Display', Georgia, serif", fontSize: 19, color: redC, marginTop: 4 }}>{fmt(tO)}</div></div><div style={{ flex: 1, background: "rgba(90,122,90,0.07)", border: "1px solid rgba(90,122,90,0.2)", borderRadius: 14, padding: "12px 14px" }}><div style={{ display: "flex", alignItems: "center", gap: 5, fontSize: 9, color: greenC, letterSpacing: "1px", textTransform: "uppercase", fontWeight: 600 }}><IconArrowDown size={11} />Owed To You</div><div style={{ fontFamily: "'Playfair Display', Georgia, serif", fontSize: 19, color: greenC, marginTop: 4 }}>{fmt(tI)}</div></div></div>}
      ...
      {eExps.length > 0 && <div style={{ ...wcc, padding: "2px 16px", marginBottom: 20, animation: "evSlideUp 0.4s 0.15s ease both" }}>{[...eExps].reverse().map((e, xi, arr) => { const cat = cats.find(c => c.id === e.categoryId) || { id: "other", name: "Other", color: "#999", neon: "#999" }; return <div key={e.id} style={{ display: "flex", alignItems: "center", gap: 12, padding: "11px 0", borderBottom: xi < arr.length - 1 ? `1px solid ${brdC}` : "none", animation: "evRowIn 0.35s ease both", animationDelay: `${Math.min(xi, 8) * 0.04}s` }}><div style={{ width: 34, height: 34, borderRadius: 10, background: dm ? "rgba(54,37,16,0.6)" : "linear-gradient(135deg, #F5EFE4, #EDE5D4)", border: "1px solid rgba(184,150,62,0.15)", display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0 }}><DI2 id={cat.id} accent={cat.neon || cat.color} size={16} /></div><div style={{ flex: 1, minWidth: 0 }}><div style={{ fontSize: 13.5, fontWeight: 500, color: inkC }}>{cat.name}</div>{e.note && <div style={{ fontSize: 11, color: mutC, marginTop: 1, whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis" }}>{e.note}</div>}{isGroup && e.paidBy && e.paidBy !== "me" && <div style={{ fontSize: 10, color: goldC, marginTop: 2, fontWeight: 600, letterSpacing: "0.3px", display: "inline-flex", alignItems: "center", gap: 3 }}><Users size={10} />paid by {e.paidBy}</div>}</div><span style={{ fontFamily: "'Playfair Display', Georgia, serif", fontSize: 15, color: terraC, flexShrink: 0 }}>−{fmt(e.amount)}</span></div>; })}</div>}
      ...
      {eSp.length > 0 && <div style={{ ...wcc, padding: "2px 16px", marginBottom: 20, animation: "evSlideUp 0.4s 0.18s ease both" }}>{eSp.map((s, si) => <div key={s.id} style={{ display: "flex", alignItems: "center", gap: 12, padding: "11px 0", borderBottom: si < eSp.length - 1 ? `1px solid ${brdC}` : "none", opacity: s.settled ? 0.45 : 1, animation: "evRowIn 0.35s ease both", animationDelay: `${Math.min(si, 8) * 0.04}s` }}><div style={{ width: 34, height: 34, borderRadius: "50%", background: s.settled || s.direction !== "owe" ? "rgba(90,122,90,0.1)" : "rgba(192,82,74,0.1)", border: `1px solid ${s.settled || s.direction !== "owe" ? "rgba(90,122,90,0.25)" : "rgba(192,82,74,0.25)"}`, display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0, color: s.settled || s.direction !== "owe" ? greenC : redC }}>{s.settled ? <CheckCircle size={16} weight="fill" /> : s.direction === "owe" ? <ArrowUpRight size={15} /> : <ArrowDownLeft size={15} />}</div><div style={{ flex: 1, minWidth: 0 }}><div style={{ fontSize: 13.5, fontWeight: 500, color: inkC }}>{s.name}</div><div style={{ fontSize: 10.5, color: mutC, marginTop: 1 }}>{s.settled ? "Settled" : s.direction === "owe" ? "You owe" : "Owes you"}</div>{s.note && <div style={{ fontSize: 10, color: mutC, marginTop: 1, fontStyle: "italic", whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis" }}>{s.note}</div>}</div><span style={{ fontFamily: "'Playfair Display', Georgia, serif", fontSize: 15, color: s.direction === "owe" ? redC : greenC, flexShrink: 0 }}>{fmt(s.amount)}</span>{!s.settled && <button className="evPress" onClick={() => sSTgt(s)} style={{ border: "1px solid rgba(184,150,62,0.4)", background: "rgba(184,150,62,0.08)", borderRadius: 20, padding: "4px 11px", fontSize: 10, fontWeight: 600, letterSpacing: "0.5px", color: goldC, cursor: "pointer", flexShrink: 0 }}>Settle</button>}<button onClick={() => oDS(s.id)} style={{ background: "none", border: "none", color: mutC, cursor: "pointer", opacity: 0.5, flexShrink: 0, padding: 0, display: "flex" }}><IconX size={14} /></button></div>)}</div>}
      ...
      {sel.status === "active" && !isGroup && (() => {
        ...
        if (bsStep === 3) return <div style={{ ...wcc, padding: 24, marginBottom: 20, textAlign: "center" }}><div style={{ display: "flex", justifyContent: "center", marginBottom: 8, color: greenC }}><CheckCircle size={34} weight="fill" /></div><div style={{ fontSize: 14, color: greenC, fontWeight: 600 }}>Split recorded</div><div style={{ fontSize: 12, color: mutC, marginTop: 4 }}>Full bill paid. Your final share is {fmt(myShare)}.</div></div>;
        if (bsStep === 2) { const cat = cats.find(c => c.id === bsCat) || cats[0]; return <div style={{ ...wcc, padding: 16, marginBottom: 20 }}><div style={{ ...secLbl, padding: "0 0 12px" }}><span style={{ display: "inline-flex", alignItems: "center", gap: 6 }}><Receipt size={11} />Confirm Split</span></div><div style={{ background: "rgba(196,96,58,0.07)", borderRadius: 10, padding: "12px 14px", marginBottom: 10, border: "1px solid rgba(196,96,58,0.25)" }}><div style={{ fontSize: 10, color: mutC, fontWeight: 600, letterSpacing: "1px", textTransform: "uppercase", marginBottom: 6 }}>Paid Now</div><div style={{ display: "flex", alignItems: "center", gap: 10 }}><DI2 id={cat?.id} accent={cat?.neon || cat?.color} size={20} /><div style={{ flex: 1, fontSize: 13, fontWeight: 600, color: inkC }}>Full bill from {wl.find(w => w.id === bsW)?.name || "wallet"}</div><div style={{ fontSize: 18, fontFamily: "'Playfair Display', Georgia, serif", color: terraC }}>−{fmt(totalNum)}</div></div></div><div style={{ background: "rgba(90,122,90,0.07)", borderRadius: 10, padding: "10px 14px", marginBottom: 14, border: "1px solid rgba(90,122,90,0.25)" }}><div style={{ display: "flex", justifyContent: "space-between", fontSize: 12, color: inkC }}><span>Your final share</span><span style={{ fontWeight: 700, color: terraC }}>{fmt(myShare)}</span></div></div><div style={{ marginBottom: 14 }}><div style={{ fontSize: 10, color: mutC, fontWeight: 600, letterSpacing: "1px", textTransform: "uppercase", marginBottom: 8 }}>They Owe You</div>{validPpl.map((p, i) => { const amt = bsMode === "equal" ? eqOthers[i] : roundMoney((Number.isFinite(parseAmount(p.amount)) ? parseAmount(p.amount) : 0)); return <div key={i} style={{ display: "flex", justifyContent: "space-between", padding: "7px 0", borderBottom: `1px solid ${brdC}` }}><span style={{ fontSize: 13, color: inkC }}>{p.name}</span><span style={{ fontSize: 13, fontWeight: 600, color: greenC }}>{fmt(amt)}</span></div> })}</div><div style={{ display: "flex", gap: 8 }}><button onClick={() => sBsS(1)} style={{ flex: 1, padding: 12, border: `1px solid ${brdC}`, borderRadius: 10, background: "transparent", color: mutC, fontSize: 13, cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center", gap: 4 }}><CaretLeft size={13} />Edit</button><button className="evPress" onClick={bsSubmit} style={{ flex: 2, padding: 12, border: "none", borderRadius: 10, background: greenC, color: "#fff", fontSize: 13, fontWeight: 700, cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center", gap: 5, boxShadow: "0 2px 8px rgba(90,122,90,0.3)" }}><IconCheck size={14} />Confirm</button></div></div> }
        return <div style={{ ...wcc, padding: 16, marginBottom: 20 }}><div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 12 }}><div style={{ ...secLbl, padding: 0 }}><span style={{ display: "inline-flex", alignItems: "center", gap: 6 }}><Receipt size={11} />Bill Splitter</span></div><button onClick={bsReset} style={{ background: "none", border: "none", color: mutC, cursor: "pointer", display: "flex", padding: 0 }}><IconX size={14} /></button></div>
