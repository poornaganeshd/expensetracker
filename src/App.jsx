...
`}</style>
    ...
    {module === "finance" && <div style={{ padding: "0 16px" }}>
      ...
      {tab === "dashboard" && <div className="pe">
        ...
        <div style={{ ...cc, padding: "18px 14px", marginBottom: 16, position: "relative", overflow: "hidden" }}><div style={{ position: "absolute", bottom: 0, right: 0, width: 60, height: 3, borderRadius: "3px 0 0 0", background: "#d4af37" }} /><div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 12, gap: 12 }}><div style={{ fontFamily: "var(--font-h)", fontSize: 12, color: "var(--text)", letterSpacing: "0.04em", fontWeight: 600 }}>Trend</div><div style={{ display: "flex", justifyContent: "flex-end", background: "rgba(255,255,255,0.02)", borderRadius: 999, padding: 2, border: "1px solid rgba(255,255,255,0.06)", gap: 2 }}>{["Day","Week","Month","Year"].map(p => { const v = p.toLowerCase(); return <button key={v} onClick={() => sTrendPeriod(v)} style={{ padding: "3px 9px", borderRadius: 999, fontSize: 10, fontFamily: "var(--font-h)", fontWeight: 500, letterSpacing: "0.02em", border: "none", cursor: "pointer", background: trendPeriod === v ? "rgba(212,175,55,0.16)" : "transparent", color: trendPeriod === v ? "#d4af37" : "var(--muted)", transition: "all 0.15s" }}>{p}</button>; })}</div></div><div style={{ overflowX: "auto", width: "100%" }}><TrendChart expenses={ex} incomes={inc} categories={cats} period={trendPeriod} /></div></div>
