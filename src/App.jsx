...
import { flushSyncQueue, getPendingSyncCount, getDeadLetterCount, clearDeadLetter, sendSupabaseRequest, subscribePendingSync, subscribeSyncDrops } from "./offlineSync";
...
function Splits({ splits: sp, onAdd, onSettle: os, onDelete: od, expanded: exp, onToggle: ot }) {
  const [nm, sN] = useState(""), [am, sA] = useState(""), [dir, sD] = useState("owe"), [snote, sSnote] = useState(""), [st, sT] = useState(null);
  const add = () => { if (!nm.trim() || !am || Number(am) <= 0) return; onAdd({ id: uid(), name: nm.trim(), amount: Number(am), direction: dir, settled: false, note: snote.trim() || undefined }); sN(""); sA(""); sSnote("") };
  ...
  return <div style={{ ...cc, borderRadius: 16, padding: 18, marginBottom: 14, position: "relative", overflow: "hidden" }}><div style={{ position: "absolute", bottom: 0, right: 0, width: 60, height: 3, borderRadius: "3px 0 0 0", background: "#D4726A" }} /><div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 4 }}><div style={{ fontFamily: "var(--font-h)", fontSize: 12, color: "#D4726A", letterSpacing: "0.5px", fontWeight: 700 }}>Split Expenses</div><button onClick={ot} style={{ background: "none", border: "none", fontSize: 12, color: "var(--muted)", cursor: "pointer", fontFamily: "var(--font-h)" }}>← Back</button></div><div style={{ fontSize: 10, color: "var(--muted)", fontFamily: "var(--font-b)", marginBottom: 12 }}>Track who owes who for informal splits. For group trips or events use the Events tab.</div>
    ...
    {sp.filter(s => !s.settled).map(s => <div key={s.id} style={{ display: "flex", alignItems: "center", gap: 10, padding: "10px 12px", background: "var(--bg)", borderRadius: 10, marginBottom: 6, border: "1px solid var(--border)" }}><span style={{ fontSize: 16 }}>{s.direction === "owe" ? "🔴" : "🟢"}</span><div style={{ flex: 1 }}><div style={{ fontSize: 13, fontWeight: 600, fontFamily: "var(--font-h)", color: "var(--text)" }}>{s.name}</div><div style={{ fontSize: 11, color: "var(--muted)", fontFamily: "var(--font-b)" }}>{s.direction === "owe" ? "You owe" : "Owes you"}{s.note && <span style={{ marginLeft: 5, opacity: 0.7 }}>· {s.note}</span>}</div></div><span style={{ fontFamily: "var(--font-h)", fontWeight: 600, fontSize: 14, color: s.direction === "owe" ? "#E07A5F" : "#6BAA75" }}>{fmt(s.amount)}</span><button onClick={() => sT(s)} style={{ border: "1px solid var(--border)", background: "none", borderRadius: 6, padding: "4px 8px", fontSize: 10, color: "var(--muted)", cursor: "pointer" }}>Settle</button><button onClick={() => od(s.id)} style={{ background: "none", border: "none", color: "var(--muted)", cursor: "pointer", fontSize: 14, opacity: 0.4 }}>✕</button></div>)}
    ...
    <div style={{ borderTop: "1px solid var(--border)", marginTop: 14, paddingTop: 14 }}><div style={{ display: "flex", gap: 6, marginBottom: 10 }}>{["owe", "owed"].map(d => <button key={d} onClick={() => sD(d)} style={{ flex: 1, padding: "8px", borderRadius: 8, fontSize: 12, fontFamily: "var(--font-h)", border: `1.5px solid ${dir === d ? (d === "owe" ? "#E07A5F" : "#6BAA75") : "var(--border)"}`, background: dir === d ? (d === "owe" ? "#E07A5F18" : "#6BAA7518") : "var(--card)", color: dir === d ? (d === "owe" ? "#E07A5F" : "#6BAA75") : "var(--muted)", cursor: "pointer", fontWeight: 500 }}>{d === "owe" ? "I owe them" : "They owe me"}</button>)}</div><div style={{ display: "flex", gap: 6, marginBottom: 6 }}><input value={nm} onChange={e => sN(e.target.value)} placeholder="Friend name" style={{ ...is, flex: 1, marginBottom: 0 }} /><input type="number" value={am} onChange={e => sA(e.target.value)} placeholder="₹" style={{ ...is, width: 80, marginBottom: 0 }} /><button onClick={add} style={{ padding: "10px 14px", border: "none", borderRadius: 10, background: "#E07A5F", color: "#fff", fontFamily: "var(--font-h)", fontSize: 13, fontWeight: 600, cursor: "pointer" }}>+</button></div><input value={snote} onChange={e => sSnote(e.target.value)} placeholder="Note (what's this for?)" style={{ ...is, marginBottom: 0 }} /></div>
...
function AddPage({ categories: cats, incomeSources: isrc, recurringCats: rCats, onAddExpense: oE, onAddIncome: oI, onAddTransfer: oT, onAddRec: oR, onError: showT = () => {}, patterns = [], autoRules = [], onLearnRule = () => {}, wallets: aw = WALLETS }) {
  ...
  return <div style={{ padding: "0 0 20px" }}>
    ...
    {type !== "recurring" && <><div style={{ marginBottom: 16 }}><label style={ls}>Amount</label>
      ...
      {aiCatLoading && <div style={{ fontSize: 11, color: "var(--muted)", marginBottom: 12, display: "flex", alignItems: "center", gap: 5 }}><span style={{ animation: "spin 1s linear infinite", display: "inline-block" }}>⏳</span> AI suggesting category…</div>}
      {aiCatSug && (() => { const c = cats.find(x => x.id === aiCatSug.categoryId); if (!c) return null; return <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 12, padding: "7px 10px", borderRadius: 8, background: (c.color || "#6BAA75") + "12", border: `1px solid ${c.color || "#6BAA75"}` }}><DI2 id={c.id} accent={c.neon || c.color} size={14} /><span style={{ flex: 1, fontSize: 12, fontFamily: "var(--font-h)", fontWeight: 600, color: "var(--text)" }}>🤖 {c.name}?</span><span style={{ fontSize: 9, color: c.color, background: (c.color || "#6BAA75") + "20", padding: "1px 5px", borderRadius: 4, fontWeight: 700 }}>{aiCatSug.confidence}</span><button onClick={() => { sCat(aiCatSug.categoryId); onLearnRule({ keyword: aiCatSug.keyword, categoryId: aiCatSug.categoryId, source: "ai", confidence: 0.9, hitCount: 0, createdAt: localDateKey() }); sAiCatSug(null); }} style={{ padding: "4px 10px", borderRadius: 6, border: "none", background: c.color || "#6BAA75", color: "#fff", fontFamily: "var(--font-h)", fontSize: 11, fontWeight: 700, cursor: "pointer" }}>✓</button><button onClick={() => sAiCatSug(null)} style={{ padding: "4px 8px", borderRadius: 6, border: "1px solid var(--border)", background: "var(--bg)", color: "var(--muted)", fontFamily: "var(--font-h)", fontSize: 11, cursor: "pointer" }}>✗</button></div>; })()}
...
const TxCard = memo(function TxCard({ item: it, categories: cats, incomeSources: isrc, events: evs, onDelete: od, recurringCats: rCats, wallets: wl = WALLETS, onRefund: oRef }) {
  ...
  if (isS) { const sW = wl.find(x => x.id === it.walletId); return <div style={{ ...cc, borderRadius: 14, padding: "14px 16px", display: "flex", alignItems: "center", gap: 12, marginBottom: 10 }}><div style={{ width: 44, height: 44, borderRadius: 12, background: it.direction === "owed" ? "#6BAA7514" : "#E07A5F14", display: "flex", alignItems: "center", justifyContent: "center" }}><DI2 id={it.direction === "owed" ? "received" : "paid"} accent={it.direction === "owed" ? "#6BAA75" : "#E07A5F"} size={22} /></div><div style={{ flex: 1, minWidth: 0 }}><div style={{ fontSize: 14, fontWeight: 600, color: "var(--text)", fontFamily: "var(--font-h)" }}>{it.direction === "owed" ? `${it.splitName} paid back` : `Paid ${it.splitName}`}</div><div style={{ fontSize: 12, color: "var(--muted)", fontFamily: "var(--font-b)", marginTop: 2, overflow: "hidden", whiteSpace: "nowrap", textOverflow: "ellipsis" }}>{sW?.name} · {dl(it.date)}</div></div><div style={{ fontFamily: "var(--font-h)", fontWeight: 600, fontSize: 15, color: it.direction === "owed" ? "#6BAA75" : "#E07A5F" }}>{it.direction === "owed" ? "+" : "−"}{fmt(it.amount)}</div><button onClick={() => od(it.id, it.type)} style={{ background: "none", border: "none", color: "var(--muted)", cursor: "pointer", fontSize: 14, opacity: 0.35 }}>✕</button></div> }
  return <div style={{ ...cc, borderRadius: 14, padding: "14px 16px", display: "flex", alignItems: "center", gap: 12, marginBottom: 10 }}><div style={{ width: 44, height: 44, borderRadius: 12, background: (cat?.color || "#999") + "14", display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0 }}>{cat ? <DI2 id={cat.id} accent={cat.neon || cat.color} size={22} /> : <span style={{ fontSize: 22 }}>❓</span>}</div><div style={{ flex: 1, minWidth: 0, overflow: "hidden" }}><div style={{ display: "flex", alignItems: "center", gap: 6, flexWrap: "wrap" }}><span style={{ fontSize: 14, fontWeight: 600, color: "var(--text)", fontFamily: "var(--font-h)" }}>{cat?.name || "Unknown"}</span>{isE && <span style={{ fontSize: 7, fontFamily: "var(--font-h)", fontWeight: 600, color: isFix(it) ? "#A78BFA" : "#FBBF24", background: isFix(it) ? "#A78BFA15" : "#FBBF2415", padding: "1px 5px", borderRadius: 3 }}>{isFix(it) ? "FIXED" : "FLEX"}</span>}{w && <span style={{ fontSize: 9, fontFamily: "var(--font-h)", fontWeight: 600, color: w.color, background: w.color + "18", padding: "2px 6px", borderRadius: 4, display: "inline-flex", alignItems: "center", gap: 2 }}><DI2 id={w.id} accent={w.neon || w.color} size={10} /></span>}</div><div style={{ fontSize: 12, color: "var(--muted)", fontFamily: "var(--font-b)", marginTop: 2, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{evT && <span style={{ fontWeight: 600, color: "var(--ts)" }}>{evT} · </span>}{dl(it.date)}{it.note ? " · " + it.note : ""}</div>{fxMeta && <div style={{ fontSize: 10, color: "#7B8CDE", fontFamily: "var(--font-h)", fontWeight: 600, marginTop: 3, letterSpacing: "0.3px" }}>{fxMeta.currency} {fxMeta.originalAmount} @ {Number(fxMeta.rateUsed).toFixed(2)}</div>}
    {(isE || isI) && it.receipt_url && (() => { let urls; try { urls = JSON.parse(it.receipt_url); if (!Array.isArray(urls)) urls = [it.receipt_url]; } catch { urls = [it.receipt_url]; } return <div style={{ marginTop: 3, display: "flex", gap: 8, flexWrap: "wrap" }}>{urls.map((u, i) => { const isPdf = typeof u === "string" && (u.startsWith("data:application/pdf") || u.toLowerCase().endsWith(".pdf")); return <a key={i} href={u} target="_blank" rel="noopener noreferrer" style={{ fontSize: 10, color: "#6BAA75", fontFamily: "var(--font-h)", fontWeight: 600, textDecoration: "none" }}>{isPdf ? "📄" : "🧾"} {urls.length > 1 ? `${isPdf ? "PDF" : "Receipt"} ${i + 1}` : (isPdf ? "PDF" : "Receipt")}</a>; })}</div>; })()}</div><div style={{ fontFamily: "var(--font-h)", fontWeight: 600, fontSize: 15, color: isE ? "#E07A5F" : "#6BAA75", flexShrink: 0 }}>{isE ? "−" : "+"}{fmt(it.amount)}</div><button onClick={() => od(it.id, it.type)} style={{ background: "none", border: "none", color: "var(--muted)", cursor: "pointer", fontSize: 14, opacity: 0.35, flexShrink: 0 }}>✕</button></div>
});
...
function NI({ type: t, active: a }) {
  ...
  if (t === "calendar") return <svg width="22" height="22" viewBox="0 0 24 24" fill="none" stroke={c} strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><rect x="3" y="4" width="18" height="18" rx="2" /><line x1="16" y1="2" x2="16" y2="6" /><line x1="8" y1="2" x2="8" y2="6" /><line x1="3" y1="10" x2="21" y2="10" /></svg>;
...
export default function Nomad() {
  ...
  const [smsSetupOpen, sSmsSetupOpen] = useState(false);
  ...
  const [aiOpen, sAiOpen] = useState(true);
  ...
  const [stgAp, sStgAp] = useState(true), [stgAu, sStgAu] = useState(false), [stgDt, sStgDt] = useState(false), [stgIn, sStgIn] = useState(false), [stgDz, sStgDz] = useState(false);
  ...
  const [drafts, sDrafts] = useState([]);
  const [draftReviewOpen, sDraftReviewOpen] = useState(false);
  const [draftCatOv, sDraftCatOv] = useState({});
  const [calDay, sCalDay] = useState(null); // selected day in calendar view (YYYY-MM-DD)
  const [calViewYear, sCalViewYear] = useState(() => new Date().getFullYear());
  const [calViewMonth, sCalViewMonth] = useState(() => new Date().getMonth());
  ...
  useEffect(() => {
    const load = async () => {
      ...
      try {
        const [dbEx, dbInc, dbTr, dbStl, dbSp, dbRec, dbWsb, dbEvs, dbDrafts] = await Promise.all([
        ...
        if (dbDrafts) sDrafts(dbDrafts.filter(d => d.status === "pending"));
          ...
          try {
            ...
            if (localRaw) {
              ...
              if (hasLocal) {
                ...
                if (exL.length) sbUpsert("expenses", exL.map(e => toSB(e, ["id", "amount", "categoryId", "walletId", "note", "date", "eventId", "groupId", "paidBy"])));
                ...
                if (evsL.length) sbUpsert("events", evsL.map(e => toSB(e, ["id", "name", "emoji", "date", "status", "type", "participants"])));
  ...
  const undoDelete = (toastId) => {
    ...
    if (buf.type === "expense") {
      sEx(p => [buf.exp, ...p]);
      sbUpsert("expenses", [{ ...toSB(buf.exp, ["id", "amount", "categoryId", "walletId", "note", "date", "eventId", "groupId"]), deleted_at: null }]);
    ...
    } else if (buf.type === "income") { sInc(p => [buf.exp, ...p]); sbUpsert("incomes", [{ ...toSB(buf.exp, ["id", "amount", "sourceId", "walletId", "note", "date"]), deleted_at: null }]); }
    else if (buf.type === "transfer") { sTr(p => [buf.exp, ...p]); sbUpsert("transfers", [{ ...toSB(buf.exp, ["id", "amount", "fromWallet", "toWallet", "note", "date"]), deleted_at: null }]); }
    ...
    else if (buf.type === "event") { sEvs(p => [buf.exp, ...p]); sbUpsert("events", [{ ...toSB(buf.exp, ["id", "name", "emoji", "date", "status", "type", "participants"]), deleted_at: null }]); }
  ...
  const parseBankCsv = (text) => {
    ...
    for (let i = 1; i < lines.length; i++) {
      ...
      const vpaMatch = (note || "").match(/\b[\w.+-]+@[a-zA-Z0-9]+\b/);
      const vpa = vpaMatch ? vpaMatch[0] : null;
      const isPersonPayment = !!vpa && (/^\d{10}@/.test(vpa) || /^[a-z]+@(paytm|ybl|okhdfcbank|okicici|oksbi|axisbank)$/.test(vpa));
      const matchedRule = autoRules.find(r => (note || "").toLowerCase().includes(r.keyword));
      const suggested_category_id = matchedRule ? matchedRule.categoryId : null;
      if (debit > 0) rows.push({ date: parsedDate, amount: debit, note, type: "expense", vpa, is_person_payment: isPersonPayment, suggested_category_id });
      else if (credit > 0) rows.push({ date: parsedDate, amount: credit, note, type: "income", vpa, is_person_payment: isPersonPayment, suggested_category_id });
      else if (generic > 0) rows.push({ date: parsedDate, amount: generic, note, type: "expense", vpa, is_person_payment: isPersonPayment, suggested_category_id });
    }
    return rows;
  };
  const impCsv = (file) => { const r = new FileReader(); r.onload = e => { const rows = parseBankCsv(e.target.result); if (rows.length === 0) { showT("No valid rows found — check CSV format", "error"); return; } sCsvPreview(rows); showT(`Parsed ${rows.length} rows — review and save to drafts`, "info"); }; r.readAsText(file); };
  const confirmCsvImport = () => { if (!csvPreview?.length) return; const existingKeys = new Set(ex.map(e => `${e.date}:${e.amount}`)); const newDrafts = csvPreview.filter(row => !existingKeys.has(`${row.date}:${row.amount}`)).map(row => ({ id: uid(), source: "csv", raw_text: row.note, date: row.date, amount: row.amount, type: row.type, merchant: row.note, vpa: row.vpa || null, suggested_category_id: row.suggested_category_id || (row.type === "expense" ? cats[0]?.id || "food" : null), suggested_wallet_id: "bank", is_person_payment: row.is_person_payment || false, status: "pending", created_at: new Date().toISOString() })); const dupeCount = csvPreview.length - newDrafts.length; if (newDrafts.length === 0) { showT(`All ${csvPreview.length} rows already exist — no drafts created`, "info"); sCsvPreview(null); return; } sDrafts(p => [...p, ...newDrafts]); if (SB_ENABLED) sbUpsert("drafts", newDrafts.map(d => toSB(d, ["id", "source", "raw_text", "date", "amount", "type", "merchant", "vpa", "suggested_category_id", "suggested_wallet_id", "is_person_payment", "status"]))); sCsvPreview(null); sDraftReviewOpen(true); showT(`${newDrafts.length} drafts saved${dupeCount > 0 ? ` (${dupeCount} skipped — duplicate)` : ""} — review on dashboard`, "success"); };
  const confirmDraft = (draft, overrides = {}) => { const catId = overrides.categoryId || draft.suggested_category_id || cats[0]?.id || "food"; const walId = overrides.walletId || draft.suggested_wallet_id || "bank"; const noteTxt = (draft.merchant || draft.raw_text || "").slice(0, 500); if (draft.type === "income") { addI({ id: uid(), amount: draft.amount, sourceId: overrides.sourceId || isrc[0]?.id || "allowance", walletId: walId, date: draft.date, note: noteTxt }); } else { addE({ id: uid(), amount: draft.amount, categoryId: catId, walletId: walId, date: draft.date, note: noteTxt }); } sDrafts(p => p.filter(d => d.id !== draft.id)); if (SB_ENABLED) sbWrite(`${SB_URL}/rest/v1/drafts?id=eq.${draft.id}`, { method: "PATCH", body: { status: "confirmed" } }); };
  const dismissDraft = (id) => { sDrafts(p => p.filter(d => d.id !== id)); if (SB_ENABLED) sbWrite(`${SB_URL}/rest/v1/drafts?id=eq.${id}`, { method: "PATCH", body: { status: "dismissed" } }); };
  const loadRecentlyDeleted = async () => { sRecDelLoading(true); const [dEx, dInc, dTr, dRec, dEvs] = await Promise.all([sbGetDeleted("expenses"), sbGetDeleted("incomes"), sbGetDeleted("transfers"), sbGetDeleted("recurring"), sbGetDeleted("events")]); const all = [...(dEx || []).map(i => ({ ...i, _tbl: "expenses" })), ...(dInc || []).map(i => ({ ...i, _tbl: "incomes" })), ...(dTr || []).map(i => ({ ...i, _tbl: "transfers" })), ...(dRec || []).map(i => ({ ...i, _tbl: "recurring" })), ...(dEvs || []).map(i => ({ ...i, _tbl: "events" }))].sort((a, b) => (b.deleted_at || "").localeCompare(a.deleted_at || "")); sRecDelItems(all); sRecDelLoading(false); };
  const restoreDeleted = (item) => { const { _tbl, ...row } = item; sbWrite(`${SB_URL}/rest/v1/${_tbl}?id=eq.${row.id}`, { method: "PATCH", body: { deleted_at: null } }); const clean = { ...row, deleted_at: null }; if (_tbl === "expenses") sEx(p => [...p, clean]); else if (_tbl === "incomes") sInc(p => [...p, clean]); else if (_tbl === "transfers") sTr(p => [...p, clean]); else if (_tbl === "recurring") sRec(p => [...p, clean]); else if (_tbl === "events") sEvs(p => [...p, clean]); sRecDelItems(p => p ? p.filter(i => i.id !== row.id) : p); showT("Restored", "success"); };
...
`}</style>
    ...
    {module === "finance" && <div style={{ padding: "0 16px" }}>
      ...
      {tab === "dashboard" && <div className="pe">
        ...
        {drafts.length > 0 && <div style={{ ...cc, padding: "14px 16px", marginBottom: 14, borderLeft: "3px solid #c9a96e" }}><div onClick={() => sDraftReviewOpen(v => !v)} style={{ display: "flex", alignItems: "center", justifyContent: "space-between", cursor: "pointer" }}><div style={{ display: "flex", alignItems: "center", gap: 8 }}><span style={{ fontSize: 16 }}>📥</span><div style={{ fontFamily: "var(--font-h)", fontSize: 13, fontWeight: 700, color: "#c9a96e" }}>{drafts.length} transaction{drafts.length === 1 ? "" : "s"} to review</div></div><span style={{ fontSize: 11, color: "#c9a96e", fontWeight: 700 }}>{draftReviewOpen ? "▲" : "▼"}</span></div>{draftReviewOpen && <div style={{ marginTop: 12 }}>{drafts.map(d => { const catId = draftCatOv[d.id] || d.suggested_category_id || (d.type === "expense" ? cats[0]?.id : null); const catName = cats.find(c => c.id === catId)?.name || "Uncategorized"; return <div key={d.id} style={{ padding: "10px 12px", borderRadius: 10, background: "var(--bg)", border: "1px solid var(--border)", marginBottom: 8 }}><div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 6 }}><div style={{ flex: 1, minWidth: 0 }}><div style={{ fontFamily: "var(--font-h)", fontSize: 12, fontWeight: 700, color: "var(--text)", overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{(d.merchant || d.raw_text || "—").slice(0, 40)}</div><div style={{ fontSize: 10, color: "var(--muted)", marginTop: 2 }}>{d.date} · {d.type === "income" ? "Income" : "Expense"}{d.is_person_payment ? " · 👤 UPI person" : ""}</div></div><div style={{ fontFamily: "var(--font-h)", fontSize: 14, fontWeight: 700, color: d.type === "income" ? "#6BAA75" : "#E07A5F", flexShrink: 0, marginLeft: 8 }}>{d.type === "income" ? "+" : "−"}{fmt(d.amount)}</div></div>{d.type === "expense" && <div style={{ marginBottom: 8 }}><select value={catId || ""} onChange={e => sDraftCatOv(p => ({ ...p, [d.id]: e.target.value }))} style={{ width: "100%", padding: "6px 8px", borderRadius: 8, border: "1px solid var(--border)", background: "var(--card)", color: "var(--text)", fontSize: 11, fontFamily: "var(--font-h)" }}>{cats.map(c => <option key={c.id} value={c.id}>{c.name}</option>)}</select></div>}<div style={{ display: "flex", gap: 6 }}><button onClick={() => { confirmDraft(d, { categoryId: draftCatOv[d.id] || undefined }); sDraftCatOv(p => { const n = { ...p }; delete n[d.id]; return n; }); }} style={{ flex: 1, padding: "7px", border: "none", borderRadius: 8, background: "#6BAA75", color: "#fff", fontFamily: "var(--font-h)", fontSize: 11, fontWeight: 700, cursor: "pointer" }}>✓ Confirm</button>{d.is_person_payment && <button onClick={() => { const amt = d.amount; const tid = uid(); const walId = d.suggested_wallet_id || "bank"; const namePart = (d.merchant || d.raw_text || "").slice(0, 30); settle({ id: tid, amount: amt, splitName: namePart, direction: d.type === "income" ? "owed" : "owe", walletId: walId, date: d.date, note: "From draft: " + namePart }); dismissDraft(d.id); sDraftCatOv(p => { const n = { ...p }; delete n[d.id]; return n; }); showT("Marked as settlement — " + fmt(amt), "success"); }} style={{ flex: 1, padding: "7px", border: "1.5px solid #7B8CDE", borderRadius: 8, background: "#7B8CDE18", color: "#7B8CDE", fontFamily: "var(--font-h)", fontSize: 11, fontWeight: 700, cursor: "pointer" }}>↔ Settle</button>}<button onClick={() => { dismissDraft(d.id); sDraftCatOv(p => { const n = { ...p }; delete n[d.id]; return n; }); }} style={{ padding: "7px 10px", border: "1.5px solid var(--border)", borderRadius: 8, background: "var(--card)", color: "var(--muted)", fontFamily: "var(--font-h)", fontSize: 11, cursor: "pointer" }}>✕</button></div></div>; })}<div style={{ display: "flex", gap: 8, marginTop: 8, paddingTop: 8, borderTop: "1px solid var(--border)" }}><button onClick={() => { drafts.forEach(d => confirmDraft(d, { categoryId: draftCatOv[d.id] || undefined })); sDraftCatOv({}); }} style={{ flex: 1, padding: "8px", border: "none", borderRadius: 9, background: "#6BAA75", color: "#fff", fontFamily: "var(--font-h)", fontSize: 12, fontWeight: 700, cursor: "pointer" }}>Confirm All ({drafts.length})</button><button onClick={() => { drafts.forEach(d => dismissDraft(d.id)); sDraftCatOv({}); }} style={{ flex: 1, padding: "8px", border: "1.5px solid var(--border)", borderRadius: 9, background: "var(--card)", color: "var(--muted)", fontFamily: "var(--font-h)", fontSize: 12, fontWeight: 600, cursor: "pointer" }}>Dismiss All</button></div></div>}</div>}
        ...
        {(() => { const sl = scoreLabel(finScore.score); return <div style={{ ...cc, padding: "10px 16px", marginBottom: 12, display: "flex", alignItems: "center", gap: 10 }}><div style={{ width: 44, height: 44, borderRadius: "50%", border: `2.5px solid ${sl.color}`, background: sl.color + "18", display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0 }}><span style={{ fontFamily: "var(--font-h)", fontSize: 15, fontWeight: 800, color: sl.color }}>{finScore.score}</span></div><div style={{ flex: 1 }}><div style={{ fontFamily: "var(--font-h)", fontSize: 12, fontWeight: 700, color: "var(--text)" }}>Health Score — {sl.label}</div><div style={{ fontSize: 10, color: "var(--muted)", marginTop: 2 }}>Savings · Bills · Spread · Logging</div></div>{finStreak > 1 && <div style={{ display: "flex", alignItems: "center", gap: 4, padding: "4px 8px", borderRadius: 8, background: "#FBBF2415", border: "1px solid #FBBF24" }}><span style={{ fontSize: 14 }}>🔥</span><span style={{ fontFamily: "var(--font-h)", fontSize: 11, fontWeight: 700, color: "#FBBF24" }}>{finStreak}d</span></div>}</div>; })()}
        {(() => { const ninety = new Date(); ninety.setDate(ninety.getDate() - 90); const d90 = ninety.toISOString().slice(0, 10); const recentEx = ex.filter(e => String(e.date || "") >= d90); const recentInc = inc.filter(i => String(i.date || "") >= d90); const cm = localDateKey().slice(0, 7); const sl = scoreLabel(finScore.score); const generateInsights = async () => { sAiInsightsLoading(true); try { const body = { expenses: redactTransactions(recentEx), incomes: redactTransactions(recentInc), categories: cats.map(c => ({ id: c.id, name: c.name })), wallets: wallets.map(w => ({ id: w.id, name: w.name })), month: cm }; const r = await fetch("/api/ai-insights", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify(body) }); const data = await r.json(); if (!r.ok) throw new Error(data.error || "AI insights failed"); const result = { ...data, ts: Date.now() }; sAiInsights(result); try { localStorage.setItem("nomad-ai-insights", JSON.stringify(result)); } catch { /* quota */ } } catch (e) { showT(e.message || "AI insights unavailable", "error"); } finally { sAiInsightsLoading(false); } }; const typeColor = { warning: "#E07A5F", tip: "#6BAA75", pattern: "#7B8CDE", achievement: "#FBBF24" }; const typeEmoji = { warning: "⚠️", tip: "💡", pattern: "📊", achievement: "🏆" }; return <div style={{ ...cc, padding: "14px 16px", marginBottom: 14, borderLeft: `3px solid ${sl.color}` }}><div onClick={() => sAiOpen(o => !o)} style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: aiOpen && aiInsights ? 12 : 0, cursor: "pointer" }}><div style={{ display: "flex", alignItems: "center", gap: 8 }}><span style={{ fontSize: 16 }}>✨</span><div style={{ fontFamily: "var(--font-h)", fontSize: 12, color: sl.color, fontWeight: 700, letterSpacing: "0.5px" }}>AI Insights</div>{aiInsights && <span style={{ fontSize: 9, color: "var(--muted)", fontFamily: "var(--font-h)" }}>Updated {Math.round((Date.now() - aiInsights.ts) / 60000)}m ago</span>}</div><div style={{ display: "flex", alignItems: "center", gap: 6 }}><button onClick={e => { e.stopPropagation(); generateInsights(); }} disabled={aiInsightsLoading} style={{ padding: "5px 12px", borderRadius: 8, border: `1.5px solid ${sl.color}`, background: aiInsightsLoading ? "var(--border)" : sl.color + "18", color: sl.color, fontFamily: "var(--font-h)", fontSize: 11, fontWeight: 700, cursor: aiInsightsLoading ? "default" : "pointer" }}>{aiInsightsLoading ? "⏳ Thinking…" : aiInsights ? "Refresh" : "Generate"}</button><span style={{ fontSize: 10, color: "var(--muted)", transition: "transform 0.2s", display: "inline-block", transform: aiOpen ? "rotate(0deg)" : "rotate(-90deg)" }}>▾</span></div></div>{aiOpen && aiInsights && <><p style={{ fontSize: 12, color: "var(--ts)", lineHeight: 1.6, margin: "0 0 10px" }}>{aiInsights.summary}</p>{aiInsights.insights.map((ins, i) => <div key={i} style={{ padding: "8px 10px", borderRadius: 8, marginBottom: 6, background: (typeColor[ins.type] || "#999") + "12", borderLeft: `3px solid ${typeColor[ins.type] || "#999"}` }}><div style={{ display: "flex", alignItems: "center", gap: 6, marginBottom: 3 }}><span style={{ fontSize: 12 }}>{typeEmoji[ins.type] || "•"}</span><span style={{ fontFamily: "var(--font-h)", fontSize: 12, fontWeight: 700, color: "var(--text)" }}>{ins.title}</span><span style={{ marginLeft: "auto", fontSize: 9, fontFamily: "var(--font-h)", fontWeight: 600, color: typeColor[ins.type] || "#999", background: (typeColor[ins.type] || "#999") + "20", padding: "1px 5px", borderRadius: 4 }}>{ins.severity}</span></div><p style={{ fontSize: 11, color: "var(--ts)", margin: 0, lineHeight: 1.5 }}>{ins.detail}</p></div>)}</> }</div>; })()}
        <Splits splits={sp} expanded={spX} onToggle={() => sSpX(!spX)} onAdd={s => { sSp(p => [...p, s]); sbUpsert("splits", [toSB(s, ["id", "name", "amount", "direction", "settled", "eventId", "groupId"])]) }} onSettle={settle} onDelete={id => { sSp(p => p.filter(s => s.id !== id)); sbDeleteWhere("splits", `id=eq.${id}`) }} />
...
{historyItems.map(it => <div key={it.id} style={{ display: "flex", alignItems: "stretch", gap: 6 }}>{bulkMode && <div onClick={() => sBulkSel(p => { const n = new Set(p); n.has(it.id) ? n.delete(it.id) : n.add(it.id); return n; })} style={{ display: "flex", alignItems: "center", padding: "0 4px", cursor: "pointer", flexShrink: 0 }}><div style={{ width: 20, height: 20, borderRadius: 5, border: `2px solid ${bulkSel.has(it.id) ? "#E07A5F" : "var(--border)"}`, background: bulkSel.has(it.id) ? "#E07A5F" : "var(--card)", display: "flex", alignItems: "center", justifyContent: "center" }}>{bulkSel.has(it.id) && <span style={{ color: "#fff", fontSize: 11, fontWeight: 700 }}>✓</span>}</div></div>}<div style={{ flex: 1, minWidth: 0 }}><TxCard item={it} categories={cats} incomeSources={isrc} events={evs} onDelete={delItem} recurringCats={recCats} wallets={wallets} onRefund={refundItem} /></div></div>)}{historyItems.length === 0 && <div style={{ textAlign: "center", padding: "60px 20px", color: "var(--muted)", fontSize: 14, lineHeight: 1.8 }}>{flt.expenses.length === 0 && flt.incomes.length === 0 ? <><div style={{ fontSize: 32, marginBottom: 12 }}>📋</div><div style={{ fontFamily: "var(--font-h)", fontSize: 15, fontWeight: 600, color: "var(--ts)", marginBottom: 6 }}>No transactions yet</div><div style={{ fontSize: 12, marginBottom: 20 }}>Log expenses, income, and transfers<br />to see your spending history here.</div><button onClick={() => sTab("add")} style={{ padding: "12px 28px", border: "none", borderRadius: 12, background: "#E07A5F", color: "#fff", fontFamily: "var(--font-h)", fontSize: 13, fontWeight: 700, cursor: "pointer" }}>+ Add First Transaction</button></> : "No results match your filters."}</div>}</div>}
    ...
    {module === "finance" && (() => { const ninety = new Date(); ninety.setDate(ninety.getDate() - 90); const d90 = ninety.toISOString().slice(0, 10); const recentEx = ex.filter(e => String(e.date || "") >= d90); const recentInc = inc.filter(i => String(i.date || "") >= d90); const cm = localDateKey().slice(0, 7); const totalInc = recentInc.reduce((s, i) => s + i.amount, 0); const totalExp = recentEx.reduce((s, e) => s + e.amount, 0); const catTotals = {}; recentEx.forEach(e => { catTotals[e.categoryId] = (catTotals[e.categoryId] || 0) + e.amount; }); const topCats = Object.entries(catTotals).sort((a, b) => b[1] - a[1]).slice(0, 5).map(([id, amt]) => ({ name: cats.find(c => c.id === id)?.name || id, amount: amt, pct: totalExp > 0 ? Math.round(amt / totalExp * 100) : 0 })); const wBals = wallets.map(w => ({ name: w.name, balance: roundMoney(wBal[w.id] || 0) })); const sendChat = async (q) => { if (!q.trim() || chatLoading) return; const userMsg = { role: "user", content: q.trim() }; sChatMsgs(p => [...p, userMsg]); sChatInput(""); sChatLoading(true); try { const r = await fetch("/api/ai-chat", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ question: q.trim(), context: { month: cm, totalIncome: totalInc, totalExpense: totalExp, topCategories: topCats, recentExpenses: redactTransactions(recentEx).slice(0, 30).map(e => ({ date: e.date, amount: e.amount, category: cats.find(c => c.id === e.categoryId)?.name || e.categoryId || "Unknown", note: e.note || "" })), walletBalances: wBals, recurringCount: rec.filter(r => r.active !== false).length, streak: finStreak } }) }); const d = await r.json(); sChatMsgs(p => [...p, { role: "assistant", content: r.ok ? d.answer : (d.error || "Something went wrong.") }]); } catch { sChatMsgs(p => [...p, { role: "assistant", content: "Network error — check your connection." }]); } finally { sChatLoading(false); } }; const QUICK_QS = ["Where am I overspending?", "How's my savings rate?", "Any unusual spending?", "Can I afford a big purchase?"]; return <><button onClick={() => sChatOpen(v => !v)} style={{ position: "fixed", bottom: 72, right: 16, width: 48, height: 48, borderRadius: "50%", background: chatOpen ? "#7B8CDE" : (dm ? "#1a1a2e" : "#fff"), border: `2px solid #7B8CDE`, boxShadow: "0 4px 16px rgba(123,140,222,0.3)", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 22, cursor: "pointer", zIndex: 55, transition: "all 0.2s" }}>{chatOpen ? "✕" : "💬"}</button>{chatOpen && <div style={{ position: "fixed", bottom: 126, left: 0, right: 0, maxWidth: 430, margin: "0 auto", background: "var(--card)", borderRadius: "16px 16px 0 0", borderTop: "1px solid var(--border)", borderLeft: "1px solid var(--border)", borderRight: "1px solid var(--border)", zIndex: 54, display: "flex", flexDirection: "column", maxHeight: "55vh", boxShadow: "0 -8px 32px rgba(0,0,0,0.12)" }}><div style={{ padding: "14px 16px 10px", borderBottom: "1px solid var(--border)", display: "flex", alignItems: "center", gap: 8 }}><span style={{ fontSize: 16 }}>💬</span><div style={{ fontFamily: "var(--font-h)", fontSize: 13, fontWeight: 700, color: "#7B8CDE" }}>Ask NOMAD</div><button onClick={() => { sChatMsgs([]); sChatInput(""); }} style={{ marginLeft: "auto", background: "none", border: "none", color: "var(--muted)", cursor: "pointer", fontSize: 11, fontFamily: "var(--font-h)", fontWeight: 600 }}>Clear</button></div><div style={{ flex: 1, overflowY: "auto", padding: "10px 14px", display: "flex", flexDirection: "column", gap: 8 }}>{chatMsgs.length === 0 && <><p style={{ fontSize: 11, color: "var(--muted)", textAlign: "center", margin: "8px 0 12px", lineHeight: 1.5 }}>Ask anything about your finances.<br />Grounded in your last 90 days of data.</p><div style={{ display: "flex", flexDirection: "column", gap: 6 }}>{QUICK_QS.map(q => <button key={q} onClick={() => sendChat(q)} style={{ padding: "8px 12px", borderRadius: 8, border: "1px solid var(--border)", background: "var(--bg)", color: "var(--ts)", fontFamily: "var(--font-h)", fontSize: 12, cursor: "pointer", textAlign: "left" }}>{q}</button>)}</div></>}{chatMsgs.map((m, i) => <div key={i} style={{ alignSelf: m.role === "user" ? "flex-end" : "flex-start", maxWidth: "85%", padding: "8px 12px", borderRadius: m.role === "user" ? "12px 12px 2px 12px" : "12px 12px 12px 2px", background: m.role === "user" ? "#7B8CDE" : (dm ? "#1f1f2e" : "#f0f0f8"), color: m.role === "user" ? "#fff" : "var(--text)", fontSize: 12, fontFamily: "var(--font-b)", lineHeight: 1.6 }}>{m.content}</div>)}{chatLoading && <div style={{ alignSelf: "flex-start", padding: "8px 12px", borderRadius: "12px 12px 12px 2px", background: dm ? "#1f1f2e" : "#f0f0f8", color: "var(--muted)", fontSize: 12, fontFamily: "var(--font-b)" }}>⏳ Thinking…</div>}</div><div style={{ padding: "10px 12px", borderTop: "1px solid var(--border)", display: "flex", gap: 8 }}><input value={chatInput} onChange={e => sChatInput(e.target.value)} onKeyDown={e => { if (e.key === "Enter" && !e.shiftKey) { e.preventDefault(); sendChat(chatInput); } }} placeholder="Ask about your finances…" style={{ flex: 1, padding: "9px 12px", borderRadius: 10, border: "1.5px solid var(--border)", background: "var(--bg)", color: "var(--text)", fontSize: 12, fontFamily: "var(--font-b)", outline: "none" }} /><button onClick={() => sendChat(chatInput)} disabled={chatLoading || !chatInput.trim()} style={{ padding: "9px 14px", borderRadius: 10, border: "none", background: chatLoading || !chatInput.trim() ? "var(--border)" : "#7B8CDE", color: "#fff", fontFamily: "var(--font-h)", fontSize: 12, fontWeight: 700, cursor: chatLoading || !chatInput.trim() ? "not-allowed" : "pointer" }}>Send</button></div></div>}</>; })()}
