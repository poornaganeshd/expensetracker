...
function Events({ events: evs, expenses: ex, splits: sp, settlements: stl, categories: cats, wallets: wl = WALLETS, staleByEvent = {}, onCreate: oC, onAddExp: oE, onAddSplit: oS, onSettleSplit: oSS, onSettleEventNet: oSEN, onDeleteSplit: oDS, onSkipSplit: oSK = () => {}, onDeleteExp: oDE, onMarkDone: oMD, onReopen: oRO, onDelete: oD, onUpdate: oU, onToast: showT = () => {}, dm = false }) {
  ...
  const addExp = () => {
    ...
    // Resolve payer ("me" → "You") against the actual participants.
    ...
    // Build the split breakdown { name -> share } for a group expense. Default is
    // an equal split among everyone; the user can exclude people and/or switch to
    // exact custom amounts. Stored on the expense as splitWith so balances reflect
    // it (legacy expenses without splitWith stay equal-split).
    let splitWith = null;
    if (isGrp && partsList.length > 0) {
      const allP = ["You", ...partsList];
      const included = allP.filter(p => !eSplitExcl.has(p));
      if (included.length === 0) { showT("Pick at least one person to split with", "error"); return false; }
      if (eSplitMode === "custom") {
        ...
        if (Math.abs(sum - a) > 0.05) { showT(`Custom shares add up to ${fmt(sum)}, not ${fmt(a)}`, "error"); return false; }
        splitWith = map;
      } else {
    ...
    const expId = uid();
    const ok = oE({ id: expId, amount: a, categoryId: ec, walletId: ew, note: en, date: localDateKey(), eventId: sel.id, ...(paidByField ? { paidBy: paidByField } : {}), ...(splitWith ? { splitWith } : {}) });
    if (ok !== false) {
      if (isGrp && splitWith) {
        const noteBase = (en || cats.find(c => c.id === ec)?.name || "Group expense").slice(0, 400);
        if (payer === "You") {
          // You paid: everyone else included owes you their share.
          Object.entries(splitWith).forEach(([person, share]) => {
            if (person === "You" || !(share > 0)) return;
            oS({ id: uid(), name: person, amount: share, direction: "owed", settled: false, eventId: sel.id, groupId: expId, note: `Auto: ${noteBase}` });
          });
        } else {
          // Someone else paid: you owe them YOUR share (only your own debt is tracked).
          const yourShare = splitWith["You"] || 0;
          if (yourShare > 0) oS({ id: uid(), name: payer, amount: yourShare, direction: "owe", settled: false, eventId: sel.id, groupId: expId, note: `Auto: ${noteBase}` });
        }
      }
