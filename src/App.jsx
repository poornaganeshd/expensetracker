...
function Splits({ splits: sp, settlements: stl, onAdd, onSettle: os, onDelete: od, onSkip: ok, expanded: exp, onToggle: ot, wallets: wl = WALLETS }) {
  const [nm, sN] = useState(""), [am, sA] = useState(""), [dir, sD] = useState("owe"), [snote, sSnote] = useState(""), [st, sT] = useState(null), [drill, sDrill] = useState(null), [showFeed, sSF] = useState(false), [delConfirm, sDelConfirm] = useState(null), [settleAllWid, sSAW] = useState(null);
  ...
  if(exp&&showFeed){
    const feed=[...sp.filter(s=>!s.eventId&&!s.deleted_at).map(s=>({...s,_k:"split"})),...(stl||[]).filter(s=>!s.eventId).map(s=>({...s,_k:"stl"}))].sort((a,b)=>{const ta=a._k==="split"?(a.createdAt||a.id):(a.createdAt||a.date);const tb=b._k==="split"?(b.createdAt||b.id):(b.createdAt||b.date);return new Date(tb)-new Date(ta);});
  ...
  {activePeople.map(name=>{const pm=personMap[name];const net=pm.net;const pending=pm.splits.filter(s=>!s.settled);const aColor=avatarColor(name);return <div key={name} onClick={()=>sDrill(name)} style={{display:"flex",alignItems:"center",gap:12,padding:"13px 14px",background:"var(--bg)",borderRadius:14,marginBottom:8,border:`1px solid ${net>0?"#6BAA7530":"#E07A5F30"}`,cursor:"pointer",transition:"opacity 0.15s"}}><div style={{width:40,height:40,borderRadius:"50%",background:aColor,display:"flex",alignItems:"center",justifyContent:"center",flexShrink:0,boxShadow:`0 2px 8px ${aColor}40`}}><span style={{fontFamily:"var(--font-h)",fontSize:14,fontWeight:700,color:"#fff"}}>{initials(name)}</span></div><div style={{flex:1,minWidth:0}}><div style={{fontSize:14,fontWeight:700,fontFamily:"var(--font-h)",color:"var(--text)"}}>{name}</div><div style={{fontSize:11,color:"var(--muted)",fontFamily:"var(--font-b)",display:"flex",alignItems:"center",gap:4,marginBottom:pending.some(s=>s.note)?2:0}}>{net>0?<IconArrowUp size={11} color="#6BAA75"/>:<IconArrowDown size={11} color="#E07A5F"/>}{net>0?"Owes you":"You owe"}{pending.length>1&&<span style={{fontSize:9,background:"var(--border)",padding:"1px 5px",borderRadius:4,fontWeight:600}}>×{pending.length}</span>}</div>{pending.some(s=>s.note)&&<div style={{fontSize:10,color:"var(--ts)",fontFamily:"var(--font-b)",overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap",maxWidth:"100%",opacity:0.75}}>{pending.find(s=>s.note)?.note}</div>}</div><div style={{textAlign:"right",flexShrink:0}}><div style={{fontFamily:"var(--font-h)",fontWeight:800,fontSize:16,color:net>0?"#6BAA75":"#E07A5F"}}>{fmt(Math.abs(net))}</div><div style={{display:"flex",alignItems:"center",gap:4,justifyContent:"flex-end",marginTop:3}}><span style={{fontSize:9,color:"var(--muted)",fontFamily:"var(--font-h)"}}>tap for details</span><IconChevronRight size={12} color="var(--muted)"/></div></div></div>;})}
  {settledPeople.length>0&&<details style={{marginTop:4}}><summary style={{fontSize:11,color:"var(--muted)",cursor:"pointer",fontFamily:"var(--font-h)",fontWeight:500,marginBottom:6,display:"flex",alignItems:"center",gap:5}}><IconCheck size={11} color="#6BAA75"/>Settled up ({settledPeople.length})</summary>{settledPeople.map(name=><div key={name} onClick={()=>sDrill(name)} style={{display:"flex",alignItems:"center",gap:10,padding:"10px 13px",background:"var(--bg)",borderRadius:12,marginBottom:6,opacity:0.5,cursor:"pointer",border:"1px solid var(--border)"}}><div style={{width:32,height:32,borderRadius:"50%",background:avatarColor(name),display:"flex",alignItems:"center",justifyContent:"center",flexShrink:0}}><span style={{fontFamily:"var(--font-h)",fontSize:11,fontWeight:700,color:"#fff"}}>{initials(name)}</span></div><span style={{flex:1,fontSize:13,fontFamily:"var(--font-h)",color:"var(--ts)",fontWeight:600}}>{name}</span><div style={{display:"flex",alignItems:"center",gap:4,fontSize:11,color:"var(--muted)",fontFamily:"var(--font-h)"}}><IconCheck size={11} color="#6BAA75"/>All settled</div></div>)}</details>}
  <div style={{borderTop:"1px solid var(--border)",marginTop:14,paddingTop:14}}><div style={{fontFamily:"var(--font-h)",fontSize:10,color:"var(--muted)",fontWeight:600,marginBottom:8,letterSpacing:"0.5px"}}>ADD SPLIT IOU</div><div style={{display:"flex",gap:6,marginBottom:8}}>{["owe","owed"].map(d=><button key={d} onClick={()=>sD(d)} style={{flex:1,padding:"8px",borderRadius:8,fontSize:11,fontFamily:"var(--font-h)",border:`1.5px solid ${dir===d?(d==="owe"?"#E07A5F":"#6BAA75"):"var(--border)"}`,background:dir===d?(d==="owe"?"#E07A5F18":"#6BAA7518"):"var(--card)",color:dir===d?(d==="owe"?"#E07A5F":"#6BAA75"):"var(--muted)",cursor:"pointer",fontWeight:600,display:"flex",alignItems:"center",justifyContent:"center",gap:4}}>{d==="owe"?<><IconArrowDown size={12}/>I owe them</>:<><IconArrowUp size={12}/>They owe me</>}</button>)}</div><div style={{display:"flex",gap:6,marginBottom:6}}><input value={nm} onChange={e=>sN(e.target.value)} placeholder="Friend name" style={{...is,flex:1,marginBottom:0}}/><input type="number" value={am} onChange={e=>sA(e.target.value)} placeholder="₹" style={{...is,width:80,marginBottom:0}}/><button onClick={add} style={{padding:"10px 14px",border:"none",borderRadius:10,background:"#E07A5F",color:"#fff",fontFamily:"var(--font-h)",fontSize:13,fontWeight:600,cursor:"pointer",display:"flex",alignItems:"center",gap:4}}><IconPlus size={14}/>Add</button></div><input value={snote} onChange={e=>sSnote(e.target.value)} placeholder="Note (what's this for?)" style={{...is,marginBottom:0}}/></div>
  {st&&<SettleM split={st} remaining={remainForSplit(st)} wallets={wl} onConfirm={(wid,amount)=>{os(st.id,wid,amount);sT(null);}} onClose={()=>sT(null)}/>}</div>
}
  ...
  const settle = (sid, wid, payAmt) => {
    ...
    if (s.direction === "owe") {
      ...
      if (wid === "upi_lite") {
        ...
        if (u.day + amount > 5000) { showT(`UPI Lite daily cap ₹5000 exceeded (₹${u.day} used)`, "error"); return }
        if (u.month + amount > 100000) { showT(`UPI Lite monthly cap ₹1L exceeded`, "error"); return }
        if (u.day + amount > 4500) { showT(`Heads up: UPI Lite at ₹${roundMoney(u.day + amount)} today`, "info") }
      }
    }
    if (s.direction === "owed" && wid === "upi_lite") { showT("UPI Lite cannot receive money", "error"); return }
    const rec = { id: uid(), type: "settlement", splitName: s.name, splitId: s.id, amount, direction: s.direction, walletId: wid, date: today, createdAt: new Date().toISOString(), ...(s.groupId && { groupId: s.groupId }), ...(s.eventId && { eventId: s.eventId }) };
    sStl(p => [...p, rec]);
...
`}</style>
    ...
    {module === "finance" && <div style={{ padding: "0 16px" }}>
      ...
      {tab === "dashboard" && <div className="pe">
        ...
        <Splits splits={sp} settlements={stl} expanded={spX} onToggle={() => sSpX(!spX)} onAdd={s => { const sr = { ...s, createdAt: new Date().toISOString() }; sSp(p => [...p, sr]); sbUpsert("splits", [toSB(sr, ["id", "name", "amount", "direction", "settled", "eventId", "groupId", "note"])]) }} onSettle={settle} onDelete={id => { sSp(p => p.filter(s => s.id !== id)); sbDelete("splits", id); }} onSkip={id => { sSp(p => p.map(s => s.id === id ? {...s, settled: true, skipped: true} : s)); sbUpsert("splits", [{id, settled: true, skipped: true}]); showT("IOU skipped ✓"); }} wallets={wallets} />
