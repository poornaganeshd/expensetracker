...
function AddPage({ categories: cats, incomeSources: isrc, recurringCats: rCats, onAddExpense: oE, onAddIncome: oI, onAddTransfer: oT, onAddRec: oR }) {
  ...
  return <div style={{ padding: "0 0 20px" }}>
    ...
    {type !== "recurring" && <><div style={{ marginBottom: 16 }}><label style={ls}>Amount</label>
      ...
      {type !== "transfer" && <div style={{ marginBottom: 18 }}><ReceiptPicker key={submitKey} onUrlReady={url => { receiptUrlRef.current = url; }} /></div>}
...
function TxCard({ item: it, categories: cats, incomeSources: isrc, events: evs, onDelete: od, recurringCats: rCats }) {
  ...
  return <div style={{ ...cc, borderRadius: 14, padding: "14px 16px", display: "flex", alignItems: "center", gap: 12, marginBottom: 10 }}><div style={{ width: 44, height: 44, borderRadius: 12, background: (cat?.color || "#999") + "14", display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0 }}>{cat ? <DI2 id={cat.id} accent={cat.neon || cat.color} size={22} /> : <span style={{ fontSize: 22 }}>❓</span>}</div><div style={{ flex: 1, minWidth: 0 }}><div style={{ display: "flex", alignItems: "center", gap: 6, flexWrap: "wrap" }}><span style={{ fontSize: 14, fontWeight: 600, color: "var(--text)", fontFamily: "var(--font-h)" }}>{cat?.name || "Unknown"}</span>{isE && <span style={{ fontSize: 7, fontFamily: "var(--font-h)", fontWeight: 600, color: isFix(it) ? "#A78BFA" : "#FBBF24", background: isFix(it) ? "#A78BFA15" : "#FBBF2415", padding: "1px 5px", borderRadius: 3 }}>{isFix(it) ? "FIXED" : "FLEX"}</span>}{w && <span style={{ fontSize: 9, fontFamily: "var(--font-h)", fontWeight: 600, color: w.color, background: w.color + "18", padding: "2px 6px", borderRadius: 4, display: "inline-flex", alignItems: "center", gap: 2 }}><DI2 id={w.id} accent={w.neon || w.color} size={10} /></span>}</div><div style={{ fontSize: 12, color: "var(--muted)", fontFamily: "var(--font-b)", marginTop: 2, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{evT && <span style={{ fontWeight: 600, color: "var(--ts)" }}>{evT} · </span>}{it.note ? it.note + " · " : ""}{dl(it.date)}</div>{fxMeta && <div style={{ fontSize: 10, color: "#7B8CDE", fontFamily: "var(--font-h)", fontWeight: 600, marginTop: 3, letterSpacing: "0.3px" }}>{fxMeta.currency} {fxMeta.originalAmount} @ {Number(fxMeta.rateUsed).toFixed(2)}</div>}
    {(isE || isI) && it.receipt_url && <div style={{ marginTop: 3 }}><a href={it.receipt_url} target="_blank" rel="noopener noreferrer" style={{ fontSize: 10, color: "#6BAA75", fontFamily: "var(--font-h)", fontWeight: 600, textDecoration: "none" }}>🧾 Receipt</a></div>}</div><div style={{ fontFamily: "var(--font-h)", fontWeight: 600, fontSize: 15, color: isE ? "#E07A5F" : "#6BAA75", flexShrink: 0 }}>{isE ? "−" : "+"}{fmt(it.amount)}</div><button onClick={() => od(it.id, it.type)} style={{ background: "none", border: "none", color: "var(--muted)", cursor: "pointer", fontSize: 14, opacity: 0.35, flexShrink: 0 }}>✕</button></div>
}
...
export default function Nomad() {
  ...
  const [bResendKey, sBResendKey] = useState(_creds.resendKey || "");
  const [bResendFrom, sBResendFrom] = useState(_creds.resendFrom || "");
  ...
  const addE = data => {
    const w = WALLETS.find(x => x.id === data.walletId), amt = roundMoney(data.amount);
    ...
    sEx(p => [rec, ...p]);
    sbUpsert("expenses", [toSB(rec, ["id", "amount", "categoryId", "walletId", "note", "date", "eventId", "groupId", "receipt_url"])]);
    dance();
...
`}</style>
    ...
    {module === "finance" && <div style={{ padding: "0 16px" }}>
      ...
      {tab === "events" && <div className="pse"><Events events={evs} expenses={ex} splits={sp} settlements={stl} categories={cats} onCreate={ev => { sEvs(p => [...p, ev]); sbUpsert("events", [toSB(ev, ["id", "name", "emoji", "date", "status"])]) }} onAddExp={addE} onAddSplit={s => { sSp(p => [...p, s]); sbUpsert("splits", [toSB(s, ["id", "name", "amount", "direction", "settled", "eventId", "groupId"])]) }} onSettleSplit={settle} onDeleteSplit={id => { sSp(p => p.filter(s => s.id !== id)); sbDelete("splits", id) }} onMarkDone={id => { sEvs(p => p.map(e => e.id === id ? { ...e, status: "completed" } : e)); sbUpsert("events", [{ id, status: "completed" }]) }} /></div>}
      ...
      {tab === "settings" && <div className="pe" style={{ paddingTop: 8 }}>
        ...
        <div style={{ ...cc, padding: "14px 20px", marginBottom: 14 }}>
          <div onClick={() => sBackendOpen(v => !v)} style={{ display: "flex", alignItems: "center", justifyContent: "space-between", cursor: "pointer" }}>
            <div style={{ fontFamily: "var(--font-h)", fontSize: 12, color: "var(--muted)", letterSpacing: "0.5px", fontWeight: 600 }}>Backend</div>
            <span style={{ fontSize: 11, color: "var(--muted)" }}>{backendOpen ? "▲" : "▼"}</span>
          </div>
          {backendOpen && <div style={{ marginTop: 14 }}>
            {/* Status indicators */}
            <div style={{ display: "flex", flexDirection: "column", gap: 8, marginBottom: 14 }}>
              <div style={{ display: "flex", alignItems: "center", gap: 10, padding: "10px 12px", background: "var(--bg)", borderRadius: 10, border: "1px solid var(--border)" }}>
                <div style={{ width: 8, height: 8, borderRadius: "50%", background: _creds.sbUrl ? "#6BAA75" : "#FBBF24", flexShrink: 0 }} />
                <div style={{ flex: 1, minWidth: 0 }}>
                  <div style={{ fontSize: 11, fontWeight: 700, fontFamily: "var(--font-h)", color: "var(--text)" }}>Supabase</div>
                  <div style={{ fontSize: 11, color: "var(--muted)", fontFamily: "var(--font-b)", marginTop: 1, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{_creds.sbUrl ? _creds.sbUrl.replace("https://", "").replace(".supabase.co", "") + ".supabase.co" : "Not configured"}</div>
                </div>
              </div>
              <div style={{ display: "flex", alignItems: "center", gap: 10, padding: "10px 12px", background: "var(--bg)", borderRadius: 10, border: "1px solid var(--border)" }}>
                <div style={{ width: 8, height: 8, borderRadius: "50%", background: _creds.cloudName ? "#6BAA75" : "#FBBF24", flexShrink: 0 }} />
                <div style={{ flex: 1, minWidth: 0 }}>
                  <div style={{ fontSize: 11, fontWeight: 700, fontFamily: "var(--font-h)", color: "var(--text)" }}>Cloudinary</div>
                  <div style={{ fontSize: 11, color: "var(--muted)", fontFamily: "var(--font-b)", marginTop: 1 }}>{_creds.cloudName || "Not configured"}</div>
                </div>
              </div>
              <div style={{ display: "flex", alignItems: "center", gap: 10, padding: "10px 12px", background: "var(--bg)", borderRadius: 10, border: "1px solid var(--border)" }}>
                <div style={{ width: 8, height: 8, borderRadius: "50%", background: _creds.resendKey ? "#6BAA75" : "#FBBF24", flexShrink: 0 }} />
                <div style={{ flex: 1, minWidth: 0 }}>
                  <div style={{ fontSize: 11, fontWeight: 700, fontFamily: "var(--font-h)", color: "var(--text)" }}>Email Reports (Resend)</div>
                  <div style={{ fontSize: 11, color: "var(--muted)", fontFamily: "var(--font-b)", marginTop: 1 }}>{_creds.resendKey ? (_creds.resendFrom || "Key saved, no from-address") : "Not configured"}</div>
                </div>
              </div>
            </div>

            {/* Resend credentials edit */}
            <div style={{ borderTop: "1px solid var(--border)", paddingTop: 14, marginBottom: 14 }}>
              <div style={{ fontSize: 11, fontWeight: 700, color: "var(--muted)", letterSpacing: "0.5px", marginBottom: 10, fontFamily: "var(--font-h)" }}>📧 EMAIL REPORTS CREDENTIALS</div>
              <label style={{ ...ls }}>Resend API Key</label>
              <input value={bResendKey} onChange={e => sBResendKey(e.target.value)} placeholder="re_xxxxxxxxxxxxxxxxxxxx" style={{ ...is, marginBottom: 10 }} autoComplete="off" spellCheck={false} />
              <label style={{ ...ls }}>From Email</label>
              <input value={bResendFrom} onChange={e => sBResendFrom(e.target.value)} placeholder="NOMAD Reports <reports@yourdomain.com>" style={{ ...is, marginBottom: 10 }} autoComplete="off" spellCheck={false} />
              <button onClick={() => { const updated = { ..._creds, resendKey: bResendKey.trim(), resendFrom: bResendFrom.trim() }; localStorage.setItem("nomad-credentials", JSON.stringify(updated)); showT("Email credentials saved", "success"); }} style={{ width: "100%", padding: "10px", border: "none", borderRadius: 10, background: "#c9a96e", color: "#1a1a1a", fontFamily: "var(--font-h)", fontSize: 12, fontWeight: 700, cursor: "pointer", marginBottom: 10 }}>Save Email Credentials</button>
              <div style={{ fontSize: 11, color: "var(--muted)", lineHeight: 1.7, background: "var(--bg)", borderRadius: 8, padding: "10px 12px", fontFamily: "var(--font-b)" }}>
                ℹ️ Also add to <strong>Vercel → Settings → Environment Variables</strong>:<br />
                <span style={{ color: "#c9a96e", fontFamily: "monospace" }}>RESEND_API_KEY</span> · <span style={{ color: "#c9a96e", fontFamily: "monospace" }}>RESEND_FROM_EMAIL</span><br />
                <span style={{ color: "#c9a96e", fontFamily: "monospace" }}>SUPABASE_SERVICE_ROLE_KEY</span> · <span style={{ color: "#c9a96e", fontFamily: "monospace" }}>CRON_SECRET</span>
              </div>
            </div>

            {/* Export / Import / Edit */}
            <div style={{ display: "flex", gap: 8, marginBottom: 8 }}>
              <button onClick={() => { const data = JSON.stringify(_creds, null, 2); const a = document.createElement("a"); a.href = URL.createObjectURL(new Blob([data], { type: "application/json" })); a.download = "nomad_credentials.json"; a.click(); showT("Credentials exported", "success"); }} style={{ flex: 1, padding: "11px", border: "1.5px solid #6BAA75", borderRadius: 10, background: "#6BAA7512", color: "#6BAA75", fontFamily: "var(--font-h)", fontSize: 12, cursor: "pointer", fontWeight: 600 }}>Export</button>
              <label style={{ flex: 1, padding: "11px", border: "1.5px solid #7B8CDE", borderRadius: 10, background: "#7B8CDE12", color: "#7B8CDE", fontFamily: "var(--font-h)", fontSize: 12, cursor: "pointer", fontWeight: 600, textAlign: "center" }}>Import<input type="file" accept=".json" style={{ display: "none" }} onChange={e => { const f = e.target.files[0]; if (!f) return; const r = new FileReader(); r.onload = ev => { try { const d = JSON.parse(ev.target.result); if (!d.sbUrl || !d.sbKey) { showT("Invalid credentials file", "error"); return; } localStorage.setItem("nomad-credentials", JSON.stringify(d)); showT("Credentials imported — reloading…", "success"); setTimeout(() => window.location.reload(), 1000); } catch { showT("Failed to read file", "error"); } }; r.readAsText(f); e.target.value = ""; }} /></label>
            </div>
            <button onClick={() => setShowSetup(true)} style={{ width: "100%", padding: "11px", border: "1.5px solid var(--border)", borderRadius: 10, background: "var(--card)", color: "var(--muted)", fontFamily: "var(--font-h)", fontSize: 12, cursor: "pointer", fontWeight: 600 }}>Edit All Credentials</button>
          </div>}
        </div>
