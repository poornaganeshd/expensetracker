...
import {
  ...
  recurringDaysOverdue, distributeAmount, historySortCompare,
} from "./financeUtils";
const APP = "NOMAD", CUR = "₹";
...
function Splits({ splits: sp, settlements: stl, onAdd, onSettle: os, onDelete: od, onSkip: ok, expanded: exp, onToggle: ot, wallets: wl = WALLETS, walletBalances: wBalances = {}, onError = () => {} }) {
  ...
  const add = () => { if((!nm.trim()&&!drill)||!am) return; const a=parseAmount(am); if(!Number.isFinite(a)||a<=0) return; onAdd({id:uid(),name:drill||nm.trim(),amount:a,direction:dir,settled:false,note:snote.trim()||undefined}); sN(""); sA(""); sSnote("") };
...
export default function Nomad() {
  ...
  const historyItems = useMemo(() => {
    ...
    let items = searching
      ? [...ex.map(e => ({ ...e, type: "expense" })), ...inc.map(i => ({ ...i, type: "income" })), ...tr.map(t => ({ ...t, type: "transfer" }))]
      : [...flt.expenses.map(e => ({ ...e, type: "expense" })), ...flt.incomes.map(i => ({ ...i, type: "income" })), ...(fm === "all" ? tr : tr.filter(t => mk(t.date) === fm)).map(t => ({ ...t, type: "transfer" }))];
    if (searching) { const q = hSearch.toLowerCase().trim(); items = items.filter(it => (it.note || "").toLowerCase().includes(q) || (cats.find(c => c.id === it.categoryId)?.name || "").toLowerCase().includes(q) || (isrc.find(s => s.id === it.sourceId)?.name || "").toLowerCase().includes(q) || (evs.find(e => e.id === it.eventId)?.name || "").toLowerCase().includes(q)); }
  ...
  }, [flt, ex, inc, tr, fm, hSearch, hMinAmt, hMaxAmt, hDateFrom, hDateTo, hType, cats, isrc, evs]);
