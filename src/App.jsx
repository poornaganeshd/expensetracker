...
function AddPage({ categories: cats, incomeSources: isrc, recurringCats: rCats, onAddExpense: oE, onAddIncome: oI, onAddTransfer: oT, onAddRec: oR, onError: showT = () => {}, patterns = [], autoRules = [], onLearnRule = () => {}, wallets: aw = WALLETS, cloudinaryEnabled = false }) {
  ...
  const [splitPreview, sSplitPreview] = useState(null); // {splits[]} | null
  const [splitLoading, sSplitLoading] = useState(false);
  ...
  const extractItems = async () => {
    ...
    try {
      ...
      if (!data) { showT("No image to read", "error"); return; }
      const r = await fetch("/api/food-vision", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ ...data, type: "receipt-items" }) });
      const d = await r.json();
      if (!r.ok) throw new Error(d.error || "OCR failed");
      if (!Array.isArray(d.items) || d.items.length === 0) { showT("No line items found", "info"); return; }
      sItemsPreview(d);
    } catch (e) {
  ...
  const confirmItemsImport = () => {
    ...
    itemsPreview.items.forEach(it => {
      ...
      const ok = oE({ id: uid(), amount, categoryId: matchCatHint(it.category), walletId: wid, date, note: itemNote }) !== false;
      if (ok) added++;
    });
  ...
  const suggestSplit = async () => {
    ...
    if (!Number.isFinite(a) || a <= 0 || !note.trim()) { showT("Need amount + note", "info"); return; }
    sSplitLoading(true); sSplitPreview(null);
    try {
      const r = await fetch("/api/ai-analyze", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ mode: "split-cats", expense: { amount: a, note: note.trim(), categoryId: catId }, categories: cats.map(c => ({ id: c.id, name: c.name })) }) });
      const d = await r.json();
      if (!r.ok) throw new Error(d.error || "split failed");
      if (!Array.isArray(d.splits) || d.splits.length < 2) { showT("No split needed — looks like a single category", "info"); return; }
      // Lock original amount + note into the preview so confirm uses the right
      // expected total even if the form changes underneath.
      sSplitPreview({ ...d, originalAmount: a, originalNote: note.trim() });
    } catch (e) {
  ...
  const confirmSplitImport = () => {
    if (!splitPreview?.splits?.length) return;
    const baseNote = (splitPreview.originalNote || note.trim()).slice(0, 100);
    let added = 0;
    splitPreview.splits.forEach(sp => {
      const amount = roundMoney(Number(sp.amount) || 0);
      if (amount <= 0) return;
      const cid = cats.find(c => c.id === sp.categoryId)?.id || matchCatHint(sp.categoryId) || catId;
      const splitNote = (baseNote + (sp.reason ? ` · ${sp.reason}` : "")).slice(0, 120);
      const ok = oE({ id: uid(), amount, categoryId: cid, walletId: wid, date, note: splitNote }) !== false;
      if (ok) added++;
    });
    showT(`Split into ${added} entries`, "success");
    sSplitPreview(null);
    sAmt("0"); sNote("");
  };
  ...
  useEffect(() => { sSplitPreview(null); sItemsPreview(null); }, [type]);
  ...
  return <div style={{ padding: "0 0 20px" }}>
    ...
    {type !== "recurring" && (() => {
      ...
      return <>
        ...
        <div style={{ background: "var(--card)", border: "1px solid var(--border)", borderRadius: 22, padding: "18px 18px 8px", boxShadow: "0 2px 10px rgba(0,0,0,0.04)", marginBottom: 14 }}>
          ...
          {isExp && parseFloat(amt) > 0 && note.trim().length >= 3 && !splitPreview && <button onClick={suggestSplit} disabled={splitLoading} style={{ display: "flex", alignItems: "center", gap: 6, padding: "7px 11px", marginBottom: 10, borderRadius: 100, border: `1.5px dashed ${alpha("#A78BFA", 0.5)}`, background: alpha("#A78BFA", 0.08), color: "#A78BFA", fontFamily: "var(--font-h)", fontSize: 11.5, fontWeight: 700, cursor: splitLoading ? "default" : "pointer", opacity: splitLoading ? 0.6 : 1 }}><Robot size={12} weight="fill" />{splitLoading ? "Suggesting split…" : "Split across categories with AI"}</button>}
          {splitPreview && (() => { const sum = roundMoney(splitPreview.splits.reduce((s, sp) => s + (Number(sp.amount) || 0), 0)); const orig = roundMoney(splitPreview.originalAmount || 0); const drift = roundMoney(sum - orig); const driftBig = Math.abs(drift) > 1; return <div style={{ background: alpha("#A78BFA", 0.08), border: `1.5px solid ${alpha("#A78BFA", 0.4)}`, borderRadius: 14, padding: "10px 12px", marginBottom: 10 }}><div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 8 }}><div style={{ fontFamily: "var(--font-h)", fontSize: 11, fontWeight: 800, color: "#A78BFA", letterSpacing: ".5px" }}>AI SPLIT PREVIEW</div><span style={{ fontSize: 9.5, color: "var(--muted)", fontFamily: "var(--font-h)", fontWeight: 700 }}>{splitPreview.confidence}</span></div>{splitPreview.splits.map((sp, i) => { const c = cats.find(x => x.id === sp.categoryId) || { name: sp.categoryId, color: "#A78BFA" }; return <div key={i} style={{ display: "flex", alignItems: "center", gap: 7, padding: "5px 0", borderBottom: i < splitPreview.splits.length - 1 ? "1px dashed var(--border)" : "none", fontSize: 11.5 }}><span style={{ flex: 1, fontFamily: "var(--font-h)", fontWeight: 700, color: c.color, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{c.name}</span><span style={{ fontSize: 10, color: "var(--muted)", flex: 1, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{sp.reason}</span><span style={{ fontFamily: "var(--font-h)", fontWeight: 800, color: "var(--text)", flexShrink: 0 }}>{fmt(Number(sp.amount) || 0)}</span></div>; })}<div style={{ display: "flex", justifyContent: "space-between", marginTop: 8, padding: "6px 8px", borderRadius: 8, background: driftBig ? alpha("#D4726A", 0.12) : alpha("#A78BFA", 0.06), border: driftBig ? "1px solid #D4726A40" : "none" }}><span style={{ fontSize: 10.5, fontFamily: "var(--font-h)", color: driftBig ? "#D4726A" : "var(--muted)", fontWeight: 700 }}>{driftBig ? `Drift ${drift > 0 ? "+" : ""}${fmt(drift)} vs ${fmt(orig)}` : `Total ${fmt(sum)} matches`}</span><span style={{ fontSize: 10.5, fontFamily: "var(--font-h)", color: "var(--ts)", fontWeight: 800 }}>{splitPreview.splits.length} entries</span></div><div style={{ display: "flex", gap: 6, marginTop: 8 }}><button onClick={confirmSplitImport} style={{ flex: 2, padding: "8px", border: "none", borderRadius: 9, background: "#A78BFA", color: "#fff", fontFamily: "var(--font-h)", fontSize: 12, fontWeight: 800, cursor: "pointer" }}>Add {splitPreview.splits.length} entries</button><button onClick={() => sSplitPreview(null)} style={{ flex: 1, padding: "8px", border: "1.5px solid var(--border)", borderRadius: 9, background: "transparent", color: "var(--muted)", fontFamily: "var(--font-h)", fontSize: 12, fontWeight: 700, cursor: "pointer" }}>Cancel</button></div></div>; })()}
          <div style={{ marginBottom: 10 }}><ReceiptPicker ref={receiptPickerRef} cloudinaryEnabled={cloudinaryEnabled} /></div>
          {itemsPreview && (() => { const sum = roundMoney(itemsPreview.items.reduce((s, it) => s + (Number(it.amount) || 0), 0)); const total = roundMoney(itemsPreview.total || 0); const drift = roundMoney(sum - total); const driftBig = total > 0 && Math.abs(drift) > 5; return <div style={{ background: alpha(tc, 0.06), border: `1.5px solid ${alpha(tc, 0.4)}`, borderRadius: 14, padding: "10px 12px", marginBottom: 10 }}><div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 8 }}><div style={{ fontFamily: "var(--font-h)", fontSize: 11, fontWeight: 800, color: tc, letterSpacing: ".5px" }}>RECEIPT ITEMS · {itemsPreview.merchant || "(no merchant)"}</div><span style={{ fontSize: 9.5, color: "var(--muted)", fontFamily: "var(--font-h)", fontWeight: 700 }}>{itemsPreview.items.length} item{itemsPreview.items.length === 1 ? "" : "s"}</span></div>{itemsPreview.items.slice(0, 10).map((it, i) => <div key={i} style={{ display: "flex", alignItems: "center", gap: 7, padding: "5px 0", borderBottom: i < Math.min(itemsPreview.items.length, 10) - 1 ? "1px dashed var(--border)" : "none", fontSize: 11.5 }}><span style={{ flex: 2, fontFamily: "var(--font-h)", fontWeight: 700, color: "var(--text)", overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{it.name}</span><span style={{ fontSize: 9, color: "var(--muted)", flexShrink: 0 }}>{it.category || "Other"}</span><span style={{ fontFamily: "var(--font-h)", fontWeight: 800, color: tc, flexShrink: 0 }}>{fmt(Number(it.amount) || 0)}</span></div>)}{itemsPreview.items.length > 10 && <div style={{ fontSize: 10, color: "var(--muted)", marginTop: 4, textAlign: "center" }}>…and {itemsPreview.items.length - 10} more</div>}<div style={{ display: "flex", justifyContent: "space-between", marginTop: 8, padding: "6px 8px", borderRadius: 8, background: driftBig ? alpha("#D4726A", 0.12) : alpha(tc, 0.06), border: driftBig ? "1px solid #D4726A40" : "none" }}><span style={{ fontSize: 10.5, fontFamily: "var(--font-h)", color: driftBig ? "#D4726A" : "var(--muted)", fontWeight: 700 }}>{driftBig ? `Items sum ${fmt(sum)} ≠ receipt ${fmt(total)} (Δ${drift > 0 ? "+" : ""}${fmt(drift)})` : total > 0 ? `Items sum ${fmt(sum)} ≈ ${fmt(total)}` : `Items sum ${fmt(sum)}`}</span></div><div style={{ display: "flex", gap: 6, marginTop: 8 }}><button onClick={confirmItemsImport} style={{ flex: 2, padding: "8px", border: "none", borderRadius: 9, background: tc, color: "#fff", fontFamily: "var(--font-h)", fontSize: 12, fontWeight: 800, cursor: "pointer" }}>Add {itemsPreview.items.length} item{itemsPreview.items.length === 1 ? "" : "s"}</button><button onClick={() => sItemsPreview(null)} style={{ flex: 1, padding: "8px", border: "1.5px solid var(--border)", borderRadius: 9, background: "transparent", color: "var(--muted)", fontFamily: "var(--font-h)", fontSize: 12, fontWeight: 700, cursor: "pointer" }}>Cancel</button></div></div>; })()}
        </div>
    ...
    {type === "recurring" && <><div style={{ fontFamily: "var(--font-h)", fontSize: 10, color: "var(--muted)", letterSpacing: "1.5px", fontWeight: 700, marginBottom: 14 }}>BILL</div><label style={ls}>Name</label><input value={rName} onChange={e => sRN(e.target.value)} placeholder="e.g. Netflix, Rent…" style={{ ...is, marginBottom: 12 }} /><label style={ls}>Amount ({CUR})</label><input type="number" value={rAmt} onChange={e => sRA(e.target.value)} placeholder="0" style={{ ...is, fontSize: 24, fontWeight: 700, fontFamily: "var(--font-h)", textAlign: "center", padding: "14px", color: "#A78BFA", borderColor: "#A78BFA", marginBottom: 16 }} /><div style={{ height: 1, background: "var(--border)", margin: "4px 0 16px" }} /><div style={{ fontFamily: "var(--font-h)", fontSize: 10, color: "var(--muted)", letterSpacing: "1.5px", fontWeight: 700, marginBottom: 14 }}>CATEGORY</div><label style={ls}>Category</label><div style={{ display: "flex", flexWrap: "wrap", gap: 6, marginBottom: 12 }}>{(rCats || RC).map(c => <button key={c.id} onClick={() => sRC(c.id)} style={{ padding: "6px 12px", borderRadius: 8, fontSize: 12, fontFamily: "var(--font-b)", border: `1.5px solid ${rCat === c.id ? c.color : "var(--border)"}`, background: rCat === c.id ? c.color + "18" : "var(--card)", color: rCat === c.id ? c.color : "var(--ts)", cursor: "pointer", display: "flex", alignItems: "center", gap: 4 }}><DI2 id={c.id} accent={c.neon || c.color} size={13} />{c.name}</button>)}</div>
      {rCat === "other_rec" && <input value={rOther} onChange={e => sRO(e.target.value)} placeholder="Name this category…" style={{ ...is, marginBottom: 12 }} />}
      <div style={{ height: 1, background: "var(--border)", margin: "4px 0 16px" }} /><div style={{ fontFamily: "var(--font-h)", fontSize: 10, color: "var(--muted)", letterSpacing: "1.5px", fontWeight: 700, marginBottom: 14 }}>SCHEDULE</div><label style={ls}>Wallet</label><WB wallets={aw.filter(w => !isUpiLite(w))} sel={rWal} onSel={sRW} /><label style={ls}>Frequency</label><div style={{ display: "flex", gap: 6, marginBottom: 12, flexWrap: "wrap" }}>{[{ id: "monthly", label: "Monthly" }, { id: "yearly", label: "Yearly" }, { id: "custom", label: "Every X Days" }].map(f => <button key={f.id} onClick={() => sRF(f.id)} style={{ flex: 1, padding: "9px", borderRadius: 9, fontSize: 12, fontFamily: "var(--font-h)", border: `1.5px solid ${rFreq === f.id ? "#A78BFA" : "var(--border)"}`, background: rFreq === f.id ? "#A78BFA18" : "var(--card)", color: rFreq === f.id ? "#A78BFA" : "var(--muted)", cursor: "pointer", fontWeight: 600, whiteSpace: "nowrap" }}>{f.label}</button>)}</div>
...
function Events({ events: evs, expenses: ex, splits: sp, settlements: stl, categories: cats, wallets: wl = WALLETS, staleByEvent = {}, onCreate: oC, onAddExp: oE, onAddSplit: oS, onSettleSplit: oSS, onDeleteSplit: oDS, onMarkDone: oMD, onDelete: oD, dm = false }) {
  ...
  if (view === "detail" && sel) {
    ...
    return <div style={{ paddingTop: 8 }}><div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 16 }}><button onClick={() => sV("list")} style={{ background: "none", border: "none", color: "var(--muted)", cursor: "pointer", fontSize: 13, fontFamily: "var(--font-h)" }}>← Events</button><div style={{ display: "flex", gap: 8, alignItems: "center" }}>{sel.status === "active" && <button onClick={() => oMD(sel.id)} style={{ padding: "6px 14px", border: "1.5px solid #6BAA75", borderRadius: 8, background: "#6BAA7518", color: "#6BAA75", fontFamily: "var(--font-h)", fontSize: 11, fontWeight: 600, cursor: "pointer" }}>Mark Done ✓</button>}<button onClick={() => sEvDelConfirm(sel.id)} style={{ padding: "6px 12px", border: "1.5px solid #D4726A", borderRadius: 8, background: "#D4726A12", color: "#D4726A", fontFamily: "var(--font-h)", fontSize: 11, fontWeight: 600, cursor: "pointer", display: "flex", alignItems: "center", gap: 4 }}><Trash size={11} />Delete</button></div></div>
      ...
      {isGroup && allParts.length > 1 && <div style={{ ...cc, padding: 14, marginBottom: 14 }}><div style={{ fontFamily: "var(--font-h)", fontSize: 11, color: "var(--muted)", fontWeight: 600, marginBottom: 10, letterSpacing: "0.5px" }}>GROUP SUMMARY</div><div style={{ fontSize: 11, color: "var(--muted)", fontFamily: "var(--font-b)", marginBottom: 10 }}>Equal share: {fmt(grpShare)} / person ({allParts.length} people)</div>{allParts.map(p => { const paid = grpPaid[p] || 0, bal = roundMoney(paid - (grpShareMap[p] ?? grpShare) - (grpSettled[p] || 0)); const settled = Math.abs(bal) < 0.01, label = settled ? "settled" : bal > 0 ? `get back ${fmt(bal)}` : `owes ${fmt(Math.abs(bal))}`; const color = settled ? "var(--muted)" : bal > 0 ? "#6BAA75" : "#E07A5F"; return <div key={p} style={{ display: "flex", alignItems: "center", padding: "7px 0", borderBottom: "1px solid var(--border)" }}><span style={{ fontSize: 13, fontFamily: "var(--font-h)", color: "var(--text)", flex: 1 }}>{p}</span><span style={{ fontSize: 12, fontFamily: "var(--font-h)", color: "var(--ts)", marginRight: 12 }}>{fmt(paid)}</span><span style={{ fontSize: 11, fontFamily: "var(--font-h)", color, fontWeight: 600 }}>→ {label}</span></div>; })}{(() => { const bals = allParts.map(p => ({ name: p, bal: roundMoney((grpPaid[p] || 0) - (grpShareMap[p] ?? grpShare) - (grpSettled[p] || 0)) })); const cr = bals.filter(b => b.bal > 0.01).map(b => ({ ...b })).sort((a, b) => b.bal - a.bal); const db = bals.filter(b => b.bal < -0.01).map(b => ({ ...b })).sort((a, b) => a.bal - b.bal); const settlements = []; let ci = 0, di = 0; while (ci < cr.length && di < db.length) { const amt = roundMoney(Math.min(cr[ci].bal, -db[di].bal)); settlements.push({ from: db[di].name, to: cr[ci].name, amt }); cr[ci].bal = roundMoney(cr[ci].bal - amt); db[di].bal = roundMoney(db[di].bal + amt); if (cr[ci].bal < 0.01) ci++; if (db[di].bal > -0.01) di++; } return settlements.length > 0 && <div style={{ marginTop: 10, paddingTop: 10, borderTop: "1px solid var(--border)" }}><div style={{ fontSize: 10, color: "var(--muted)", fontFamily: "var(--font-h)", fontWeight: 600, marginBottom: 6, letterSpacing: "0.5px" }}>SUGGESTED SETTLEMENTS</div>{settlements.map((s, i) => <div key={i} style={{ display: "flex", alignItems: "center", gap: 6, fontSize: 12, fontFamily: "var(--font-h)", fontWeight: 600, marginBottom: 4, color: s.from === "You" ? "#E07A5F" : s.to === "You" ? "#6BAA75" : "var(--ts)" }}><span>{s.from}</span><span style={{ opacity: 0.5 }}>→</span><span>{s.to}</span><span style={{ marginLeft: "auto" }}>{fmt(s.amt)}</span></div>)}</div>; })()}</div>}
...
function NI({ type: t, active: a }) {
  ...
  if (t === "ai") return <svg width="22" height="22" viewBox="0 0 24 24" fill="none" stroke={c} strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M12 2L9 9L2 12L9 15L12 22L15 15L22 12L15 9Z" /></svg>;
  return null
}
...
export default function Nomad() {
  ...
  const impLedger = async (file) => {
    ...
    try {
      ...
      const r = await fetch("/api/food-vision", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ type: "ledger", imageBase64: base64, mimeType }) });
      const d = await r.json();
...
`}</style>
    ...
    {module === "finance" && <div style={{ padding: "0 16px" }}>
      ...
      {tab === "dashboard" && <div className="pe">
        ...
        <button onClick={() => sTab("ai")} style={{ width: "100%", padding: "11px 14px", borderRadius: 12, border: "1.5px solid #7B8CDE", background: "#7B8CDE15", color: "#7B8CDE", fontFamily: "var(--font-h)", fontSize: 12, fontWeight: 700, cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center", gap: 6, marginBottom: 14 }}><Sparkle size={16} weight="fill" /><span>AI Toolbox</span><span style={{ marginLeft: "auto", fontSize: 10, color: "#7B8CDE99", fontWeight: 600 }}>15 features</span></button>
...
{historyItems.map(it => { const tlBal = hTimeline ? timelineData[it.id] : null; const hasSB = hTimeline && (it.balBefore !== undefined || it.fromBalBefore !== undefined); const showTL = tlBal || hasSB; const tlWallets = showTL ? (it.type === "transfer" ? [{ id: it.fromWallet }, { id: it.toWallet }] : [{ id: it.type === "expense" ? (it.walletId || "upi_lite") : it.type === "income" ? (it.walletId || "bank") : it.walletId }]) : []; return <div key={it.id} style={{ position: "relative", ...(hTimeline ? { paddingLeft: 28, marginBottom: 4 } : {}) }}>{hTimeline && <div style={{ position: "absolute", left: 10, top: 0, bottom: 0, width: 2, background: "var(--border)" }} />}{hTimeline && <div style={{ position: "absolute", left: 5, top: 22, width: 12, height: 12, borderRadius: "50%", background: it.type === "expense" ? "#E07A5F" : it.type === "income" ? "#6BAA75" : "#7B8CDE", border: "2px solid var(--bg)", zIndex: 1 }} />}<div style={{ display: "flex", alignItems: "stretch", gap: 6 }}>{bulkMode && <div onClick={() => sBulkSel(p => { const n = new Set(p); n.has(it.id) ? n.delete(it.id) : n.add(it.id); return n; })} style={{ display: "flex", alignItems: "center", padding: "0 4px", cursor: "pointer", flexShrink: 0 }}><div style={{ width: 20, height: 20, borderRadius: 5, border: `2px solid ${bulkSel.has(it.id) ? "#E07A5F" : "var(--border)"}`, background: bulkSel.has(it.id) ? "#E07A5F" : "var(--card)", display: "flex", alignItems: "center", justifyContent: "center" }}>{bulkSel.has(it.id) && <span style={{ color: "#fff", fontSize: 11, fontWeight: 700 }}>✓</span>}</div></div>}<div style={{ flex: 1, minWidth: 0 }}><TxCard item={it} categories={cats} incomeSources={isrc} events={evs} onDelete={delItem} recurringCats={recCats} wallets={wallets} onRefund={refundItem} />{showTL && <div style={{ display: "flex", gap: 6, flexWrap: "wrap", padding: "0 4px 10px 4px", marginTop: -4 }}>{tlWallets.map(tw => { const wName = wallets.find(w => w.id === tw.id)?.name || tw.id; let bef, aft; if (it.type === "transfer" && it.fromBalBefore !== undefined) { bef = tw.id === it.fromWallet ? it.fromBalBefore : (it.toBalBefore ?? 0); aft = tw.id === it.fromWallet ? roundMoney(it.fromBalBefore - it.amount) : roundMoney((it.toBalBefore ?? 0) + it.amount); } else if (it.balBefore !== undefined) { bef = it.balBefore; aft = roundMoney(it.balBefore + (it.type === "expense" ? -it.amount : it.amount)); } else { bef = tlBal?.before[tw.id] ?? 0; aft = tlBal?.after[tw.id] ?? 0; } return <div key={tw.id} style={{ fontSize: 10, fontFamily: "var(--font-h)", color: "var(--muted)", background: "var(--bg)", borderRadius: 6, padding: "3px 8px", display: "inline-flex", alignItems: "center", gap: 5, border: "1px solid var(--border)" }}><span style={{ fontWeight: 600, color: "var(--ts)", fontSize: 9 }}>{wName}</span><span>{fmt(bef)}</span><span style={{ opacity: 0.4, fontSize: 9 }}>→</span><span style={{ fontWeight: 700, color: bef > aft ? "#E07A5F" : bef < aft ? "#6BAA75" : "var(--muted)" }}>{fmt(aft)}</span></div>; })}</div>}</div></div></div>; })}{historyItems.length === 0 && <div style={{ textAlign: "center", padding: "60px 20px", color: "var(--muted)", fontSize: 14, lineHeight: 1.8 }}>{flt.expenses.length === 0 && flt.incomes.length === 0 ? <><div style={{ marginBottom: 12 }}><ClipboardText size={32} color="var(--muted)" /></div><div style={{ fontFamily: "var(--font-h)", fontSize: 15, fontWeight: 600, color: "var(--ts)", marginBottom: 6 }}>No transactions yet</div><div style={{ fontSize: 12, marginBottom: 20 }}>Log expenses, income, and transfers<br />to see your spending history here.</div><button onClick={() => sTab("add")} style={{ padding: "12px 28px", border: "none", borderRadius: 12, background: "#E07A5F", color: "#fff", fontFamily: "var(--font-h)", fontSize: 13, fontWeight: 700, cursor: "pointer" }}>+ Add First Transaction</button></> : "No results match your filters."}</div>}</div>}
      ...
      {tab === "ai" && <div className="pe" style={{ paddingTop: 14, paddingBottom: 100 }}><AIHub expenses={exAll} incomes={inc} categories={cats} wallets={wallets} budgets={budgets} recurring={rec} onShowToast={showT} onApplyBudgets={(b) => { sBudgets(b); showT("Budgets applied — review in Settings", "success"); }} onApplyMerchantRules={(mappings) => { const SECTOR_ALIASES = { dining: ["food", "dining", "restaurant"], "food delivery": ["food", "dining"], transport: ["transport", "travel", "commute"], shopping: ["shopping", "misc"], utilities: ["utilities", "bills", "rent"], entertainment: ["entertainment", "fun", "leisure"], health: ["health", "medical", "fitness"], personal: ["personal", "misc"], groceries: ["groceries", "food"] }; const matchCat = (sector) => { const s = String(sector || "").toLowerCase().trim(); if (!s) return null; const direct = cats.find(c => c.name.toLowerCase() === s); if (direct) return direct.id; const aliases = SECTOR_ALIASES[s] || [s]; for (const a of aliases) { const m = cats.find(c => c.name.toLowerCase().includes(a) || a.includes(c.name.toLowerCase())); if (m) return m.id; } return null; }; const rules = mappings.filter(m => m.canonical && m.raw).map(m => ({ keyword: String(m.raw).toLowerCase().slice(0, 30), categoryId: matchCat(m.sector) })).filter(r => r.keyword && r.categoryId); if (rules.length === 0) { showT("No sectors matched your categories", "info"); return; } sAutoRules(prev => { const seen = new Set(prev.map(r => r.keyword)); const merged = [...prev]; rules.forEach(r => { if (!seen.has(r.keyword)) merged.push(r); }); return merged; }); showT(`Saved ${rules.length} of ${mappings.length} rule${rules.length === 1 ? "" : "s"}`, "success"); }} /></div>}
